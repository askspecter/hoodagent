...
* publish holt to npm via release-please ([#367](https://github.com/askspecter/holt/issues/367)) ([8eccc26](https://github.com/askspecter/holt/commit/8eccc2669887bc38d35bc16a315c888e4d9ec43a))
* **tui:** FILES sidebar panel with click-to-select and file drill-in ([#365](https://github.com/askspecter/holt/issues/365)) ([142c548](https://github.com/askspecter/holt/commit/142c548c89a8652ce300e64ddf1228ee36df7606))


### Bug Fixes

* **auth:** propagate credentials to every provider-build surface and pin children to the live provider ([#366](https://github.com/askspecter/holt/issues/366)) ([6e0a665](https://github.com/askspecter/holt/commit/6e0a665118fe0e09c4b07d482dd18f86045acd2b))
...
- Ten built-in color themes alongside the `dark`/`light` built-ins — `dracula`, `nord`, `gruvbox`,
  ...
  `solarized-light` — selectable via `/theme <name>`, `--theme <name>`, or `HOLT_THEME`. Every palette
...
- `--theme <name>` flag for the TUI, accepting `auto` or any registered theme (previously only the
  `HOLT_THEME` env var existed).
- "Accessibility / Appearance" section in the README documenting `NO_COLOR`, `HOLT_THEME`, `/theme`,
  and `HOLT_NO_FADE`.
...
- Auth (401/403) errors now show a curated, actionable message pointing at `holt auth` / setup; the
  raw upstream body is shown only under a verbose/debug flag.
- No-provider / missing-key errors now point at `holt setup` and `holt auth`, and distinguish a
  missing key from a rejected key.
- `holt doctor` no longer reports "Overall: pass" when no provider credential is configured, and
