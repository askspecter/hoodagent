...
func TestParseBuildArgsUsesEnvAndCliOverrides(t *testing.T) {
	env := func(key string) string {
		values := map[string]string{
			"HOLT_BUILD_GOOS":   "linux",
			"HOLT_BUILD_GOARCH": "arm64",
		}
		return values[key]
	}
	...
	options, help, err = parseBuildArgs([]string{"--goos=windows", "--goarch", "amd64", "--output", "dist/holt.exe"}, emptyEnv)
	if err != nil {
	...
	if options.GOOS != "windows" || options.GOARCH != "amd64" || options.Output != "dist/holt.exe" {
...
func TestParseSmokeArgsAcceptsPathAlias(t *testing.T) {
	options, help, err := parseSmokeArgs([]string{"--binary=dist/holt", "--goos", "linux", "--version", "0.1.0"})
	if err != nil {
	...
	if options.BinaryPath != "dist/holt" || options.GOOS != "linux" || options.Version != "0.1.0" {
...
func TestBuildHelpIncludesEnvironmentOverrides(t *testing.T) {
	...
	for _, want := range []string{"holt-release build", "--goos", "--goarch", "HOLT_BUILD_GOOS"} {
