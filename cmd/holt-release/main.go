...
import (
	...
	"github.com/askspecter/holt/internal/release"
)
...
func run(args []string, stdout io.Writer, stderr io.Writer) int {
	if len(args) == 0 {
		_, _ = fmt.Fprintln(stderr, "holt-release command required. Use `holt-release --help`.")
		return 2
	}
	switch args[0] {
	case "-h", "--help", "help":
	...
	default:
		_, _ = fmt.Fprintf(stderr, "unknown holt-release command %q\n", args[0])
		return 2
	}
}
...
func runBuild(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "[holt] Build failed: "+err.Error())
		return 1
	}
...
func runPackage(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "[holt] Release packaging failed: "+err.Error())
		return 1
	}
...
func runSmoke(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "[holt] Smoke failed: "+err.Error())
		return 1
	}
...
func runVerify(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "[holt] Release checksum verification failed: "+err.Error())
		return 1
	}
...
func parseBuildArgs(args []string, getenv func(string) string) (release.BuildOptions, bool, error) {
	options := release.BuildOptions{
		GOOS:   strings.TrimSpace(getenv("HOLT_BUILD_GOOS")),
		GOARCH: strings.TrimSpace(getenv("HOLT_BUILD_GOARCH")),
	}
...
func writeHelp(w io.Writer) error {
  ...
  holt-release <command>

Commands:
  build      Build the Go-native holt binary
  package    Build and package the current platform release archive
  smoke      Verify the built holt binary prints the package version
...
func writeBuildHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  holt-release build [flags]

Builds the Go-native holt binary.
...
Environment overrides:
  HOLT_BUILD_GOOS, HOLT_BUILD_GOARCH
`)
	return err
}
...
func writePackageHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  holt-release package [flags]

Builds the Go-native holt binary, stages npm wrapper files, writes a release
...
func writeSmokeHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  holt-release smoke [flags]

Runs the built holt binary with --version and verifies it prints the package
version.

Flags:
      --root <path>     Repository root (default: current directory)
      --path <path>     Binary path (default: ./holt or ./holt.exe)
