...
// taskOptions configures the `holt-perf-bench tasks` subcommand: the reproducible
// Terminal-Bench-style harness that runs HOLT headlessly against a task set and
...
func runTasksCommand(args []string, getenv func(string) string, stdout io.Writer, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "[holt] Task benchmark failed: "+err.Error())
		return 1
	}
	...
	if !options.DryRun {
		...
		if binary == "" {
			_, _ = fmt.Fprintln(stderr, "[holt] Task benchmark failed: --binary is required unless --dry-run is set")
			return 2
		}
...
func parseTaskArgs(args []string, getenv func(string) string) (taskOptions, error) {
	options := taskOptions{
		Version: strings.TrimSpace(getenv("HOLT_BENCH_VERSION")),
		Commit:  strings.TrimSpace(getenv("HOLT_BENCH_COMMIT")),
	}
...
func taskHelpText() string {
	return strings.Join([]string{
		"Usage: holt-perf-bench tasks [options]",
		"",
		"Runs HOLT headlessly against a Terminal-Bench-style task set and records a",
		...
		"  --binary <path>     Path to the `holt` binary (required unless --dry-run)",
		"  --version <v>       Record the HOLT version (default: $HOLT_BENCH_VERSION)",
		"  --commit <sha>      Record the HOLT commit (default: $HOLT_BENCH_COMMIT)",
