...
func TestParseArgsUsesCliAndEnvOverrides(t *testing.T) {
	env := func(key string) string {
		values := map[string]string{
			"HOLT_PERF_COLD_START_WARN_MS":      "250",
			"HOLT_PERF_HARNESS_END_RSS_WARN_MB": "384",
		}
		return values[key]
	}
...
func TestParseArgsUsesEnvOnlyThreshold(t *testing.T) {
	options, err := parseArgs(nil, func(key string) string {
		if key == "HOLT_PERF_FIRST_OUTPUT_WARN_MS" {
			return "610"
		}
		return ""
	})
...
func TestParseArgsRejectsInvalidValues(t *testing.T) {
	...
	if _, err := parseArgs(nil, func(key string) string {
		if key == "HOLT_PERF_ITERATIONS" {
			return "nope"
		}
		return ""
	}); err == nil || !strings.Contains(err.Error(), "HOLT_PERF_ITERATIONS must be a positive integer") {
...
func TestHelpText(t *testing.T) {
	...
	for _, want := range []string{"Usage: holt-perf-bench", "--iterations", "HOLT_PERF_ITERATIONS"} {
