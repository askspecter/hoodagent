...
import (
	...
	"github.com/askspecter/holt/internal/perfbench"
)
...
func run(args []string, getenv func(string) string, stdout io.Writer, stderr io.Writer) int {
	...
	if err != nil {
		_, _ = fmt.Fprintln(stderr, "[holt] Performance benchmark failed: "+err.Error())
		return 1
	}
	if options.Output != "" {
...
func parseArgs(args []string, getenv func(string) string) (cliOptions, error) {
	iterations, err := readPositiveIntegerEnv(getenv, "HOLT_PERF_ITERATIONS", defaultIterations)
	if err != nil {
		return cliOptions{}, err
	}
	warmupIterations, err := readNonNegativeIntegerEnv(getenv, "HOLT_PERF_WARMUP_ITERATIONS", defaultWarmupIterations)
	if err != nil {
		return cliOptions{}, err
	}
	coldStartP95Ms, err := readPositiveNumberEnv(getenv, "HOLT_PERF_COLD_START_WARN_MS", perfbench.DefaultThresholds.ColdStartP95Ms)
	if err != nil {
		return cliOptions{}, err
	}
	firstOutputP95Ms, err := readPositiveNumberEnv(getenv, "HOLT_PERF_FIRST_OUTPUT_WARN_MS", perfbench.DefaultThresholds.FirstOutputP95Ms)
	if err != nil {
		return cliOptions{}, err
	}
	harnessEndRssMaxMb, err := readPositiveNumberEnv(getenv, "HOLT_PERF_HARNESS_END_RSS_WARN_MB", perfbench.DefaultThresholds.HarnessEndRssMaxMb)
	if err != nil {
		return cliOptions{}, err
	}
	options := cliOptions{
...
func helpText() string {
	return strings.Join([]string{
		"Usage: holt-perf-bench [options]",
		"       holt-perf-bench tasks [options]   (Terminal-Bench-style task harness; see `tasks --help`)",
		"",
		"Options:",
		...
		"  HOLT_PERF_ITERATIONS, HOLT_PERF_WARMUP_ITERATIONS",
		"  HOLT_PERF_COLD_START_WARN_MS, HOLT_PERF_FIRST_OUTPUT_WARN_MS",
		"  HOLT_PERF_HARNESS_END_RSS_WARN_MB",
	}, "\n") + "\n"
}
