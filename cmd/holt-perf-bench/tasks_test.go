...
func TestParseTaskArgsReadsFlagsAndEnv(t *testing.T) {
	env := func(key string) string {
		return map[string]string{
			"HOLT_BENCH_COMMIT":  "deadbeef",
			"HOLT_BENCH_VERSION": "9.9.9",
		}[key]
	}
	...
	if options.Binary != "/usr/local/bin/holt" || options.Output != "dist/bench.json" {
...
func TestParseTaskArgsFlagsOverrideEnv(t *testing.T) {
	env := func(key string) string {
		return map[string]string{"HOLT_BENCH_COMMIT": "fromenv"}[key]
	}
...
func TestRunTasksCommandWritesRecordWithoutBinary(t *testing.T) {
	...
	// needing a real holt binary or model.
...
func TestTasksSubcommandRoutedFromMain(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "holt-perf-bench tasks") {
