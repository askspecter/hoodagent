...
func TestPackageBinPointsToNodeWrapper(t *testing.T) {
	...
	if pkg.Name != "@askspecter/holt" {
		t.Fatalf("name = %q, want @askspecter/holt", pkg.Name)
	}
	if pkg.Bin["holt"] != "bin/holt.js" {
		t.Fatalf("bin.holt = %q, want bin/holt.js", pkg.Bin["holt"])
	}
	if pkg.Module != "bin/holt.js" {
		t.Fatalf("module = %q, want bin/holt.js", pkg.Module)
	}
	...
	wantFiles := map[string]bool{"bin/holt.js": false, "scripts/postinstall.mjs": false}
...
func TestPostinstallComputesAssetPlan(t *testing.T) {
	...
	cases := []struct {
		...
		{"linux", "x64", "holt-v" + version + "-linux-x64.tar.gz", "holt"},
		{"darwin", "arm64", "holt-v" + version + "-macos-arm64.tar.gz", "holt"},
		{"win32", "x64", "holt-v" + version + "-windows-x64.zip", "holt.exe"},
	}
	for _, tc := range cases {
		stdout, stderr, err := runPostinstall(t,
			"HOLT_INSTALL_DRY_RUN=1",
			"HOLT_INSTALL_PLATFORM="+tc.platform,
			"HOLT_INSTALL_ARCH="+tc.arch,
		)
		if err != nil {
		...
		wantURL := "https://github.com/askspecter/holt/releases/download/v" + version + "/" + tc.wantAsset
...
func TestPostinstallSkipsUnsupportedPlatform(t *testing.T) {
	stdout, stderr, err := runPostinstall(t,
		"HOLT_INSTALL_DRY_RUN=1",
		"HOLT_INSTALL_PLATFORM=plan9",
		"HOLT_INSTALL_ARCH=x64",
	)
	if err != nil {
...
func TestPostinstallSkipsWindowsArm64(t *testing.T) {
	...
	stdout, stderr, err := runPostinstall(t,
		"HOLT_INSTALL_DRY_RUN=1",
		"HOLT_INSTALL_PLATFORM=win32",
		"HOLT_INSTALL_ARCH=arm64",
	)
	if err != nil {
...
func TestPostinstallHonorsSkipEnv(t *testing.T) {
	stdout, stderr, err := runPostinstall(t, "HOLT_SKIP_DOWNLOAD=1")
	if err != nil {
		t.Fatalf("HOLT_SKIP_DOWNLOAD should exit 0, got err=%v stderr=%s", err, stderr)
	}
	if strings.TrimSpace(stdout) != "" {
...
func TestNodeWrapperIsExecutableAndDoesNotImportBun(t *testing.T) {
	root := repoRoot(t)
	wrapperPath := filepath.Join(root, "bin", "holt.js")
...
func TestNodeWrapperReportsMissingNativeBinary(t *testing.T) {
	...
	command.Env = append(withoutEnvKey(command.Env, "HOLT_LOCAL_CONTROL_HELPERS"), "HOLT_LOCAL_CONTROL_HELPERS=")
...
func TestNodeWrapperLaunchesNativeBinary(t *testing.T) {
	...
	nativePath := filepath.Join(root, "holt")
	if err := os.WriteFile(nativePath, []byte("#!/usr/bin/env sh\nprintf 'mock-holt'; for arg in \"$@\"; do printf ' %s' \"$arg\"; done; printf '\\n'\n"), 0o755); err != nil {
	...
	if got := strings.TrimSpace(string(output)); got != "mock-holt --version" {
...
func TestNodeWrapperClearsInheritedLocalControlHelperManifestWhenNoHelpers(t *testing.T) {
	...
	root := filepath.Dir(filepath.Dir(wrapperPath))
	nativePath := filepath.Join(root, "holt")
	if err := os.WriteFile(nativePath, []byte("#!/usr/bin/env sh\nif [ -n \"${HOLT_LOCAL_CONTROL_HELPERS+x}\" ]; then printf 'set\\n'; else printf 'unset\\n'; fi\n"), 0o755); err != nil {
		t.Fatalf("WriteFile native fixture: %v", err)
	}
	...
	defer cancel()
	command := nodeWrapperCommand(ctx, node, wrapperPath, "--version")
	command.Env = append(withoutEnvKey(command.Env, "HOLT_LOCAL_CONTROL_HELPERS"), `HOLT_LOCAL_CONTROL_HELPERS={"version":1,"helpers":{"agent-browser":{"command":"stale"}}}`)
	output, err := command.CombinedOutput()
	if ctx.Err() != nil {
		t.Fatalf("wrapper timed out launching native binary: %v; output: %s", ctx.Err(), output)
	}
	...
	if got := strings.TrimSpace(string(output)); got != "unset" {
		t.Fatalf("HOLT_LOCAL_CONTROL_HELPERS state = %q, want unset", got)
	}
}
...
func copyWrapperFixture(t *testing.T) string {
	t.Helper()
	root := repoRoot(t)
	bytes, err := os.ReadFile(filepath.Join(root, "bin", "holt.js"))
	if err != nil {
		t.Fatalf("ReadFile wrapper: %v", err)
	}
	dir := t.TempDir()
	...
	wrapperPath := filepath.Join(binDir, "holt.js")
