...
func TestInspectSummarizesChangesAndRedactsDiff(t *testing.T) {
	...
	runner := &fakeRunner{results: []CommandResult{
		...
		{Stdout: "feature/m5\n"},
		{Stdout: "abc1234\n"},
		{Stdout: " M internal/verify/verify.go\x00?? internal/holtgit/holtgit.go\x00"},
		{Stdout: "abc1234\n"},
		{},
		{},
	...
	if summary.Files[1].Path != "internal/holtgit/holtgit.go" || summary.Files[1].Status != "untracked" || !summary.Files[1].Untracked {
...
func TestInspectPreviewIncludesUntrackedOnlyChanges(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(root, "notes.md"), "hello holt\n")
	...
	if !strings.Contains(summary.Diff, "diff --git a/notes.md b/notes.md") || !strings.Contains(summary.Diff, "+hello holt") {
...
func TestInspectBaseRefRealGitDiffsBranchAgainstBase(t *testing.T) {
	...
	runGitCommand(t, root, "-c", "user.name=Holt", "-c", "user.email=holt@example.invalid", "commit", "-m", "Add feature")
...
func initGitRepo(t *testing.T, withCommit bool) string {
	...
	if withCommit {
		...
		runGitCommand(t, root, "-c", "user.name=Holt", "-c", "user.email=holt@example.invalid", "commit", "-m", "Initial commit")
	}
	return root
}
