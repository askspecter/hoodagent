...
func TestBuildHasStableJSONContractAndCategoryMath(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, root, "HOLT.md", strings.Repeat("holt rules\n", 16))
	...
	report, err := Build(Options{
		...
		ProjectContextFiles: []string{"AGENTS.md", "HOLT.md"},
	})
	if err != nil {
		t.Fatalf("Build returned error: %v", err)
	}
	...
	if report.ProjectGuidelineFile != "HOLT.md" {
		t.Fatalf("ProjectGuidelineFile = %q, want HOLT.md", report.ProjectGuidelineFile)
	}
	if report.FreeTokens+report.UsedTokens != report.ContextWindow {
		t.Fatalf("free + used = %d, want %d", report.FreeTokens+report.UsedTokens, report.ContextWindow)
	}
...
func TestBuildAccountsForWorkspaceMapContext(t *testing.T) {
	...
	writeTestFile(t, root, "cmd/holt/main.go", "package main\n")
...
func TestFormatIncludesRootModelAndCategories(t *testing.T) {
	...
	for _, want := range []string{"Holt context report", "root: D:/repo", "model: gpt-4.1", "api_model: gpt-4.1", "System prompt", "Free"} {
