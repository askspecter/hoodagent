...
func initGitBaseline(ctx context.Context, workspace string) error {
	commands := [][]string{
		{"init"},
		{"config", "user.name", "Holt Eval"},
		{"config", "user.email", "holt-eval@example.invalid"},
		{"add", "."},
		{"commit", "-m", "baseline"},
	}
	for _, args := range commands {
		...
		cmd.Env = append(os.Environ(),
			"GIT_AUTHOR_NAME=Holt Eval",
			"GIT_AUTHOR_EMAIL=holt-eval@example.invalid",
			"GIT_COMMITTER_NAME=Holt Eval",
			"GIT_COMMITTER_EMAIL=holt-eval@example.invalid",
		)
