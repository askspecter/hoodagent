...
func TestDefaultDirHonorsEnvOverride(t *testing.T) {
	got := DefaultDir(map[string]string{"HOLT_SKILLS_DIR": "/custom/skills"})
...
func TestDefaultDirHonorsXDGDataHome(t *testing.T) {
	...
	want := filepath.Join("/xdg/data", "holt", "skills")
	if got != want {
...
func TestDefaultDirFallsBackToHome(t *testing.T) {
	got := DefaultDir(map[string]string{"HOME": "/home/holt"})
	want := filepath.Join("/home/holt", ".local", "share", "holt", "skills")
	if got != want {
		t.Fatalf("DefaultDir = %q, want %q", got, want)
	}
}
