...
// explicit HOLT_SKILLS_DIR override wins; otherwise it is
// $XDG_DATA_HOME/holt/skills or ~/.local/share/holt/skills. The directory is
// NOT created — a missing directory simply yields no skills.
func DefaultDir(env map[string]string) string {
	if override := strings.TrimSpace(envValue(env, "HOLT_SKILLS_DIR")); override != "" {
		return override
	}
	...
	if base == "" {
		if home == "" {
			...
			// here (".local/share/holt/skills") would bind skills to the process
			// CWD, so signal "no skills dir" and let the caller handle it.
			return ""
		}
		base = filepath.Join(home, ".local", "share")
	}
	return filepath.Join(base, "holt", "skills")
}
...
// any such collisions.
//
// NOTE: Load currently scans a single root (HOLT_SKILLS_DIR / the data dir).
...
// large) Content body — handy for `holt skills` listings.
