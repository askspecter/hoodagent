...
func backendLifecycleSnapshot(deps appDeps) (holtcommands.BackendLifecycleSnapshot, error) {
	cwd, err := deps.getwd()
	if err != nil {
		return holtcommands.BackendLifecycleSnapshot{}, fmt.Errorf("failed to resolve workspace: %w", err)
	}

	cfg, err := deps.resolveMCPConfig(cwd)
	if err != nil {
		return holtcommands.BackendLifecycleSnapshot{}, err
	}
	...
	return holtcommands.NewBackendLifecycleSnapshot(servers, hookResult.Config.Hooks, pluginResult.Plugins), nil
}

func backendDoctorReport(deps appDeps) (holtcommands.BackendDoctorReport, error) {
	cwd, err := deps.getwd()
	if err != nil {
		return holtcommands.BackendDoctorReport{}, fmt.Errorf("failed to resolve workspace: %w", err)
	}

	cfg, err := deps.resolveMCPConfig(cwd)
	if err != nil {
		return holtcommands.BackendDoctorReport{}, err
	}
	hookResult, err := deps.loadHooks(hooks.LoadOptions{Cwd: cwd})
	if err != nil {
		return holtcommands.BackendDoctorReport{}, err
	...
	return holtcommands.NewBackendDoctorReport(holtcommands.BackendDoctorInput{
		MCP:     cfg,
		Hooks:   hookResult,
...
func formatBackendLifecycleSnapshot(snapshot holtcommands.BackendLifecycleSnapshot) string {
	lines := []string{"Holt Backends:"}
...
func formatBackendDoctorReport(report holtcommands.BackendDoctorReport) string {
	lines := []string{
		"Holt backend doctor",
