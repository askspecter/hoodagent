...
func TestRunWorktreesPrepareTextAndJSON(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		t.Run(strings.Join(args, " "), func(t *testing.T) {
			...
			} else if !strings.Contains(stdout.String(), "Holt worktree ready") || !strings.Contains(stdout.String(), prepared.Path) {
...
func TestRunVerifyTextAndJSON(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		t.Run(strings.Join(args, " "), func(t *testing.T) {
			...
			} else if !strings.Contains(stdout.String(), "Holt verification") || !strings.Contains(stdout.String(), "go.test") || !strings.Contains(stdout.String(), cwd) || !strings.Contains(stdout.String(), "tests: 2 total, 1 passed, 1 failed") {
...
func TestRunVerifyAttemptsFormatsSelfVerifyText(t *testing.T) {
	...
	for _, want := range []string{"Holt self-verification", "root: " + cwd, "stop: passed", "attempt 1: failed", "remediation: applied - prepared retry", "attempt 2: passed"} {
...
func TestRunChangesInspectAndCommit(t *testing.T) {
	cwd := t.TempDir()
	summary := holtgit.ChangeSummary{
		Root:   cwd,
		Branch: "main",
		Commit: "abc1234",
		Files:  []holtgit.FileChange{{Path: "README.md", Status: "modified", Unstaged: true}},
	}
	commit := holtgit.CommitResult{
		Root:       cwd,
	...
	t.Run("inspect json", func(t *testing.T) {
		...
		exitCode := runWithDeps([]string{"changes", "inspect", "--json"}, &stdout, &stderr, appDeps{
			getwd: func() (string, error) { return cwd, nil },
			inspectChanges: func(ctx context.Context, options holtgit.InspectOptions) (holtgit.ChangeSummary, error) {
				if options.Cwd != cwd {
		...
		var decoded holtgit.ChangeSummary
		if err := json.Unmarshal(stdout.Bytes(), &decoded); err != nil {
		...
		var snapshot holtgit.ChangeSnapshot
		if err := json.Unmarshal(stdout.Bytes(), &snapshot); err != nil {
			t.Fatalf("decode changes snapshot JSON: %v\n%s", err, stdout.String())
		}
		if snapshot.Contract != holtgit.ChangeContractVersion || len(snapshot.Events) == 0 {
	...
	t.Run("commit dry-run", func(t *testing.T) {
		...
		exitCode := runWithDeps([]string{"changes", "commit", "--message", "Update README", "--dry-run"}, &stdout, &stderr, appDeps{
			getwd: func() (string, error) { return cwd, nil },
			commitChanges: func(ctx context.Context, options holtgit.CommitOptions) (holtgit.CommitResult, error) {
		...
		if !strings.Contains(stdout.String(), "Holt changes commit") || !strings.Contains(stdout.String(), "dry-run: true") {
...
func TestRunChangesInspectThreadsBaseRef(t *testing.T) {
	...
	summary := holtgit.ChangeSummary{
		...
		Branch: "feature",
		Base:   "main",
		Files:  []holtgit.FileChange{{Path: "feature.md", Status: "added"}},
	}
...
func TestRunChangesCommitRejectsBaseRef(t *testing.T) {
	...
	exitCode := runWithDeps([]string{"changes", "commit", "--base", "main"}, &stdout, &stderr, appDeps{
		...
		commitChanges: func(context.Context, holtgit.CommitOptions) (holtgit.CommitResult, error) {
			...
			return holtgit.CommitResult{}, nil
		},
	})
...
func TestRunExecRejectsWorktreeDirWithoutWorktree(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	exitCode := runWithDeps([]string{"exec", "--worktree-dir", "/tmp/holt", "hello"}, &stdout, &stderr, appDeps{
		prepareWorktree: func(context.Context, worktrees.Options) (worktrees.Result, error) {
			t.Fatal("prepareWorktree should not be called for invalid flags")
			return worktrees.Result{}, nil
		},
