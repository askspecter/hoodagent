...
func TestRunDoctorFormatsRedactedProviderDiagnostics(t *testing.T) {
	...
	for _, want := range []string{"Holt doctor report", "Overall: pass", "[pass] provider.config", "[warn] provider.connectivity"} {
...
func TestRunDoctorReportsConfigValidationForMalformedFile(t *testing.T) {
	cwd := t.TempDir()
	holtDir := filepath.Join(cwd, ".holt")
	if err := os.MkdirAll(holtDir, 0o755); err != nil {
		t.Fatalf("mkdir .holt: %v", err)
	}
	if err := os.WriteFile(filepath.Join(holtDir, "config.json"), []byte("{\n  \"activeProvider\": \"openai\",\n"), 0o600); err != nil {
	...
	exitCode := runWithDeps([]string{"doctor", "--json"}, &stdout, &stderr, appDeps{
		...
		resolveConfig: func(string, config.Overrides) (config.ResolvedConfig, error) {
			return config.ResolvedConfig{}, fmt.Errorf("invalid config JSON %s: unexpected end of JSON input", filepath.Join(holtDir, "config.json"))
		},
