...
import (
	...
	"github.com/askspecter/holt/internal/agenteval"
)
...
func TestRunEvalRunJSONModePassesRunnerOptions(t *testing.T) {
	...
	exitCode := runWithDeps([]string{
		...
		"--task", "edit-reader",
		"--workspace", "D:\\work\\holt-fixture",
		"--json",
	}, &stdout, &stderr, appDeps{
		runAgentEval: func(ctx context.Context, options agentEvalOptions) (agentEvalReport, error) {
			if options.Mode != "run" || options.SuitePath != "evals/context.json" || options.TaskID != "edit-reader" || options.WorkspacePath != "D:\\work\\holt-fixture" || !options.JSON {
...
func TestRunEvalBenchJSONModePassesHarnessOptions(t *testing.T) {
	...
	exitCode := runWithDeps([]string{
		...
		"--suite", "evals/context.json",
		"--task", "edit-reader",
		"--work-root", "D:\\tmp\\holt-evals",
		"--json",
		"--agent-command", "holt", "exec", "{prompt}",
	}, &stdout, &stderr, appDeps{
		runAgentEval: func(ctx context.Context, options agentEvalOptions) (agentEvalReport, error) {
			if options.Mode != "bench" || options.SuitePath != "evals/context.json" || options.TaskID != "edit-reader" || options.WorkRoot != "D:\\tmp\\holt-evals" || !options.JSON {
				t.Fatalf("unexpected eval bench options: %#v", options)
			}
			if got, want := strings.Join(options.AgentCommand, "\x00"), strings.Join([]string{"holt", "exec", "{prompt}"}, "\x00"); got != want {
				t.Fatalf("agent command = %#v, want holt exec {prompt}", options.AgentCommand)
			}
			return agentEvalReport{
				Suite:  "quality-context",
				TaskID: "edit-reader",
...
func TestRunEvalBenchTextOutputSurfacesWorkRoot(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "work-root: /tmp/holt-eval-abc") {
...
func TestRunEvalRunRejectsBenchOnlyFlags(t *testing.T) {
	tests := []struct {
		...
		{
			name: "work root",
			args: []string{"eval", "run", "--suite", "evals/context.json", "--work-root", "D:\\tmp\\holt-evals"},
		...
		{
			name: "agent command",
			args: []string{"eval", "run", "--suite", "evals/context.json", "--agent-command", "holt", "exec", "{prompt}"},
...
func TestRunEvalDefaultRunnerLoadsSuite(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{
		...
		"tasks": [{
			...
			"workspaceFixture": "fixtures/holt",
			"verificationCommands": [
...
func TestRunEvalFailingSuiteReturnsProviderExit(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "Holt agent eval") || !strings.Contains(stdout.String(), "context.recall") {
