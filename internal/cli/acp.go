...
import (
	...
	"github.com/askspecter/holt/internal/acp"
...
const acpUsage = `holt acp — serve the Agent Client Protocol (ACP) over stdio

Editors that speak ACP (Zed, JetBrains, Neovim, ...) spawn this command and drive
HOLT as a backend over JSON-RPC 2.0 on stdin/stdout. HOLT keeps your provider,
...
// runACP serves ACP over stdio so an editor can drive HOLT's agent core. It
// speaks JSON-RPC 2.0 (newline-delimited JSON) on stdin/stdout; stderr stays free
// for human-readable diagnostics. The session lifecycle maps onto HOLT's own
// session store, and provider/model/keys remain owned by HOLT.
func runACP(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	acp.NewAgent(conn, acp.Deps{
		...
		AgentInfo:            acp.Implementation{Name: "holt", Version: version},
	})
...
// editor must not be able to point HOLT's file/shell tools at the whole disk.
