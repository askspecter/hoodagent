...
func runSessionsList(store *sessions.Store, options sessionCommandOptions, stdout io.Writer, stderr io.Writer) int {
	...
	if options.json {
		if err := writePrettyJSON(stdout, redaction.RedactValue(holtcommands.SessionSnapshots(items), redaction.Options{})); err != nil {
			return exitCrash
		}
		return exitSuccess
	}
	if _, err := fmt.Fprintln(stdout, formatSessionSnapshotsList(holtcommands.SessionSnapshots(items))); err != nil {
		return exitCrash
	}
	return exitSuccess
}
...
func runSessionsLineage(store *sessions.Store, sessionID string, options sessionCommandOptions, stdout io.Writer, stderr io.Writer) int {
	...
	if options.json {
		if err := writePrettyJSON(stdout, redaction.RedactValue(holtcommands.SessionSnapshots(lineage), redaction.Options{})); err != nil {
			return exitCrash
		}
		return exitSuccess
	}
	ids := make([]string, 0, len(lineage))
	for _, session := range holtcommands.SessionSnapshots(lineage) {
...
func runSessionsTree(store *sessions.Store, sessionID string, options sessionCommandOptions, stdout io.Writer, stderr io.Writer) int {
	...
	if options.json {
		if err := writePrettyJSON(stdout, redaction.RedactValue(holtcommands.SessionTreeSnapshotFromNode(tree), redaction.Options{})); err != nil {
			return exitCrash
		}
		return exitSuccess
	}
	if _, err := fmt.Fprint(stdout, formatSessionSnapshotTree(holtcommands.SessionTreeSnapshotFromNode(tree))); err != nil {
		return exitCrash
	}
	return exitSuccess
}
...
func runSessionsRewind(store *sessions.Store, sessionID string, options sessionCommandOptions, stdout io.Writer, stderr io.Writer) int {
	...
	if session == nil {
		return writeExecUsageError(stderr, "Holt session not found: "+redact(sessionID))
	}
...
func writeSessionCommandError(stderr io.Writer, err error) int {
	message := strings.TrimPrefix(err.Error(), "holt session")
	if message != err.Error() {
		message = "Holt session" + message
	}
	if strings.Contains(message, "not found") || strings.Contains(message, "invalid holt session id") {
...
func formatSessionSnapshotsList(items []holtcommands.SessionSnapshot) string {
	if len(items) == 0 {
		return "No Holt sessions found."
	}
	lines := []string{fmt.Sprintf("Holt sessions (%d):", len(items))}
...
func formatSessionSnapshotTree(node holtcommands.SessionTreeSnapshot) string {
	lines := []string{"Holt session tree:"}
	appendSessionSnapshotTree(&lines, node, "")
	return strings.Join(lines, "\n") + "\n"
}

func appendSessionSnapshotTree(lines *[]string, node holtcommands.SessionTreeSnapshot, prefix string) {
...
func formatSessionSnapshotLine(session holtcommands.SessionSnapshot) string {
...
func writeSessionsHelp(w io.Writer) error {
  ...
  holt sessions <command> [flags]

Commands:
  list                  List local Holt sessions
