...
func runSandbox(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "sandbox subcommand required. Use `holt sandbox policy` or `holt sandbox grants list`.")
	}
	switch args[0] {
	case "-h", "--help", "help":
...
func runSandboxPolicy(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	policy := applyConfiguredSandboxPolicy(holtSandbox.DefaultPolicy(), resolved.Sandbox)
	backend := deps.selectSandboxBackend(holtSandbox.BackendOptions{})
	...
	if options.effective {
		...
		// exists) must not crash `holt sandbox policy --effective` — fall
		// back to the workspace root and surface the error visibly instead.
		writeRoots := []string{workspaceRoot}
		var writeRootsErr error
		if scope, scopeErr := holtSandbox.NewScope(workspaceRoot, resolved.Sandbox.AdditionalWriteRoots); scopeErr != nil {
			...
			if fallback, fallbackErr := holtSandbox.NewScope(workspaceRoot, nil); fallbackErr == nil {
	...
	if options.json {
		payload := struct {
			Policy     holtSandbox.Policy      `json:"policy"`
			Backend    holtSandbox.Backend     `json:"backend"`
			Plan       holtSandbox.BackendPlan `json:"plan"`
...
func runSandboxSetup(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if backend.Name != holtSandbox.BackendWindowsRestrictedToken || !backend.Available || backend.Executable == "" {
	...
	scope, err := holtSandbox.NewScope(workspaceRoot, resolved.Sandbox.AdditionalWriteRoots)
	if err != nil {
		return writeExecUsageError(stderr, err.Error())
	}
	profile := holtSandbox.PermissionProfileFromPolicy(workspaceRoot, policy, scope)
	// Resolve the setup helper the same way the runner is resolved: a standalone
	// .exe when shipped (release), else self-dispatch via the running holt binary
	// (dev / plain build). This mirrors the backend's command-runner resolution
	// so `holt sandbox setup` works in every layout, not just release.
	setupHelper := holtSandbox.ResolveWindowsSandboxSetupHelper(nil)
	if !setupHelper.Available() {
		return writeAppError(stderr, "Windows sandbox setup helper is not available", exitProvider)
	}
	setupArgs, err := holtSandbox.BuildWindowsSandboxSetupArgs(holtSandbox.WindowsSandboxSetupArgsOptions{
...
type sandboxSetupResult struct {
	...
	Backend  holtSandbox.BackendName `json:"backend"`
...
func resolveSandboxGuards(policy holtSandbox.Policy) sandboxGuards {
	return sandboxGuards{
		...
		Network:            policy.Network == holtSandbox.NetworkDeny,
		Workspace:          policy.EnforceWorkspace,
	}
}

func runSandboxPolicyEffective(options sandboxCommandOptions, workspaceRoot string, policy holtSandbox.Policy, backend holtSandbox.Backend, plan holtSandbox.BackendPlan, grantsPath string, writeRoots []string, writeRootsErr error, stdout io.Writer) int {
...
func formatEffectiveSandboxPolicy(workspaceRoot string, policy holtSandbox.Policy, backend holtSandbox.Backend, plan holtSandbox.BackendPlan, guards sandboxGuards, grantsPath string, writeRoots []string, writeRootsErr error) string {
	lines := []string{
		"Holt effective sandbox policy",
		"root: " + workspaceRoot,
...
func runSandboxGrants(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "sandbox grants subcommand required. Use `holt sandbox grants list`.")
	}
	switch args[0] {
	case "-h", "--help", "help":
	...
	case "list":
		...
		if options.json {
			if err := writePrettyJSON(stdout, struct {
				Grants          []holtSandbox.Grant              `json:"grants"`
				CommandPrefixes []holtSandbox.CommandPrefixGrant `json:"commandPrefixes,omitempty"`
			}{Grants: grants, CommandPrefixes: prefixes}); err != nil {
				return exitCrash
			}
			return exitSuccess
		}
		if _, err := fmt.Fprintln(stdout, holtSandbox.FormatGrantListWithCommandPrefixes(grants, prefixes)); err != nil {
			return exitCrash
		}
		return exitSuccess
	case "allow", "deny":
...
func runSandboxGrantSet(command string, args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(positional) != 1 {
		return writeExecUsageError(stderr, "usage: holt sandbox grants "+command+" <tool> [--path file] [--reason text] [--json]")
	}
	decision := holtSandbox.GrantAllow
	if command == "deny" {
		decision = holtSandbox.GrantDeny
	}
	store, err := deps.newSandboxStore()
	if err != nil {
		return writeAppError(stderr, err.Error(), exitCrash)
	}
	input := holtSandbox.GrantInput{
	...
	if options.path != "" {
		...
		input.Scope = abs
		input.ScopeKind = holtSandbox.ScopeFile
	}
	...
	if options.json {
		if err := writePrettyJSON(stdout, struct {
			Grant holtSandbox.Grant `json:"grant"`
		}{Grant: grant}); err != nil {
			return exitCrash
		}
		return exitSuccess
...
func runSandboxGrantRevoke(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(positional) != 1 {
		return writeExecUsageError(stderr, "usage: holt sandbox grants revoke <tool> [--path file] [--json]")
	}
	store, err := deps.newSandboxStore()
	if err != nil {
		return writeAppError(stderr, err.Error(), exitCrash)
...
func runSandboxGrantClear(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if !options.confirm {
		return writeExecUsageError(stderr, "holt sandbox grants clear requires --confirm")
	}
	store, err := deps.newSandboxStore()
	if err != nil {
		return writeAppError(stderr, err.Error(), exitCrash)
...
func formatSandboxPolicy(workspaceRoot string, policy holtSandbox.Policy, backend holtSandbox.Backend, plan holtSandbox.BackendPlan, grantsPath string) string {
	lines := []string{
		"Holt sandbox policy",
		"root: " + workspaceRoot,
		"mode: " + string(policy.Mode),
...
func writeSandboxGrantSetHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  holt sandbox grants allow <tool> [flags]
  holt sandbox grants deny <tool> [flags]
...
func writeSandboxGrantRevokeHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  holt sandbox grants revoke <tool> [flags]
...
func writeSandboxGrantClearHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  holt sandbox grants clear --confirm [flags]
