...
func TestRunExecUseSpecCreatesDraftSession(t *testing.T) {
	...
	if draft.SpecID == "" || !strings.Contains(filepath.ToSlash(draft.SpecFilePath), ".holt/specs/") {
...
func (provider *submitSpecExecProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	...
	case <-ctx.Done():
		close(ch)
		return ch, ctx.Err()
	case ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: specmode.SubmitToolName}:
	}
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: string(arguments)}
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"}
