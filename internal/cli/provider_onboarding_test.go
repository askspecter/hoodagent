...
func TestRunProvidersUseSetsActiveProvider(t *testing.T) {
	...
	for _, want := range []string{"Active provider set to fast", "holt providers check fast"} {
...
func TestRunProvidersSetupPrintsCommandPlan(t *testing.T) {
	...
	for _, want := range []string{
		...
		"holt providers add groq --name fast --model llama-3.1-70b --base-url https://gateway.example/v1 --api-key-env FAST_API_KEY",
		"holt providers check fast --connectivity",
...
func TestRunProvidersSetupJSONIncludesCommands(t *testing.T) {
	...
	if payload.CatalogID != "groq" ||
		payload.Name != "fast" ||
		payload.AddCommand != "holt providers add groq --name fast --api-key-env GROQ_API_KEY --set-active" ||
		payload.CheckCommand != "holt providers check fast --connectivity" ||
		payload.UseCommand != "" ||
...
func TestRunProvidersSetupHelpListsOnboardingCommands(t *testing.T) {
	...
	output := stdout.String()
	for _, want := range []string{"holt providers use <name>", "holt providers setup <catalog-id>"} {
		if !strings.Contains(output, want) {
