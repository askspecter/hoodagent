...
// Distribution command handlers: `holt skill {add,info,remove}`,
// `holt plugin {add,remove}`, and `holt tools {make,list}`. These wire the
...
func runSkillAdd(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if source == "" {
		return writeExecUsageError(stderr, "usage: holt skill add <git-url|path> [--force] [--json]")
	}
	if dir == "" {
...
func runSkillInfo(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if name == "" {
		return writeExecUsageError(stderr, "usage: holt skill info <name> [--json]")
	}
...
func runSkillRemove(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if name == "" {
		return writeExecUsageError(stderr, "usage: holt skill remove <name>")
	}
...
func runPluginAdd(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if source == "" {
		return writeExecUsageError(stderr, "usage: holt plugin add <git-url|path> [--force] [--json]")
	}
	if dir == "" {
...
func runPluginRemove(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if id == "" {
		return writeExecUsageError(stderr, "usage: holt plugin remove <id>")
	}
...
func runTools(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "tools subcommand required. Use `holt tools make <name>` or `holt tools list`.")
	}
	switch args[0] {
	case "-h", "--help", "help":
...
func runToolsMake(args []string, dir string, stdout io.Writer, stderr io.Writer) int {
	...
	if name == "" {
		return writeExecUsageError(stderr, "usage: holt tools make <name> [--runtime shell|node|python] [--description text]")
	}
	if dir == "" {
	...
	lines := []string{
		...
		"  2. Run `holt tools list` to confirm it loads.",
...
func formatToolboxList(items []toolboxItem, diagnostics []plugins.Diagnostic, dir string) string {
	lines := []string{}
	if len(items) == 0 {
		lines = append(lines, fmt.Sprintf("No Holt plugin-tools found in %s.", dir))
	} else {
		lines = append(lines, "Holt Tools:")
...
func writeSkillAddHelp(w io.Writer) error {
  ...
  holt skill add <git-url|path> [flags]
...
func writePluginAddHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  holt plugin add <git-url|path> [flags]
