...
func writeAgentEvalHelp(w io.Writer) error {
  ...
  holt eval --suite <path> [flags]
  holt eval run --suite <path> [flags]
  holt eval bench --suite <path> [flags] [--model <id>] [--agent-command <argv...>]
...
func agentEvalBenchWorkRoot(options agentEvalOptions) (string, string, error) {
	...
	created, err := os.MkdirTemp("", "holt-eval-")
	if err != nil {
