...
// sandboxCheckReport is the combined snapshot `holt sandbox check` emits: the
...
// tool. It is the production consumer of the holtcommands sandbox-snapshot
...
type sandboxCheckReport struct {
	...
	Plan     holtcommands.SandboxPlanSnapshot       `json:"plan"`
	Decision holtcommands.SandboxDecisionSnapshot   `json:"decision"`
	Grant    holtcommands.SandboxGrantMatchSnapshot `json:"grant"`
}
...
func runSandboxCheck(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if strings.TrimSpace(options.tool) == "" {
		return writeExecUsageError(stderr, "tool name is required: holt sandbox check <tool> [flags]")
	}
	...
	engine := holtSandbox.NewEngine(holtSandbox.EngineOptions{
	...
	defer stop()
	decision := engine.Evaluate(ctx, holtSandbox.Request{
		WorkspaceRoot:  workspaceRoot,
		ToolName:       options.tool,
		SideEffect:     sideEffect,
		PermissionMode: holtSandbox.PermissionModeAsk,
		Args:           requestArgs,
	...
	report := sandboxCheckReport{
		...
		Plan:     holtcommands.SandboxPlanSnapshotFromPlan(plan),
		Decision: holtcommands.SandboxDecisionSnapshotFromDecision(decision),
		Grant:    holtcommands.SandboxGrantMatchSnapshotFromLookup(options.tool, lookup),
	}
...
func parseSandboxCheckSideEffect(value string) (holtSandbox.SideEffect, error) {
	switch strings.ToLower(strings.TrimSpace(value)) {
	case "", "read":
		return holtSandbox.SideEffectRead, nil
	case "write":
		return holtSandbox.SideEffectWrite, nil
	case "shell":
		return holtSandbox.SideEffectShell, nil
	case "network":
		return holtSandbox.SideEffectNetwork, nil
	case "none":
		return holtSandbox.SideEffectNone, nil
	default:
