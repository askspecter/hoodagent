...
func stubInspectChanges(stat string) func(context.Context, holtgit.InspectOptions) (holtgit.ChangeSummary, error) {
	return func(context.Context, holtgit.InspectOptions) (holtgit.ChangeSummary, error) {
		return holtgit.ChangeSummary{Root: "/repo", DiffStat: stat}, nil
	}
}
...
func TestRunUsageHelp(t *testing.T) {
	...
	for _, want := range []string{"holt usage report", "--json", "--days", "--since", "--session"} {
...
// with no usage events exits successfully, prints the header, shows a holt
