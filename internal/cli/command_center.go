...
type configSummary = holtcommands.ConfigSnapshot
type providerSummary = holtcommands.ProviderSnapshot
type modelSummary = holtcommands.ModelSnapshot
type providerCatalogSummary = holtcommands.ProviderCatalogSnapshot
...
func summarizeConfig(resolved config.ResolvedConfig) configSummary {
	summary := holtcommands.ConfigSnapshotFromResolved(resolved)
...
func listModelSummaries(registry modelregistry.Registry, options commandCenterOptions) ([]modelSummary, error) {
	summaries, err := holtcommands.ModelSnapshots(registry, holtcommands.ModelSnapshotOptions{
...
func listProviderCatalogSummaries(options commandCenterOptions) ([]providerCatalogSummary, error) {
	summaries, err := holtcommands.ProviderCatalogSnapshots(holtcommands.ProviderCatalogSnapshotOptions{
...
func formatProviderCatalogLine(provider providerCatalogSummary) string {
	...
	if provider.RuntimeSupported {
		lines = append(lines, "    setup: holt providers setup "+displayCLIValue(provider.ID, "unknown")+" --set-active")
	} else {
...
func writeModelsHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  holt models list [flags]

Lists Holt model registry entries.
...
func writeProvidersHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  holt providers current [flags]
  holt providers list [flags]
  holt providers catalog [flags]
  holt providers add <catalog-id> [flags]
  holt providers check [name] [flags]
  holt providers use <name> [flags]
  holt providers setup <catalog-id> [flags]
  holt providers detect [flags]
  holt providers models [name] [flags]
