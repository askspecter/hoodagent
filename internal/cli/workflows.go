...
func runChanges(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	switch command {
	case "inspect", "status":
		summary, err := deps.inspectChanges(context.Background(), holtgit.InspectOptions{
			Cwd:          workspaceRoot,
		...
		if options.json {
			if err := writePrettyJSON(stdout, holtgit.SnapshotFromSummary(safeSummary)); err != nil {
				return exitCrash
			}
			return exitSuccess
		}
		...
		return exitSuccess
	case "commit":
		result, err := deps.commitChanges(context.Background(), holtgit.CommitOptions{
			Cwd:          workspaceRoot,
...
func parseChangesArgs(args []string, command string) (changesCommandOptions, bool, error) {
	...
	if command != "commit" && (options.message != "" || options.dryRun) {
		return options, false, execUsageError{"--message and --dry-run are only valid with `holt changes commit`"}
	}
	if command == "commit" && options.baseRef != "" {
		return options, false, execUsageError{"--base is only valid with `holt changes inspect`"}
	}
	return options, false, nil
}
...
func redactChangeSummary(summary holtgit.ChangeSummary) holtgit.ChangeSummary {
...
func redactCommitResult(result holtgit.CommitResult) holtgit.CommitResult {
...
func formatChangeSummary(summary holtgit.ChangeSummary) string {
	lines := []string{
		"Holt changes",
		"root: " + summary.Root,
...
func formatCommitResult(result holtgit.CommitResult) string {
	lines := []string{
		"Holt changes commit",
		"root: " + result.Root,
...
func writeWorktreesHelp(w io.Writer) error {
  ...
  holt worktrees prepare [flags] [name]

Prepares an isolated git worktree for a Holt task.

Flags:
      --name <name>       Worktree name; defaults to a timestamped task name
      --dir <path>        Base directory for Holt worktrees
...
func writeChangesHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `Usage:
  holt changes inspect [flags]
  holt changes commit [flags]
...
Flags:
  ...
  -m, --message <text>    Commit message for `+"`holt changes commit`"+`
