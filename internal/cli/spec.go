...
func parseSpecArgs(args []string) (string, string, specCommandOptions, bool, error) {
	...
	if len(args) == 0 {
		return "", "", options, false, execUsageError{"spec command required. Use `holt spec show <spec>`."}
	}
	command := ""
	target := ""
	...
	if strings.TrimSpace(target) == "" {
		return command, target, options, false, execUsageError{fmt.Sprintf("holt spec %s requires a spec id or draft session id", command)}
	}
	if options.commentProvided && command != "approve" {
		return command, target, options, false, execUsageError{"--comment is only valid for holt spec approve"}
	}
	if options.reasonProvided && command != "reject" {
		return command, target, options, false, execUsageError{"--reason is only valid for holt spec reject"}
	}
...
func resolveSpecReviewTarget(store *sessions.Store, target string) (sessions.Metadata, error) {
	...
	if sessions.ValidSessionID(target) {
		...
		if session != nil {
			if session.SpecID == "" || session.SpecFilePath == "" {
				return sessions.Metadata{}, fmt.Errorf("holt session has no recorded spec: %s", redact(target))
			}
			return *session, nil
		}
	}
	...
	if len(draftMatches) > 1 {
		return sessions.Metadata{}, fmt.Errorf("holt spec id is ambiguous: %s; use the draft session id", redact(target))
	}
	if len(matches) == 0 {
		return sessions.Metadata{}, fmt.Errorf("holt spec not found: %s", redact(target))
	}
	if len(matches) > 1 {
		return sessions.Metadata{}, fmt.Errorf("holt spec id is ambiguous: %s; use the draft session id", redact(target))
	}
...
func runSpecApprove(store *sessions.Store, draft sessions.Metadata, options specCommandOptions, stdout io.Writer, stderr io.Writer) int {
	if draft.SessionKind != sessions.SessionKindSpecDraft {
		return writeExecUsageError(stderr, "holt spec approve requires a spec-draft session")
	}
	if draft.SpecStatus == sessions.SpecStatusRejected {
		return writeExecUsageError(stderr, "holt spec approve cannot approve a rejected spec")
	}
	if draft.SpecStatus == sessions.SpecStatusApproved && draft.SpecImplSessionID != "" {
		return writeSpecResult(stdout, options, specCommandResult{
			...
			Next:                    "holt exec --resume " + draft.SpecImplSessionID + ` "Start implementation"`,
		})
	}
	body, path, err := specmode.LoadSpecFile(draft.Cwd, draft.SpecFilePath)
	if err != nil {
	...
	return writeSpecResult(stdout, options, specCommandResult{
		...
		Next:                    "holt exec --resume " + impl.SessionID + ` "Start implementation"`,
	})
}
...
func runSpecReject(store *sessions.Store, draft sessions.Metadata, options specCommandOptions, stdout io.Writer, stderr io.Writer) int {
	if draft.SessionKind != sessions.SessionKindSpecDraft {
		return writeExecUsageError(stderr, "holt spec reject requires a spec-draft session")
	}
	if draft.SpecStatus == sessions.SpecStatusApproved && draft.SpecImplSessionID != "" {
		return writeExecUsageError(stderr, "holt spec reject cannot reject an approved spec with an implementation session")
	}
	updated, _, err := store.RecordSpec(draft.SessionID, sessions.RecordSpecInput{
		SpecID:              draft.SpecID,
		SpecFilePath:        draft.SpecFilePath,
	...
	return writeSpecResult(stdout, options, specCommandResult{
		...
		Next:           "Run holt exec --use-spec again with a revised task.",
	})
}
...
func writeSpecHelp(w io.Writer) error {
  ...
  holt spec show <spec-id|draft-session-id> [--json]
  holt spec approve <spec-id|draft-session-id> [--comment <text>] [--json]
  holt spec reject <spec-id|draft-session-id> [--reason <text>] [--json]
