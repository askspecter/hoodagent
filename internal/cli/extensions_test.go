...
func TestRunPluginsListsJSONAndText(t *testing.T) {
	...
	if payload.Plugins[0].ID != "holt.docs" || payload.Plugins[0].Name != "Docs" || payload.Plugins[0].Version != "1.0.0" {
	...
	for _, want := range []string{"Holt Plugins:", "holt.docs", "Docs", "1.0.0", "1 prompts"} {
...
func TestRunHooksListsRedactedJSONAndText(t *testing.T) {
	...
	if payload.Hooks.Hooks[0].ID != "holt.preflight" {
	...
	if !strings.Contains(stdout.String(), "Holt Hooks") || !strings.Contains(stdout.String(), "holt.preflight") {
...
func TestRunMCPPermissionsHelpDoesNotOpenStore(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		t.Run(strings.Join(args, " "), func(t *testing.T) {
			...
			if !strings.Contains(stdout.String(), "holt mcp permissions") {
