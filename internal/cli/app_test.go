...
func TestRunPrintsVersion(t *testing.T) {
	...
	if got := stdout.String(); got != "holt dev\n" {
...
func TestRunNoArgsLaunchesSetupTUIWithNilProviderWhenNoProviderConfigured(t *testing.T) {
	...
	projectConfigPath := filepath.Join(cwd, ".holt", "config.json")
	...
	userConfigPath := filepath.Join(t.TempDir(), "holt", "config.json")
...
func TestRunSkipPermissionsUnsafeLaunchesTUIInUnsafeMode(t *testing.T) {
	...
	// "holt --skip-permissions-unsafe" must launch the interactive TUI in unsafe
...
func TestRunSetupProviderWritesActiveConfig(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "Holt setup complete") || !strings.Contains(stdout.String(), "next: holt") {
...
func TestRunUpdateCheckTextAndJSON(t *testing.T) {
	result := update.Result{
		CurrentVersion: "dev",
		LatestVersion:  "0.2.0",
		ReleaseURL:     "https://github.com/askspecter/holt/releases/tag/v0.2.0",
		TagName:        "v0.2.0",
		ReleaseAsset: update.AssetCheck{
			Platform:      "linux",
			Arch:          "x64",
			ArchiveName:   "holt-v0.2.0-linux-x64.tar.gz",
			ArchiveURL:    "https://example.test/holt-v0.2.0-linux-x64.tar.gz",
			ChecksumName:  "holt-v0.2.0-linux-x64.tar.gz.sha256",
			ChecksumURL:   "https://example.test/holt-v0.2.0-linux-x64.tar.gz.sha256",
			ArchiveFound:  true,
			ChecksumFound: true,
			Verified:      true,
		},
	...
	if !strings.Contains(stdout.String(), "Release asset: holt-v0.2.0-linux-x64.tar.gz") || !strings.Contains(stdout.String(), "Checksum asset: holt-v0.2.0-linux-x64.tar.gz.sha256") {
	...
	exitCode := runWithDeps([]string{"update", "--check"}, &stdout, &stderr, appDeps{
		checkUpdate: func(context.Context, update.Options) (update.Result, error) {
			return update.Result{
				CurrentVersion:  "dev",
				LatestVersion:   "dev",
				ReleaseURL:      "https://github.com/askspecter/holt/releases/tag/dev",
				TagName:         "dev",
...
func (cliFakeProvider) StreamCompletion(context.Context, holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
...
func TestRunThemeFlagPopulatesTUIOptions(t *testing.T) {
	...
	// over HOLT_THEME). Previously Options.Theme was read but never set by the CLI.
...
// A custom endpoint saved without a model previously bricked bare `holt` and
// `holt setup` — the exact commands that could have fixed it (the resolve
...
func TestRunNoArgsEntersSetupWhenActiveProviderMissesModel(t *testing.T) {
	...
	exitCode := runWithDeps([]string{}, &stdout, &stderr, appDeps{
		...
		userConfigPath: func() (string, error) { return filepath.Join(t.TempDir(), "holt", "config.json"), nil },
		registerMCPTools: func(context.Context, *tools.Registry, config.MCPConfig, mcp.RegisterOptions) (mcpToolRuntime, error) {
			return noopMCPRuntime{}, nil
		},
		runTUI: func(_ context.Context, options tui.Options) int {
...
// `holt login`/`holt logout` don't exist (it's `holt auth login`); first-run
...
func TestRunUnknownLoginSuggestsAuthLogin(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), `"holt auth login"`) {
		t.Fatalf("stderr = %q, want a holt auth login suggestion", stderr.String())
	}
