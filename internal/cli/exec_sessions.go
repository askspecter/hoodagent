...
func preflightExecSession(options execOptions) error {
	...
	case options.fork != "":
		...
		if session == nil {
			return execUsageError{"Holt session not found: " + options.fork}
		}
	case options.resume != "":
		...
		if session == nil {
			return execUsageError{"Holt session not found: " + options.resume}
		}
	case options.resumeLatest:
		...
		if latest == nil {
			return execUsageError{"No Holt sessions available to resume."}
		}
	}
	return nil
}
...
func (recorder *execSessionRecorder) warnIfRecordingFailed(stderr io.Writer) {
	if recorder.err != nil {
		fmt.Fprintf(stderr, "[holt] WARNING: session not fully recorded: %v\n", recorder.err)
	}
}
