...
import (
	...
	"github.com/askspecter/holt/internal/observability"
	...
	"github.com/askspecter/holt/internal/provideronboarding"
	...
	"github.com/askspecter/holt/internal/skills"
	"github.com/askspecter/holt/internal/specialist"
	"github.com/askspecter/holt/internal/swarm"
...
type appDeps struct {
	...
	inspectChanges         func(context.Context, holtgit.InspectOptions) (holtgit.ChangeSummary, error)
	commitChanges          func(context.Context, holtgit.CommitOptions) (holtgit.CommitResult, error)
...
func defaultAppDeps() appDeps {
	return appDeps{
		...
		newProvider: func(profile config.ProviderProfile) (holtruntime.Provider, error) {
		...
		inspectChanges:   holtgit.Inspect,
		commitChanges:    holtgit.Commit,
		runTUI:           tui.Run,
...
// ($XDG_CONFIG_HOME/holt/plugins) used as the install target for `plugin add`
...
func runWithDeps(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) (exitCode int) {
	...
	// is shipped (dev / plain `go build`), the sandbox launches the running holt
	...
	// HOLT_THEME). Re-split --add-dir afterward so it may appear on either side of --theme.
	...
	case "--skip-permissions-unsafe":
		...
		// dropping it, so `holt --skip-permissions-unsafe "fix bug"` doesn't appear to
		// hang in the TUI with the prompt discarded. (AUDIT-L3)
		for _, arg := range rest {
			if strings.TrimSpace(arg) != "" {
				return writeAppError(stderr, "--skip-permissions-unsafe launches the interactive TUI and takes no prompt or subcommand; for a one-shot unsafe run use `holt exec --skip-permissions-unsafe -p \"...\"`", 1)
			}
		}
	...
	case "-v", "--version", "version":
		for _, a := range args[1:] {
			if a == "-h" || a == "--help" {
				if _, err := fmt.Fprintln(stdout, "Usage: holt version\n\nPrint the Holt CLI version. Takes no flags."); err != nil {
					return 1
				}
				return 0
			}
		}
		if _, err := fmt.Fprintf(stdout, "holt %s\n", version); err != nil {
			return 1
		}
		return 0
	case "-p", "--prompt":
		...
		// dash (e.g. `holt -p "-foo"`) is taken verbatim instead of being mistaken for
	...
	default:
		...
		// First-run users reach for `holt login`/`holt logout` (reported in the
		// wild); point them at the real command instead of a bare usage pointer.
		switch strings.ToLower(args[0]) {
		case "login", "logout":
			if _, err := fmt.Fprintf(stderr, "did you mean %q?\n", "holt auth "+strings.ToLower(args[0])); err != nil {
				return 1
			}
		}
		if _, err := fmt.Fprintln(stderr, "Run holt --help for usage."); err != nil {
			return 1
		}
		return 2
	}
...
func fillAppDeps(deps appDeps) appDeps {
	...
	deps.newProvider = func(profile config.ProviderProfile) (holtruntime.Provider, error) {
...
func runInteractiveTUIWithSetup(stderr io.Writer, deps appDeps, permissionMode agent.PermissionMode, addDirs []string, theme string, forceSetup bool) int {
	...
	if err != nil {
		...
		// no catalog default) — previously the second shape bricked bare `holt`
		// and `holt setup`, the exact commands that could have fixed it. Any
		// other error (malformed JSON, directory conflict, etc.) is still fatal,
		// and headless commands (holt config / holt exec) still fail with the
		// actionable message.
	...
	if needsSetup && !forceSetup {
		...
		// any time with `holt provider use <name>`. Onboarding only runs when no
	...
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "[holt] warning: failed to initialize MCP OAuth tokens: %s\n", err)
		mcpTokenStore = nil
		err = nil
	}
...
func buildProvider(resolved config.ResolvedConfig, deps appDeps) (holtruntime.Provider, error) {
	...
	// provider switch persisted by ANOTHER holt process mid-session would silently
...
func localArtifactsDirFromConfig(workspaceRoot string, cfg config.LocalControlConfig) string {
	...
	if dir == "" {
		dir = filepath.Join(".holt", "artifacts")
	}
...
func registerSpecialistTools(registry *tools.Registry, workspaceRoot string, maxTeamSize int) (*agentToolRuntime, error) {
	...
	sw, err := swarm.New(swarm.Options{
		BaseDir:     filepath.Join(workspaceRoot, ".holt", "swarm"),
...
func closeMCPRuntime(stderr io.Writer, runtime mcpToolRuntime) {
	...
	if err := runtime.Close(); err != nil {
		_, _ = fmt.Fprintf(stderr, "[holt] mcp_close_error: %s\n", err)
	}
}
...
func closeSpecialistRuntime(stderr io.Writer, runtime *agentToolRuntime) {
	...
	if runtime.specialist != nil {
		if err := runtime.specialist.Close(); err != nil {
			_, _ = fmt.Fprintf(stderr, "[holt] specialist_cleanup_error: %s\n", err)
		}
	}
}

func writeAppError(stderr io.Writer, message string, exitCode int) int {
	if _, err := fmt.Fprintf(stderr, "[holt] %s\n", message); err != nil {
		return 1
	}
	return exitCode
}
...
func writeHelp(w io.Writer) error {
	_, err := fmt.Fprint(w, `HOLT terminal coding agent
...
Commands:
  ...
  models     List Holt model registry entries
  ...
  search     Search persisted local Holt session events
  find       Alias for search
  sessions   Inspect local Holt session lineage
  spec       Review and approve saved spec-mode drafts
  specialist Manage local Holt specialist profiles
  plugins    Inspect, install, and remove local Holt plugins
  backends   Inspect MCP, hook, and plugin backend lifecycle state
  skills     Inspect, install, and remove local Holt skills
  tools      Scaffold and list local Holt plugin-tools
  hooks      Inspect Holt hook configuration
  ...
  update     Check for Holt CLI updates
  ...
  serve      Run Holt protocol servers
...
// argument list (holt --add-dir <path> [--add-dir <path>] [subcommand …]).
...
func writePromptRequired(stderr io.Writer) int {
	if _, err := fmt.Fprintln(stderr, "[holt] Prompt required. Use `holt exec \"prompt\"` or `holt exec --file prompt.txt`."); err != nil {
		return 1
	}
	return 2
}
