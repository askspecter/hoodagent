...
import (
	...
	"github.com/askspecter/holt/internal/background"
	"github.com/askspecter/holt/internal/daemon"
	"github.com/askspecter/holt/internal/daemon/remote"
)

// runDaemon dispatches the `holt daemon ...` subcommands. The daemon supervises a
// pool of headless `holt exec` workers and routes sessions to them over an
...
func writeDaemonUsage(w io.Writer, code int) int {
	fmt.Fprint(w, `Usage: holt daemon <command>
...
Commands:
  ...
  serve-remote --addr <host:port> --tls-cert <f> --tls-key <f> [--bundle-dir <d>]
                            ...
                            daemon. Requires a bearer token in $HOLT_DAEMON_REMOTE_TOKEN
                            (or $HOLT_DAEMON_REMOTE_TOKEN_FILE). --bundle-dir enables
...
// worker is a headless `holt exec` child with the sandbox re-entrancy markers
...
func runDaemonStartDetached(paths daemon.Paths, stdout io.Writer, stderr io.Writer) int {
	if daemonReachable(paths) {
		fmt.Fprintln(stdout, "holt daemon is already running")
		return exitSuccess
	}
	...
	for time.Now().Before(deadline) {
		if daemonReachable(paths) {
			fmt.Fprintf(stdout, "holt daemon started (socket %s)\n", paths.Socket)
			return exitSuccess
		}
...
func runDaemonStop(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if err != nil {
		fmt.Fprintln(stdout, "holt daemon is not running")
		return exitSuccess
	}
	defer client.Close()
	...
	fmt.Fprintln(stdout, "holt daemon stopped")
	return exitSuccess
}
...
func runDaemonRun(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			// Forwarded verbatim to the worker `holt exec` (reuses exec's own flag
...
func daemonDialError(flags remoteDialFlags, err error) string {
	...
	return "holt daemon is not running (start it with `holt daemon start`)"
}
...
func runDaemonServeRemote(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	fmt.Fprintf(stdout, "holt daemon remote bridge listening on %s (TLS)\n", addr)
	select {
	case <-sigCh:
		srv.Shutdown()
		_ = bridge.Close()
...
func runDaemonLink(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	fmt.Fprintf(stdout, "run it with: holt daemon run --remote %s --cwd %s ...\n", link.Address, link.RemotePath)
