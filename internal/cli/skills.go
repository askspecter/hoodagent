...
func runSkills(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(args) > 0 {
		...
		default:
			...
			// `list` command so `holt skills --json` works like `holt plugins`.
...
func formatSkillList(discovered []skills.Skill, dir string) string {
	if len(discovered) == 0 {
		return fmt.Sprintf("No Holt skills found in %s.", dir)
	}
	lines := []string{"Holt Skills:"}
...
Commands:
  list                 List discovered Holt skills
