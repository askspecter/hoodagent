...
func TestRunExecUsesInitSessionIDAndSessionTitle(t *testing.T) {
	...
	store := sessions.NewStore(sessions.StoreOptions{RootDir: filepath.Join(dataHome, "holt", "sessions")})
...
func (provider recordingExecProvider) StreamCompletion(context.Context, holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	*provider.called = true
	return nil, errors.New("provider should not run")
}

func TestRunPromptFlagRoutesToExecRunner(t *testing.T) {
	execExitCode, execStdout, execStderr := runExecWithEcho(t, []string{"exec", "hello holt"})
...
func TestRunExecUsesProjectConfigAndOpenAICompatibleProvider(t *testing.T) {
	clearProviderEnv(t)
	root := t.TempDir()
	configDir := filepath.Join(root, ".holt")
...
func (canceledExecProvider) StreamCompletion(context.Context, holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	return nil, context.Canceled
}

type echoExecProvider struct{}

func (echoExecProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	prompt := ""
	for index := len(request.Messages) - 1; index >= 0; index-- {
		if request.Messages[index].Role == holtruntime.MessageRoleUser {
	...
	case <-ctx.Done():
		close(ch)
		return ch, ctx.Err()
	case ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventText, Content: prompt}:
	}
...
func (provider *escalatingExecProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	turn := provider.turns
	provider.turns++
	ch := make(chan holtruntime.StreamEvent, 4)
	if provider.escalateOnce && turn == 0 {
		ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call_escalate", ToolName: "escalate_model"}
		ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call_escalate", ArgumentsFragment: "{}"}
		ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call_escalate"}
		ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventDone}
		close(ch)
		return ch, nil
	}
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventText, Content: "done"}
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventDone}
	close(ch)
	return ch, nil
}
...
func (provider *escalateThenAnswerProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	turn := provider.turns
	provider.turns++
	ch := make(chan holtruntime.StreamEvent, 6)
	if turn == 0 {
		ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call_escalate", ToolName: "escalate_model"}
		ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call_escalate", ArgumentsFragment: "{}"}
		ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call_escalate"}
		ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventUsage, Usage: holtruntime.Usage{InputTokens: 3, OutputTokens: 4}}
		ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventDone}
		close(ch)
		return ch, nil
	}
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventText, Content: "done"}
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventUsage, Usage: holtruntime.Usage{InputTokens: 5, OutputTokens: 7}}
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventDone}
	close(ch)
	return ch, nil
}
...
func (provider *usageEmittingEscalatingProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	ch := make(chan holtruntime.StreamEvent, 6)
...
func (usageEmittingEchoProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
...
func (provider *usageThenAnswerProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	turn := provider.turns
	provider.turns++
	ch := make(chan holtruntime.StreamEvent, 6)
	if turn == 0 {
