...
func runPlugins(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "plugins subcommand required. Use `holt plugins list`.")
	}
	switch args[0] {
	case "-h", "--help", "help":
...
func runHooks(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "hooks subcommand required. Use `holt hooks list`.")
	}
	switch args[0] {
	case "-h", "--help", "help":
...
func runMCPWithContext(ctx context.Context, args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(args) == 0 {
		return writeExecUsageError(stderr, "mcp subcommand required. Use `holt mcp permissions list`.")
	}
	switch args[0] {
	case "-h", "--help", "help":
...
func runMCPTools(ctx context.Context, args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(args) == 0 {
		return writeExecUsageError(stderr, "mcp tools subcommand required. Use `holt mcp tools list`.")
	}
...
func runMCPPermissions(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "mcp permissions subcommand required. Use `holt mcp permissions list`.")
	}
	if args[0] == "-h" || args[0] == "--help" || args[0] == "help" {
...
func runMCPPermissionsRevoke(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(positional) == 0 || len(positional) > 2 {
		return writeExecUsageError(stderr, "usage: holt mcp permissions revoke <server> [<tool>] [--json]")
	}
...
func writePluginsHelp(w io.Writer) error {
  ...
  holt plugins <command>

Commands:
  list                 List local Holt plugins
...
Commands:
  list      List configured Holt hooks
