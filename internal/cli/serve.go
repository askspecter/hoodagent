...
func runServe(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if !options.mcp {
		return writeExecUsageError(stderr, "serve requires --mcp. Use `holt serve --mcp`.")
	}
	...
	if options.allowUnsafeTools {
		if _, err := fmt.Fprintln(stderr, "[holt] Unsafe MCP server tools enabled because --allow-unsafe-tools was passed."); err != nil {
			return exitCrash
		}
	}
...
func writeServeHelp(w io.Writer) error {
  ...
  holt serve --mcp [flags]

Starts Holt as an MCP stdio server.
