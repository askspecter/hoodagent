...
type execOptions struct {
	...
	// mutating tool call HOLT runs the workspace verification plan and feeds
...
func runExec(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	// modelRegistry is the holt Registry when the catalog fails to build; the
	...
	if !config.HasProviderProfile(resolved.Provider) {
		return writeExecProviderError(stdout, stderr, options.outputFormat, "provider_error", "No provider configured. Run `holt setup` (guided), `holt auth` (OAuth providers), set a provider API key env var (e.g. OPENAI_API_KEY / ANTHROPIC_API_KEY / GEMINI_API_KEY), or add .holt/config.json.")
	}
	...
	// Opt-in webhook fan-out (HOLT_NOTIFY_WEBHOOK_URL). Headless runs can safely
...
// or with a holt/negative threshold, tool_search is never registered, so the
...
func resolveExecPrompt(options execOptions, workspaceRoot string, stdin io.Reader) (string, []holtruntime.ImageBlock, error) {
	...
	if prompt == "" {
		return "", nil, execUsageError{"Prompt required. Use `holt exec \"prompt\"` or `holt exec --file prompt.txt`."}
	}
...
func resolveExecImages(paths []string, workspaceRoot string) ([]holtruntime.ImageBlock, error) {
	if len(paths) == 0 {
		return nil, nil
	}
	images := make([]holtruntime.ImageBlock, 0, len(paths))
