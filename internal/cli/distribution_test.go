...
func TestRunPluginAddListRemove(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "holt.demo") {
	...
	stdout.Reset()
	stderr.Reset()
	if exit := runWithDeps([]string{"plugin", "remove", "holt.demo"}, &stdout, &stderr, deps); exit != 0 {
		t.Fatalf("plugin remove exit = %d, stderr = %s", exit, stderr.String())
	}
	if _, err := os.Stat(filepath.Join(pluginsDir, "holt.demo")); err == nil {
