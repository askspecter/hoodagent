...
func TestRunBackendsJSONUsesLifecycleSnapshotWithoutConnectingMCP(t *testing.T) {
	...
	deps := appDeps{
		...
		registerMCPTools: func(context.Context, *tools.Registry, config.MCPConfig, mcp.RegisterOptions) (mcpToolRuntime, error) {
			return nil, errors.New("holt backends must not connect to MCP servers")
		},
	}
	...
	var snapshot holtcommands.BackendLifecycleSnapshot
	...
	if len(snapshot.Hooks) != 1 || snapshot.Hooks[0].ID != "holt.preflight" || strings.Contains(strings.Join(snapshot.Hooks[0].Args, " "), secret) {
		t.Fatalf("unexpected hook snapshots: %#v", snapshot.Hooks)
	}
	if len(snapshot.Plugins) != 1 || snapshot.Plugins[0].ID != "holt.docs" || snapshot.Plugins[0].ToolCount != 1 || snapshot.Plugins[0].PromptCount != 1 || snapshot.Plugins[0].HookCount != 1 {
...
func TestRunBackendsTextAndHelp(t *testing.T) {
	...
	for _, want := range []string{"Holt Backends:", "MCP servers: 0", "Hooks: 0", "Plugins: 0"} {
	...
	for _, want := range []string{"Usage:", "holt backends", "--json"} {
		if !strings.Contains(stdout.String(), want) {
...
func TestRunBackendsDoctorJSONAndTextWithoutConnectingMCP(t *testing.T) {
	...
	deps := appDeps{
		...
		registerMCPTools: func(context.Context, *tools.Registry, config.MCPConfig, mcp.RegisterOptions) (mcpToolRuntime, error) {
			return nil, errors.New("holt backends doctor must not connect to MCP servers")
		},
	}
	...
	var payload holtcommands.BackendDoctorReport
	...
	if payload.Status != holtcommands.BackendDoctorStatusFail {
		t.Fatalf("payload.Status = %q, want %q", payload.Status, holtcommands.BackendDoctorStatusFail)
	}
	assertBackendDoctorPayloadCheck(t, payload, "backend.mcp.invalid", "broken")
	assertBackendDoctorPayloadCheck(t, payload, "backend.hooks.diagnostic", "holt.bad")
	...
	for _, want := range []string{"Holt backend doctor", "[fail] backend.mcp.invalid", "holt mcp add broken", "[fail] backend.hooks.diagnostic"} {
		if !strings.Contains(stdout.String(), want) {
...
func TestRunBackendsDoctorHelp(t *testing.T) {
	...
	for _, want := range []string{"Usage:", "holt backends doctor", "--json"} {
		if !strings.Contains(stdout.String(), want) {
...
func TestRunBackendsDoctorDoesNotConnectOrExecuteConfiguredBackends(t *testing.T) {
	...
	helperArgs := func(path string) []string {
		return []string{"-test.run=TestBackendDoctorHelperProcess", "--", "--holt-backend-doctor-sentinel", path}
	}
	...
	hookConfigPath := filepath.Join(cwd, ".holt", "hooks.json")
	...
	pluginDir := filepath.Join(cwd, ".holt", "plugins", "sentinel")
	...
	deps := appDeps{
		...
		loadPlugins: func(options plugins.LoadOptions) (plugins.LoadResult, error) {
			return plugins.Load(plugins.LoadOptions{
				Roots: []plugins.Root{{Source: plugins.SourceProject, Path: filepath.Join(cwd, ".holt", "plugins")}},
				Cwd:   options.Cwd,
			})
		},
		registerMCPTools: func(context.Context, *tools.Registry, config.MCPConfig, mcp.RegisterOptions) (mcpToolRuntime, error) {
			return nil, errors.New("holt backends doctor must not register MCP tools")
		},
	}
...
func assertBackendDoctorPayloadCheck(t *testing.T, report holtcommands.BackendDoctorReport, id string, target string) {
	t.Helper()
...
func TestBackendDoctorHelperProcess(t *testing.T) {
	for index, arg := range os.Args {
		if arg != "--holt-backend-doctor-sentinel" || index+1 >= len(os.Args) {
			continue
		}
