...
// Project scope (the default) targets <cwd>/.holt/hooks.json; --user targets the
...
func runHooksRemove(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(positional) != 1 {
		return writeExecUsageError(stderr, "usage: holt hooks remove <id> [--user] [--json]")
	}
	hookID := positional[0]
	store, path, err := hookConfigStore(deps, options.user)
	if err != nil {
...
func runHooksToggle(args []string, stdout io.Writer, stderr io.Writer, deps appDeps, disabled bool) int {
	...
	if len(positional) != 1 {
		return writeExecUsageError(stderr, fmt.Sprintf("usage: holt hooks %s <id> [--user] [--json]", commandName))
	}
	hookID := positional[0]
	store, path, err := hookConfigStore(deps, options.user)
	if err != nil {
...
func parseHooksAddArgs(args []string) (hookAddOptions, bool, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case options.def.ID == "":
			options.def.ID = arg
		default:
			return options, false, execUsageError{"usage: holt hooks add <id> --event <event> --command <cmd> [flags]"}
		}
	}
	...
	if strings.TrimSpace(string(options.def.Event)) == "" {
		return options, false, execUsageError{"holt hooks add requires --event"}
	}
	...
	if strings.TrimSpace(options.def.Command) == "" {
		return options, false, execUsageError{"holt hooks add requires --command"}
	}
...
func writeHooksAddHelp(w io.Writer) error {
  ...
  holt hooks add <id> --event <event> --command <cmd> [flags]
...
New hooks are enabled; use "holt hooks disable <id>" to turn one off.
`)
	return err
}
