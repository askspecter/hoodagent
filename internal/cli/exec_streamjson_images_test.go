...
func (provider *capturingImageProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	provider.mu.Lock()
	for index := len(request.Messages) - 1; index >= 0; index-- {
		if request.Messages[index].Role == holtruntime.MessageRoleUser {
			provider.images = append([]holtruntime.ImageBlock(nil), request.Messages[index].Images...)
			break
		}
	}
	provider.mu.Unlock()
	...
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventText, Content: "ok"}
