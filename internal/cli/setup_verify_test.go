...
func TestFormatSetupCompleteIncludesTryThisExample(t *testing.T) {
	out := formatSetupComplete(tui.SetupResult{
		ConfigPath: "/home/u/.config/holt/config.json",
	...
	if !strings.Contains(out, "Holt setup complete") {
	...
	if !strings.Contains(out, `holt exec "`) || !strings.Contains(out, "--model gpt-4.1") {
