...
func runMCPOAuth(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "mcp oauth subcommand required. Use `holt mcp oauth status`.")
	}
...
func runMCPOAuthLogin(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if opts.json {
		...
		return writeExecUsageError(stderr, "holt mcp oauth login does not support --json")
	}
	if len(positional) != 1 {
		return writeExecUsageError(stderr, "usage: holt mcp oauth login <server>")
	}
...
func runMCPOAuthLogout(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(positional) != 1 {
		return writeExecUsageError(stderr, "usage: holt mcp oauth logout <server>")
	}
	serverName := positional[0]
...
func runMCPOAuthStatus(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(positional) > 1 {
		return writeExecUsageError(stderr, "usage: holt mcp oauth status [server]")
	}
