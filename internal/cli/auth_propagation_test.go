...
func seedStoredProviderKey(t *testing.T, providerName, key string) (userConfigPath string) {
	t.Helper()
	t.Setenv("HOLT_CRED_STORAGE", "encrypted-file") // never touch the real OS keychain in tests
	dir := t.TempDir()
...
// HOLT_PROVIDER so children spawned at any point in the run are pinned to the
// parent's provider from launch — not only after an in-session switch. Without
// this, a provider switch persisted by another holt process mid-session moves
