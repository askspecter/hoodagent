...
// runAuth dispatches `holt auth <command>` for provider OAuth login. It is
// additive and independent of `holt mcp oauth` (MCP server auth), which is
// unchanged.
func runAuth(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	if len(args) == 0 {
		return writeExecUsageError(stderr, "auth subcommand required. Use `holt auth status`.")
	}
...
func runAuthOpenRouter(args []string, stdout io.Writer, stderr io.Writer, _ appDeps) int {
	...
	if len(args) > 0 {
		return writeExecUsageError(stderr, fmt.Sprintf("holt auth openrouter takes no arguments (got %q)", args[0]))
	}
	key, err := provideroauth.OpenRouterLogin(context.Background(), provideroauth.OpenRouterOptions{
		Out:        stdout,
		HTTPClient: &http.Client{Timeout: 30 * time.Second},
		// HOLT_OPENROUTER_BASE_URL overrides the endpoint (self-hosted gateway or tests).
		BaseURL: strings.TrimSpace(os.Getenv("HOLT_OPENROUTER_BASE_URL")),
	})
	if err != nil {
		return writeAppError(stderr, redaction.ErrorMessage(err, redaction.Options{}), exitCrash)
	}
	if _, err := fmt.Fprintf(stdout, "\nOpenRouter login complete — new API key minted.\nUse it with holt, e.g.:\n  export OPENROUTER_API_KEY=%s\n(or add it to a provider profile with catalogID \"openrouter\").\n", key); err != nil {
		return exitCrash
	}
	return exitSuccess
}
...
func runAuthChatGPT(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(args) > 0 {
		return writeExecUsageError(stderr, fmt.Sprintf("holt auth chatgpt takes no arguments (got %q)", args[0]))
	}
	...
	// layered: process env first, then HOLT_OAUTH_ALLOW_PRESETS=1.
	...
	// holt auth status / holt auth refresh / TokenResolver use is hit.
	...
	if _, err := fmt.Fprint(stdout, "\nUse it with holt, e.g.:\n  holt --provider chatgpt --model gpt-5.5\n"); err != nil {
		return exitCrash
	}
	return exitSuccess
}
...
// shape `holt auth status` prints, so the user sees a consistent view.
...
func validateAuthFlags(sub string, a authArgs) error {
	...
	bad := func(name string) error { return fmt.Errorf("holt auth %s does not accept %s", sub, name) }
...
// HOLT_OAUTH_TOKENS_PATH (env), so callers/tests can redirect it. Setting
// HOLT_OAUTH_STORAGE=encrypted-file selects the AES-256-GCM encrypted-at-rest
...
func newAuthManager(deps appDeps, out io.Writer) (*oauth.Manager, error) {
	// Validate HOLT_OAUTH_STORAGE up front: a mistyped value must fail fast rather
	// than silently change the backend. Empty = default (plaintext 0600 file);
	// "encrypted-file" = AES-256-GCM; "keyring" = the OS keyring.
	storage := strings.ToLower(strings.TrimSpace(os.Getenv("HOLT_OAUTH_STORAGE")))
	switch storage {
	case "", "file", "encrypted-file", "keyring":
	default:
		return nil, fmt.Errorf("invalid HOLT_OAUTH_STORAGE %q (supported: file, encrypted-file, keyring)", storage)
	}
	store, err := oauth.NewStore(oauth.StoreOptions{
		Now:     deps.now,
		Storage: storage,
	...
	return oauth.NewManager(oauth.ManagerOptions{
		...
		OpenBrowser: func(string) error { return nil },
		// `holt auth login <preset>` (e.g. xai) should resolve the baked-in preset
		// without the operator exporting HOLT_OAUTH_ALLOW_PRESETS first.
		AllowPresets: true,
	})
}
...
func runAuthLogin(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(parsed.positional) != 1 {
		return writeExecUsageError(stderr, "usage: holt auth login <provider> [--device] [--scope <scope>]")
	}
...
func runAuthLogout(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(parsed.positional) != 1 {
		return writeExecUsageError(stderr, "usage: holt auth logout <provider>")
	}
	provider := parsed.positional[0]
	manager, err := newAuthManager(deps, stdout)
	if err != nil {
...
func runAuthStatus(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(parsed.positional) > 1 {
		return writeExecUsageError(stderr, "usage: holt auth status [provider]")
	}
	manager, err := newAuthManager(deps, stdout)
	if err != nil {
		return writeAppError(stderr, redaction.ErrorMessage(err, redaction.Options{}), exitCrash)
...
func runAuthRefresh(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if len(parsed.positional) != 1 {
		return writeExecUsageError(stderr, "usage: holt auth refresh <provider>")
	}
	provider := parsed.positional[0]
	manager, err := newAuthManager(deps, stdout)
	if err != nil {
...
A provider is any OAuth 2.0 / OIDC server. "openrouter" ('holt auth openrouter')
works out of the box. "xai" ('holt auth login xai') uses a built-in preset that is
off by default — enable it with HOLT_OAUTH_ALLOW_PRESETS=1, or set the
HOLT_OAUTH_XAI_* vars yourself. Any preset field is overridable via the env vars
below. For a custom provider named <name>, set:
  HOLT_OAUTH_<NAME>_CLIENT_ID       (required)
  HOLT_OAUTH_<NAME>_CLIENT_SECRET   (optional)
  HOLT_OAUTH_<NAME>_AUTHORIZE_URL   HOLT_OAUTH_<NAME>_TOKEN_URL
  HOLT_OAUTH_<NAME>_DEVICE_URL      HOLT_OAUTH_<NAME>_ISSUER_URL (for discovery)
  HOLT_OAUTH_<NAME>_SCOPES          HOLT_OAUTH_<NAME>_FLOW (loopback|device)
Endpoint URLs must be https (loopback exempt).

Storage: tokens are written 0600 under $XDG_CONFIG_HOME/holt (override with
HOLT_OAUTH_TOKENS_PATH). Set HOLT_OAUTH_STORAGE=encrypted-file to encrypt them at
rest with AES-256-GCM (a per-user secret beside the file), or
HOLT_OAUTH_STORAGE=keyring to use the OS keyring (macOS Keychain / Linux
