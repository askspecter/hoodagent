...
func TestRunExecStreamJSONOutputsRunEndAndRecordsSession(t *testing.T) {
	...
	store := sessions.NewStore(sessions.StoreOptions{
		RootDir: filepath.Join(dataHome, "holt", "sessions"),
	})
...
func TestRunExecJSONInterruptedEmitsTerminalEvents(t *testing.T) {
	...
	exitCode := runWithDeps([]string{"exec", "--output-format", "json", "hello"}, &stdout, &stderr, appDeps{
		...
		newProvider:   func(config.ProviderProfile) (holtruntime.Provider, error) { return canceledExecProvider{}, nil },
	})
...
func (provider toolCallingExecProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	for _, message := range request.Messages {
		if message.Role == holtruntime.MessageRoleTool {
			ch := make(chan holtruntime.StreamEvent, 2)
			ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventText, Content: provider.answer}
	...
	case <-ctx.Done():
		close(ch)
		return ch, ctx.Err()
	case ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallStart, ToolCallID: provider.toolCallID, ToolName: provider.toolName}:
	}
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: provider.toolCallID, ArgumentsFragment: provider.arguments}
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: provider.toolCallID}
	ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventDone}
	close(ch)
	return ch, nil
}
