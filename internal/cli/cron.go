...
import (
	...
	"github.com/askspecter/holt/internal/cron"
)

// runCron is the dispatch entry for `holt cron`.
...
func runCronWith(store *cron.Store, now func() time.Time, args []string, stdout io.Writer, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "Usage: holt cron <add|list|rm|pause|resume|run> ...")
		return exitUsage
	}
...
func cronAdd(store *cron.Store, now func() time.Time, args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if expr == "" {
		fmt.Fprintln(stderr, "A cron expression is required (e.g. `holt cron add \"0 9 * * *\" --prompt ...`).")
		return exitUsage
	}
...
func cronResume(store *cron.Store, now func() time.Time, args []string, stdout io.Writer, stderr io.Writer) int {
	...
	// impossible spec) — it would leave a stale/holt NextRunAt for the runner.
...
func writeCronHelp(w io.Writer) {
	fmt.Fprint(w, `holt cron — schedule agent jobs (foreground, file-backed)

Usage:
  holt cron add <cron-expr> [--prompt P | --recipe R] [--cwd D] [--model M] [--run-now]
  holt cron list
  holt cron pause <id> | resume <id> | rm <id>
  holt cron run [--once] [--catch-up] [id...]
