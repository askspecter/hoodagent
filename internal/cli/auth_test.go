...
func withAuthStore(t *testing.T) string {
	...
	t.Setenv("HOLT_OAUTH_TOKENS_PATH", path)
	return path
}
...
func TestRunAuthRejectsInvalidStorageMode(t *testing.T) {
	...
	t.Setenv("HOLT_OAUTH_STORAGE", "encryptd")
	var stdout, stderr bytes.Buffer
	if code := runWithDeps([]string{"auth", "status"}, &stdout, &stderr, appDeps{}); code == exitSuccess {
		t.Fatalf("invalid HOLT_OAUTH_STORAGE should fail, got success; stdout=%q", stdout.String())
	}
	if !strings.Contains(stderr.String(), "HOLT_OAUTH_STORAGE") {
...
func TestRunAuthRefreshNoToken(t *testing.T) {
	withAuthStore(t)
	t.Setenv("HOLT_OAUTH_DEMO_CLIENT_ID", "client") // so config resolves; refresh still fails (no token)
	var stdout, stderr bytes.Buffer
...
func TestRunAuthHelp(t *testing.T) {
	...
	for _, want := range []string{"holt auth", "login", "logout", "status", "refresh", "--device"} {
