...
func TestRunRepoMapHelpDocumentsFlags(t *testing.T) {
	...
	for _, want := range []string{"Usage:", "holt repo-map [flags]", "--json", "--query", "--max-files", "--scan-max-files", "--max-depth"} {
...
func TestRunRepoMapTextSummaryUsesCurrentWorkingDirectory(t *testing.T) {
	...
	for _, want := range []string{"Repo map", "Root: " + cwd, "Files:", "Languages:", "cmd/holt/main.go"} {
...
func TestRunRepoMapQueryPrintsRankedMatches(t *testing.T) {
	...
	if repoMapOutputContains(out, "cmd/holt/main.go") {
...
func newRepoMapFixture(t *testing.T) string {
	...
	writeRepoMapFixtureFile(t, dir, "cmd/holt/main.go", "package main\n\nfunc main() {}\n")
