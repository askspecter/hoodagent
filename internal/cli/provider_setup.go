...
func runProvidersAdd(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	if options.json {
		if err := writePrettyJSON(stdout, map[string]any{
			...
			"provider":       holtcommands.ProviderSnapshotFromProfile(profile, cfg.ActiveProvider == profile.Name),
		}); err != nil {
			return exitCrash
		}
		return exitSuccess
...
func runProvidersCheck(args []string, stdout io.Writer, stderr io.Writer, deps appDeps) int {
	...
	snapshot := holtcommands.ProviderSnapshotFromProfile(profile, profile.Name == resolved.ActiveProvider)
	if options.json {
...
func providerCheckNextActions(profile config.ProviderProfile, connectivity bool, health providerhealth.Result) []string {
	...
	if !connectivity {
		return []string{fmt.Sprintf("run holt providers check %s --connectivity", name)}
	}
	switch health.Status {
	case providerhealth.StatusFail:
		return []string{fmt.Sprintf("verify the API key, base URL, and model, then rerun holt providers check %s --connectivity", name)}
	case providerhealth.StatusWarn:
		return []string{fmt.Sprintf("review the warning, then rerun holt providers check %s --connectivity", name)}
	default:
		model := strings.TrimSpace(profile.Model)
		if model == "" {
			return []string{"provider is ready"}
		}
		return []string{fmt.Sprintf("run holt exec %q --model %s", "hello", model)}
	}
}
...
func providerCheckMissingKeyNextAction(profile config.ProviderProfile) string {
	...
	return fmt.Sprintf("set %s and rerun holt providers check %s", apiKeyEnv, providerCheckName(profile))
}
