...
func TestRunProvidersCatalogJSONIncludesDescriptors(t *testing.T) {
	...
	var payload struct {
		Providers []holtcommands.ProviderCatalogSnapshot `json:"providers"`
	}
...
func TestRunProvidersAddWritesCatalogProfile(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	configPath := filepath.Join(t.TempDir(), "holt", "config.json")
...
func TestRunProvidersAddWritesCustomHeaders(t *testing.T) {
	...
	exitCode := runWithDeps([]string{
		...
		"--auth-scheme", "Token",
		"--header", "HTTP-Referer=https://holt.dev",
		"--header", "X-Title=Holt",
	...
	if profile.CustomHeaders["HTTP-Referer"] != "https://holt.dev" || profile.CustomHeaders["X-Title"] != "Holt" {
...
func TestRunProvidersCheckConnectivityJSON(t *testing.T) {
	...
	deps.newProvider = func(config.ProviderProfile) (holtruntime.Provider, error) {
...
func TestRunProvidersPositionalHelp(t *testing.T) {
	...
	output := stdout.String()
	for _, want := range []string{"Usage:", "holt providers", "list", "current", "catalog"} {
		if !strings.Contains(output, want) {
...
func findProviderCatalogSnapshot(t *testing.T, snapshots []holtcommands.ProviderCatalogSnapshot, id string) holtcommands.ProviderCatalogSnapshot {
	t.Helper()
	...
	return holtcommands.ProviderCatalogSnapshot{}
}

type commandCenterProvider struct{}

func (commandCenterProvider) StreamCompletion(context.Context, holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
