...
func verifySetupProvider(deps appDeps, profile config.ProviderProfile) (setupVerification, error) {
	...
	if !profileHasCredential(profile) && !baseURLIsLoopback(profile.BaseURL) {
		...
		return setupVerification{Ran: true, OK: false, Summary: "no api key"},
			fmt.Errorf("no API key found for %s — set its API key (export the provider's API key env var, or pass --api-key-env) or run `holt auth`, then re-run setup", name)
	}
...
func setupRequired(resolved config.ResolvedConfig) bool {
	...
	// A stored OAuth login (e.g. `holt auth login xai`) is a credential too, even
...
func formatSetupComplete(result tui.SetupResult) string {
	lines := []string{"Holt setup complete"}
	...
	lines = append(lines, "next: "+setupCheckCommand(result.Provider.Name), "next: holt")
...
func setupTryThisExample(profile config.ProviderProfile) string {
	example := `holt exec "say hello in one short sentence"`
...
func setupCheckCommand(name string) string {
	...
	if name == "" {
		return "holt providers check --connectivity"
	}
	return "holt providers check " + setupCommandArg(name) + " --connectivity"
}
...
func writeSetupHelp(w io.Writer) error {
  ...
  holt setup [provider] [flags]

Guides first-run Holt setup. Without a provider, opens the setup UI. With a
provider catalog id, writes that provider as the active provider.

Examples:
  holt setup
  holt setup openai --api-key-env OPENAI_API_KEY
  holt setup ollama
  holt setup ollama-cloud
