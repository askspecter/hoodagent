...
func TestWebhookSinkPostsExpectedJSON(t *testing.T) {
	...
	sink.Emit(Completion, "Holt: ready")
	...
	case r := <-got:
		...
		if r.body.Message != "Holt: ready" {
			t.Fatalf("payload message = %q, want %q", r.body.Message, "Holt: ready")
		}
...
func TestWebhookSinkIncludesSummaryAndLinks(t *testing.T) {
	...
	sink.Emit(AwaitingInput, "Holt: needs input")
...
func TestWebhookSinkRedactsSecretsInPayloadAndLog(t *testing.T) {
	token := "xoxb" + "-1234567890-abcdefghijklmno" //nolint:goconst
...
func TestWebhookSinkRedactsWebhookURLInLog(t *testing.T) {
	...
	logf, getLog := captureLog()
	secretSuffix := "xoxb" + "-1234567890-secrettoken"
	secretURL := "http://127.0.0.1:0/services/T000/B000/" + secretSuffix
	sink := NewWebhookSink(WebhookConfig{
		URL:    secretURL,
		Logf:   logf,
		Client: &http.Client{Timeout: 200 * time.Millisecond},
	...
	sink.Emit(Completion, "hi")
	if log := getLog(); strings.Contains(log, secretSuffix) {
