...
func TestNotifyWritesAndRespectsPolicy(t *testing.T) {
	...
	n.Notify(Completion, "Holt: ready")
	if buf.String() != "\x07\x1b]9;Holt: ready\x07" {
...
func TestDefaultMessage(t *testing.T) {
	if DefaultMessage(Completion) != "Holt: ready" {
		t.Fatal("completion message")
	}
	if DefaultMessage(AwaitingInput) != "Holt: needs input" {
