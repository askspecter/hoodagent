...
func TestDefaultResolveOptionsUsesExistingConfigPathsAndProviderCommand(t *testing.T) {
	...
	userPath := filepath.Join(userConfigRoot, "holt", "config.json")
	projectPath := filepath.Join(workspaceRoot, ".holt", "config.json")
	writeFileAt(t, userPath, "{}")
	writeFileAt(t, projectPath, "{}")
	t.Setenv("HOLT_PROVIDER_COMMAND", "  holt-provider  ")
	...
	if options.ProviderCommand != "holt-provider" {
		t.Fatalf("ProviderCommand = %q, want holt-provider", options.ProviderCommand)
	}
}
...
func TestDefaultResolveOptionsRejectsDirectoryConfigPaths(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			makeDirectory: func(t *testing.T, userConfigRoot string, workspaceRoot string) string {
				t.Helper()
				path := filepath.Join(userConfigRoot, "holt", "config.json")
				mkdirAll(t, path)
				return path
			},
		},
		{
			...
			makeDirectory: func(t *testing.T, userConfigRoot string, workspaceRoot string) string {
				t.Helper()
				path := filepath.Join(workspaceRoot, ".holt", "config.json")
				mkdirAll(t, path)
				return path
			},
		},
...
func TestDefaultUserConfigPathUsesXDGConfigOnMacOS(t *testing.T) {
	...
	want := filepath.Join(home, ".config", "holt", "config.json")
	if path != want {
