...
func TestResolveLoadsProviderCatalogSnakeAndCamelJSONFields(t *testing.T) {
	path := writeConfig(t, `{
		...
		"providers": [{
			...
			"api_key_env": "HOLT_SNAKE_API_KEY",
			"api_format": "responses",
			"auth_header": "X-API-Key",
			"auth_scheme": "Token",
			"auth_header_value": "env:HOLT_SNAKE_HEADER"
		}, {
			"name": "camel",
			"provider": "anthropic",
			"model": "camel-model",
			"catalogID": "anthropic",
			"apiKeyEnv": "HOLT_CAMEL_API_KEY",
			"apiFormat": "messages",
			"authHeader": "Authorization",
			"authScheme": "Bearer",
			"authHeaderValue": "env:HOLT_CAMEL_HEADER"
		}]
	}`)
	...
	if resolved.Provider.APIKeyEnv != "HOLT_SNAKE_API_KEY" {
	...
	if resolved.Provider.AuthHeaderValue != "env:HOLT_SNAKE_HEADER" {
	...
	if camel.APIKeyEnv != "HOLT_CAMEL_API_KEY" {
	...
	if camel.AuthHeaderValue != "env:HOLT_CAMEL_HEADER" {
...
func TestResolveMergesProviderCatalogFieldsByLayerPrecedence(t *testing.T) {
	userPath := writeConfig(t, `{
		...
		"providers": [{
			...
			"model": "user-model",
			"catalog_id": "openai",
			"api_key_env": "HOLT_USER_API_KEY",
			"api_format": "user-format",
			"auth_header": "X-User-Key",
			"auth_scheme": "UserScheme",
			"auth_header_value": "env:HOLT_USER_HEADER"
		}]
	}`)
	projectPath := writeConfig(t, `{
		"providers": [{
			...
			"authHeaderValue": "env:HOLT_PROJECT_HEADER"
		}]
	}`)
	...
	if resolved.Provider.APIKeyEnv != "HOLT_USER_API_KEY" {
	...
	if resolved.Provider.AuthHeaderValue != "env:HOLT_PROJECT_HEADER" {
...
func TestResolveAPIKeyEnvLooksUpEnvOnlyWhenAPIKeyMissing(t *testing.T) {
	path := writeConfig(t, `{
		...
		"providers": [{
			"name": "from-env",
			"provider": "openai",
			"apiKeyEnv": "HOLT_FROM_ENV_API_KEY",
			"model": "gpt-from-env"
		}, {
			"name": "direct",
			"provider": "openai",
			"apiKey": "sk-direct",
			"apiKeyEnv": "HOLT_DIRECT_API_KEY",
			"model": "gpt-direct"
		}]
	}`)
	...
	resolved, err := Resolve(ResolveOptions{
		...
		Env: map[string]string{
			"HOLT_FROM_ENV_API_KEY": "sk-from-env",
			"HOLT_DIRECT_API_KEY":   "sk-should-not-win",
		},
	})
	if err != nil {
		t.Fatalf("Resolve() error = %v", err)
	...
	if resolved.Provider.APIKeyEnv != "HOLT_FROM_ENV_API_KEY" {
...
func TestResolveAPIKeyEnvRedactsResolvedSecretOnErrors(t *testing.T) {
	...
	_, err := Resolve(ResolveOptions{
		ProjectConfigPath: path,
		Env: map[string]string{
			"HOLT_CUSTOM_API_KEY": "sk-env-secret-value",
		},
	})
	if err == nil {
...
func TestResolveReplacesMCPServerOverlayCollections(t *testing.T) {
		...
		"mcp": {
			"servers": {
				"docs": {
					...
					"command": "docs-mcp",
					"args": ["--user"],
					"env": {"HOLT_DOCS_TOKEN": "user-token"}
				}
			}
		}
	}`)
	...
	if _, ok := docs.Env["HOLT_DOCS_TOKEN"]; ok || docs.Env["HOLT_DOCS_PROJECT"] != "1" {
		t.Fatalf("docs.Env = %#v, want HOLT_DOCS_TOKEN absent and HOLT_DOCS_PROJECT=1", docs.Env)
	}
...
func TestResolveRejectsActiveProviderWithoutConfiguredProfiles(t *testing.T) {
	...
	if resolved.MaxTurns != 0 {
		t.Fatalf("MaxTurns = %d, want holt on failed resolve", resolved.MaxTurns)
	}
}
...
func TestResolveProviderProfileExtendedJSONAliases(t *testing.T) {
	...
	if profile.CustomHeaders["X-Holt"] != "1" {
		t.Fatalf("CustomHeaders = %#v, want X-Holt header", profile.CustomHeaders)
	}
...
func writeConfig(t *testing.T, body string) string {
	...
	path := filepath.Join(t.TempDir(), "holt.json")
...
// including holt config and bare holt setup — the only commands that could
