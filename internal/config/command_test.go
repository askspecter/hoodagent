...
func TestLoadProviderCommandDoesNotResolveAPIKeyEnvFromProcess(t *testing.T) {
	t.Setenv("HOLT_CMD_API_KEY", "sk-process")
	command := writeCommand(t, commandScript{
		Stdout: `{"name":"cmd","provider":"openai","apiKeyEnv":"HOLT_CMD_API_KEY","model":"gpt-command"}`,
	})
	...
	if provider.APIKeyEnv != "HOLT_CMD_API_KEY" {
