...
// DefaultMCPServers returns the MCP servers Holt ships ENABLED by default so web
...
// `holt mcp disable <name>` (which writes `"disabled": true`).
//
...
// use. Holt only calls it over the network, so Firecrawl's license never reaches
// into Holt's own code.
...
// IsDefaultMCPServer reports whether name is one of Holt's built-in default MCP
