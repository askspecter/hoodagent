...
func DefaultResolveOptions(workspaceRoot string) (ResolveOptions, error) {
	...
	projectConfigPath, err := existingConfigFile(filepath.Join(workspaceRoot, ".holt", "config.json"))
	if err != nil {
		return ResolveOptions{}, err
	}
	...
	return ResolveOptions{
		...
		ProviderCommand:   strings.TrimSpace(os.Getenv("HOLT_PROVIDER_COMMAND")),
	}, nil
}
...
func DefaultUserConfigPath() (string, error) {
	...
	return filepath.Join(userConfigDir, "holt", "config.json"), nil
}

// UserConfigDir returns the base directory Holt stores per-user config under.
// It mirrors os.UserConfigDir everywhere except macOS: there Go defaults to
// ~/Library/Application Support, but Holt deliberately uses ~/.config (XDG-style,
