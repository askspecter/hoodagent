...
// openai-/anthropic-compatible endpoints — Holt cannot guess a gateway's model).
...
// (holt config, holt exec) still fail with the actionable message.
...
// Holt collapses their full JSON schemas into compact `tool_search` reminder
...
func ResolveMCP(options ResolveOptions) (MCPConfig, error) {
	// Seed Holt's built-in default MCP servers (e.g. keyless Firecrawl for free,
...
func mergeProjectConfig(dst *FileConfig, src FileConfig) error {
	...
	// config: a cloned repo's .holt/config.json must not be able to grant
...
// every command that resolves config up front (holt config, bare holt setup).
...
const ActiveProviderEnv = "HOLT_PROVIDER"
...
const MaxTurnsEnv = "HOLT_MAX_TURNS"
...
func normalizeProvidersWithOptions(providers []ProviderProfile, activeName string, options normalizeOptions, envMaps ...map[string]string) ([]ProviderProfile, ProviderProfile, error) {
	...
	if active.Model == "" {
		return nil, ProviderProfile{}, &setupFixableError{
			err:      providerError(active, "provider %s requires model — add \"model\" to its entry in config.json, or re-run: holt setup <catalog-id> --model <model>", active.Name),
