...
import (
	...
	"github.com/askspecter/holt/internal/credstore"
)
...
// opt-out via HOLT_CRED_STORAGE. The dir parameter exists so tests can point the file
...
// catalogID:"chatgpt"} — still finds a `holt auth login chatgpt` token).
