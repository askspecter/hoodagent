...
func ResolveImages(events []InputEvent) ([]holtruntime.ImageBlock, error) {
	...
	for _, event := range events {
		for _, image := range event.Images {
			...
			mediaType := holtruntime.NormalizeImageMediaType(image.MediaType)
			if mediaType == "" {
				return nil, ProtocolError{fmt.Sprintf("Stream-json image has an unsupported image media type %q.", image.MediaType)}
			}
			images = append(images, holtruntime.ImageBlock{MediaType: mediaType, Data: data})
		}
	}
	return images, nil
}
