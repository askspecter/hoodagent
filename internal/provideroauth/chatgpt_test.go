...
// preset, so the resolver can build a Config without HOLT_OAUTH_CHATGPT_* vars.
func chatgptTestEnv() map[string]string {
	return map[string]string{"HOLT_OAUTH_ALLOW_PRESETS": "1"}
}
...
func TestChatGPTLoginUsesPreset(t *testing.T) {
	...
	// overriding HOLT_OAUTH_CHATGPT_CLIENT_ID.
	env := chatgptTestEnv()
	env["HOLT_OAUTH_CHATGPT_AUTHORIZE_URL"] = ts.AuthorizeURL()
	env["HOLT_OAUTH_CHATGPT_TOKEN_URL"] = ts.TokenURL()
...
func TestChatGPTLoginMissingAuthorizationEndpointErrors(t *testing.T) {
	...
	env := chatgptTestEnv()
	env["HOLT_OAUTH_CHATGPT_AUTHORIZE_URL"] = ""
	env["HOLT_OAUTH_CHATGPT_TOKEN_URL"] = "https://chatgpt.invalid/oauth/token"
