...
func TestActivateRegistersToolThatInvokesCommand(t *testing.T) {
	...
	if result.Tools[0].ToolName != "lookup" || result.Tools[0].PluginID != "holt.demo" {
...
func TestActivateSkipsMalformedPluginAndContinues(t *testing.T) {
	...
	if !containsSubstring(result.Warnings, "bad") && !containsSubstring(result.Warnings, "holt.bad") {
...
func TestActivateIsDeterministic(t *testing.T) {
	...
	plugins := []LoadedPlugin{
		{ID: "holt.b", Name: "B", Enabled: true, PluginDir: dirB, Hooks: []HookExtension{{Name: "hb", Event: HookSessionStart, Command: "b.sh"}}},
		{ID: "holt.a", Name: "A", Enabled: true, PluginDir: dirA, Hooks: []HookExtension{{Name: "ha", Event: HookSessionStart, Command: "a.sh"}}},
	}
	...
	// Deterministic order is by plugin ID then extension name: holt.a before holt.b.
	if first.Hooks[0].ID != "holt.a.ha" {
		t.Fatalf("first hook id = %q, want holt.a.ha (deterministic by plugin then name)", first.Hooks[0].ID)
	}
}
