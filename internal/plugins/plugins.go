...
func ResolveRoots(options ResolveRootOptions) ([]Root, error) {
	...
	return []Root{
		{Source: SourceUser, Path: filepath.Join(configHome, "holt", "plugins")},
		{Source: SourceProject, Path: filepath.Join(cwd, ".holt", "plugins")},
	}, nil
}
...
func ParseManifest(raw any, options ParseManifestOptions) (LoadedPlugin, error) {
	...
	// DISCOVERY (the `holt plugins` listing + backend snapshots) and ACTIVATION
...
func FormatList(plugins []LoadedPlugin, diagnostics []Diagnostic) string {
	lines := []string{}
	if len(plugins) == 0 {
		lines = append(lines, "No local Holt plugins loaded.")
	} else {
		lines = append(lines, "Holt Plugins:")
