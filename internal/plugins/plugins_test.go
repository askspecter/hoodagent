...
func TestParseManifestNormalizesExtensionsAndPaths(t *testing.T) {
	...
	pluginDir := filepath.Join(root, "holt-demo")
	...
	if plugin.ID != "holt.demo" || plugin.Name != "Holt Demo" || !plugin.Enabled {
...
func TestParseManifestWithoutOptionalMetadataLeavesZeroValues(t *testing.T) {
	root := filepath.Join(t.TempDir(), "plugins")
	pluginDir := filepath.Join(root, "holt-bare")
	manifestPath := filepath.Join(pluginDir, "plugin.json")
...
func TestLoadPluginsDiscoversDiagnosticsAndProjectPrecedence(t *testing.T) {
	...
	if got := []string{result.Plugins[0].ID + ":" + result.Plugins[0].Name, result.Plugins[1].ID + ":" + result.Plugins[1].Name}; got[0] != "holt.demo:Project Demo" || got[1] != "holt.docs:Docs" {
		t.Fatalf("unexpected plugins: %#v", result.Plugins)
	}
	if result.Plugins[0].Enabled {
		t.Fatalf("project plugin should remain disabled")
	}
	if !hasPluginDiagnostic(result.Diagnostics, DiagnosticDuplicate, "holt.demo") {
...
func TestResolveRootsUsesConfigHomeAndProjectRoot(t *testing.T) {
	...
	if roots[0].Path != filepath.Join(dir, "xdg", "holt", "plugins") {
		t.Fatalf("user root = %q", roots[0].Path)
	}
	if roots[1].Path != filepath.Join(dir, ".holt", "plugins") {
