...
func TestInstallFromRealLocalGitRepo(t *testing.T) {
	...
	if result.ID != "holt.demo" {
		t.Fatalf("ID = %q, want holt.demo", result.ID)
	}
	...
	if _, err := os.Stat(filepath.Join(destDir, "holt.demo", ".git")); err == nil {
...
func TestInstallCopiesLocalPluginAndRecordsHash(t *testing.T) {
	...
	if len(loaded.Plugins) != 1 || loaded.Plugins[0].ID != "holt.demo" {
	...
	if entries["holt.demo"].Hash != result.Hash || entries["holt.demo"].Source != canonicalSource(src) {
		t.Fatalf("lockfile entry unexpected: %#v", entries["holt.demo"])
	}
}
...
func TestInstallNameClashWarnsAndDoesNotOverwriteWithoutForce(t *testing.T) {
	...
	if entries["holt.demo"].Source != canonicalSource(srcB) {
		t.Fatalf("forced overwrite did not update source: %#v", entries["holt.demo"])
	}
}
...
func TestInstallSameLocalSourceDifferentSpellingIsNotAClash(t *testing.T) {
	...
	if entries["holt.demo"].Source != canonicalSource(abs) {
		t.Fatalf("lockfile should record the canonical source %q, got %q", canonicalSource(abs), entries["holt.demo"].Source)
	}
}
...
func TestRemoveDeletesPluginAndLockEntry(t *testing.T) {
	...
	if err := Remove(destDir, "holt.demo"); err != nil {
