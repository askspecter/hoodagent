...
func TestScanHonorsTraversalCaps(t *testing.T) {
	...
	t.Run("max depth holt includes root files only", func(t *testing.T) {
		root := t.TempDir()
...
func TestHelpersClassifySharedWorkspaceRules(t *testing.T) {
	for _, name := range []string{".git", ".holt", ".cache", ".next", "node_modules", "vendor", "dist", "build", "coverage", ".worktrees"} {
