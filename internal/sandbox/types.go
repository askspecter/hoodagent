...
// EnvSandboxed marks a process that holt has already wrapped in a sandbox: every
// wrapped command carries HOLT_SANDBOXED=1 in its environment. When such a
...
// default.
const EnvSandboxed = "HOLT_SANDBOXED"
...
// ambient flag (a stray or hand-exported HOLT_SANDBOXED=1 with no backend marker
// no longer forces an unsandboxed pass-through).
const EnvSandboxBackend = "HOLT_SANDBOX_BACKEND"

// IsAlreadySandboxed reports whether the current process is already running
// inside a holt-created sandbox. It requires BOTH correlated markers that
// sandboxEnvironment sets together — EnvSandboxed == "1" AND a non-empty
// EnvSandboxBackend — so a single user-set/inherited HOLT_SANDBOXED=1 cannot by
// itself disable wrapping. holt sets both only on genuinely wrapped commands;
...
const (
	...
	// elevated `holt sandbox setup` has not run: commands still wrap through the
