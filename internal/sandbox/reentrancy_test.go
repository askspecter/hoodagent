...
func TestIsAlreadySandboxed(t *testing.T) {
	// A lone HOLT_SANDBOXED=1 (no corroborating backend marker) must NOT count as
	...
	// The backend marker alone (no HOLT_SANDBOXED=1) must not count either.
...
func TestBuildCommandPlanWrapsWhenNotAlreadySandboxed(t *testing.T) {
	...
	if !plan.Wrapped || plan.Name != "/usr/bin/holt-linux-sandbox" {
