...
func TestWindowsSandboxSetupPathForRunner(t *testing.T) {
	got := WindowsSandboxSetupPathForRunner(filepath.Join("C:", "holt", WindowsSandboxCommandRunnerName))
	want := filepath.Join("C:", "holt", WindowsSandboxSetupName)
	if got != want {
...
func TestWindowsSandboxSetupConfigFromCommandPreservesProfileInputs(t *testing.T) {
	command := WindowsSandboxCommandConfig{
		...
		Env:     map[string]string{"HOLT_SANDBOXED": "1"},
