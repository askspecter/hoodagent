...
const WindowsSandboxSetupName = "holt-windows-sandbox-setup.exe"
...
func ValidateWindowsSandboxSetupMarker(config WindowsSandboxSetupConfig) error {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return fmt.Errorf("windows sandbox is not initialized for this workspace — run `holt sandbox setup` from an elevated (Administrator) terminal (missing %s)", filepath.Base(path))
		}
