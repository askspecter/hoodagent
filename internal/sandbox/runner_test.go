...
func TestBuildCommandPlanWrapsLinuxHelper(t *testing.T) {
	...
	engine := NewEngine(EngineOptions{
		...
		Backend: Backend{
			...
			Available:  true,
			Executable: "/usr/bin/holt-linux-sandbox",
			Platform:   "linux",
	...
	if !plan.Wrapped || plan.Name != "/usr/bin/holt-linux-sandbox" || plan.Backend.Name != BackendLinuxBwrap {
...
func TestSeatbeltProfileProtectsMetadataAndDenyOrdering(t *testing.T) {
	profile := PermissionProfile{
		FileSystem: FileSystemPolicy{
			...
			WriteRoots: []WritableRoot{{
				...
				ProtectedMetadataNames: []string{".git", ".holt"},
			}},
	...
	for _, want := range []string{
		...
		`(deny file-write* (regex #"^/repo/\.holt(/.*)?$"))`,
		denySecretReadRule,
...
func TestLinuxHelperPlanCarriesExtraWriteRoots(t *testing.T) {
	...
	engine := NewEngine(EngineOptions{
		...
		Policy:        DefaultPolicy(),
		Scope:         scope,
		Backend:       Backend{Name: BackendLinuxBwrap, Available: true, Executable: "/usr/bin/holt-linux-sandbox"},
	})
...
func TestLinuxHelperPlanPreservesRealExtraRootCwd(t *testing.T) {
	...
	bwrapArgs, err := BuildLinuxSandboxBwrapArgs(LinuxSandboxBwrapOptions{Config: config, HelperPath: "/usr/bin/holt-linux-sandbox"})
	if err != nil {
