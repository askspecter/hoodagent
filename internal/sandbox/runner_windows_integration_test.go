...
func TestWindowsRestrictedTokenRealSandboxSmoke(t *testing.T) {
	if os.Getenv("HOLT_SANDBOX_REAL_SMOKE") != "1" {
		t.Skip("set HOLT_SANDBOX_REAL_SMOKE=1 to run real Windows sandbox smoke tests")
	}
	setupExe := realSmokeExecutable(t, "HOLT_WINDOWS_SANDBOX_SETUP_EXE", WindowsSandboxSetupName)
	runnerExe := realSmokeExecutable(t, "HOLT_WINDOWS_COMMAND_RUNNER_EXE", WindowsSandboxCommandRunnerName)

	root := t.TempDir()
	sandboxHome := filepath.Join(root, ".holt-sandbox")
	profile := PermissionProfile{
		FileSystem: FileSystemPolicy{
			Kind:                 FileSystemRestricted,
			ReadRoots:            []string{root},
			WriteRoots:           []WritableRoot{{Root: root, ProtectedMetadataNames: []string{".git", ".holt", ".agents"}}},
