...
func TestPermissionProfileFromPolicyBuildsWorkspaceWriteProfile(t *testing.T) {
	...
	if !stringSliceContains(profile.FileSystem.WriteRoots[0].ProtectedMetadataNames, ".git") || !stringSliceContains(profile.FileSystem.WriteRoots[0].ProtectedMetadataNames, ".holt") {
...
func TestSandboxManagerBuildsExecutionRequestFromProfile(t *testing.T) {
	backend := Backend{Name: BackendLinuxBwrap, Available: true, Executable: "/usr/bin/holt-linux-sandbox", Platform: "linux"}
	policy := DefaultPolicy()
...
func TestSandboxManagerBuildsCommandPlanThroughLinuxHelper(t *testing.T) {
	...
	if !plan.Wrapped || plan.Name != "/usr/bin/holt-linux-sandbox" || plan.TargetBackend != BackendLinuxBwrap {
...
func TestSandboxManagerBuildsCommandPlanThroughWindowsRunner(t *testing.T) {
	...
	backend := Backend{Name: BackendWindowsRestrictedToken, Available: true, Executable: `C:\holt\holt-windows-command-runner.exe`, Platform: "windows"}
	policy := DefaultPolicy()
	...
	if !plan.Wrapped || plan.Name != `C:\holt\holt-windows-command-runner.exe` || plan.TargetBackend != BackendWindowsRestrictedToken {
...
func TestSandboxManagerSelectsPlatformBackend(t *testing.T) {
	tests := []struct {
		...
		{name: "linux", goos: "linux", lookupName: LinuxSandboxHelperName, lookupPath: "/usr/bin/holt-linux-sandbox", want: BackendLinuxBwrap, wantTarget: BackendLinuxBwrap},
		...
		{name: "windows", goos: "windows", lookupName: WindowsSandboxCommandRunnerName, lookupPath: `C:\holt\holt-windows-command-runner.exe`, setupPath: `C:\holt\holt-windows-sandbox-setup.exe`, want: BackendWindowsRestrictedToken, wantTarget: BackendWindowsRestrictedToken},
	...
	backend := SelectBackend(BackendOptions{
		...
		LookupExecutable: func(name string) (string, error) {
			if name == LinuxSandboxHelperName {
				return "/usr/bin/holt-linux-sandbox", nil
			}
			if name == "bwrap" {
				return "/usr/bin/bwrap", nil
			}
