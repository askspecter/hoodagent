...
const WindowsSandboxCommandRunnerName = "holt-windows-command-runner.exe"

const windowsCapabilitySIDSchemaVersion = 2

// Hidden subcommands the main holt binary answers to when it acts as its own
...
// self-dispatch — the running holt binary acting as its own helper — Name is
...
// wins; (2) the same name on PATH; (3) SELF-DISPATCH — the running holt binary
...
func windowsRestrictedTokenCommandPlan(execRequest SandboxExecutionRequest, policy Policy) (CommandPlan, error) {
	...
	// subcommand token (Name is the running holt binary), so the launched command
	// is `holt __windows-command-runner <sandbox args>`. Empty for a standalone
...
func windowsSandboxChildEnv(specEnv []string, policy Policy, workspaceRoot string) []string {
	...
	env = upsertEnvList(env,
		...
		"HOLT_SANDBOX_NETWORK="+string(policy.Network),
		EnvSandboxed+"=1",
...
func ResolveWindowsSandboxHome(env map[string]string) (string, error) {
	if override := strings.TrimSpace(envValue(env, "HOLT_WINDOWS_SANDBOX_HOME")); override != "" {
