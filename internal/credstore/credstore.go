...
// headless/CI machines, so it is opt-in via HOLT_CRED_STORAGE=keyring). A plaintext
// file is available only as an explicit HOLT_CRED_STORAGE=file opt-out. OAuth tokens
...
import (
	...
	"github.com/askspecter/holt/internal/keyring"
	"github.com/askspecter/holt/internal/securefile"
)
...
type Options struct {
	...
	// (plaintext opt-out). When empty it falls back to HOLT_CRED_STORAGE, then auto:
	...
	// users can still opt into the keyring with HOLT_CRED_STORAGE=keyring.
	Storage string
...
func New(options Options) (*Store, error) {
	...
	if storage == "" {
		storage = strings.TrimSpace(env("HOLT_CRED_STORAGE"))
	}
	if storage == "" {
		...
		// encrypted file; keyring stays available via an explicit HOLT_CRED_STORAGE.
