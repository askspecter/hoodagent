...
func TestPowerShellInstallerScriptMatchesWindowsReleaseContracts(t *testing.T) {
	...
	containsAll(t, script, []string{
		`[string]$Repository = $(if ($env:HOLT_REPO)`,
		`Join-Path $env:LOCALAPPDATA "holt\bin"`,
		`$archiveName = "holt-v$releaseVersion-windows-$arch.zip"`,
		`$checksumName = "$archiveName.sha256"`,
		`Get-FileHash -Path $archivePath -Algorithm SHA256`,
		`Expand-Archive -Path $archivePath -DestinationPath $extractDir -Force`,
		`Find-HoltExtractedFile -Root $extractDir -FileName $fileName`,
		`"holt-windows-command-runner.exe"`,
		`"holt-windows-sandbox-setup.exe"`,
		`Copy-Item -Path $sourcePath -Destination (Join-Path $InstallDir $fileName) -Force`,
		`Find-HoltOptionalExtractedDirectory -Root $extractDir -DirectoryName "helpers"`,
...
func TestUnixInstallerInstallsFromPrefixedReleaseArchiveWithoutNetwork(t *testing.T) {
	...
	if !strings.Contains(stdout, "Installed "+filepath.Join(fixture.installDir, "holt")) {
		t.Fatalf("install.sh stdout missing install path:\n%s", stdout)
	}

	installed := readFile(t, filepath.Join(fixture.installDir, "holt"))
	if !strings.Contains(string(installed), "mock-holt") {
		t.Fatalf("installed binary = %q, want mock-holt script", string(installed))
	}
	...
	if _, err := os.Stat(filepath.Join(fixture.installDir, "holt-linux-sandbox")); err != nil {
...
func TestUnixInstallerRejectsChecksumMismatchWithoutNetwork(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(fixture.installDir, "holt")); !os.IsNotExist(err) {
...
func newUnixInstallFixture(t *testing.T) unixInstallFixture {
	...
	packageName := fmt.Sprintf("holt-v0.1.0-%s-%s", releasePlatform, releaseArch)
	...
	writeFile(t, filepath.Join(packageDir, "holt"), []byte("#!/usr/bin/env sh\necho mock-holt\n"), 0o755)
	writeFile(t, filepath.Join(packageDir, "holt-linux-sandbox"), []byte("#!/usr/bin/env sh\n"), 0o755)
	writeFile(t, filepath.Join(packageDir, "holt-seccomp"), []byte("#!/usr/bin/env sh\n"), 0o755)
...
func runUnixInstaller(t *testing.T, fixture unixInstallFixture) (string, string, error) {
	...
	command.Env = append(os.Environ(),
		...
		"HOLT_GITHUB_BASE_URL=https://example.test",
		"HOLT_REPO=askspecter/holt",
	)
