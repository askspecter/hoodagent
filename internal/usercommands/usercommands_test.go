...
func TestLoadParsesFrontmatterAndBody(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, ".holt", "commands")
	...
	if !c.Project {
		t.Fatal("expected Project=true for a .holt/commands file")
	}
}
...
func TestLoadProjectOverridesUser(t *testing.T) {
	root := t.TempDir()
	userCfg := t.TempDir()
	writeFile(t, filepath.Join(userCfg, "holt", "commands"), "review.md", "USER version")
	writeFile(t, filepath.Join(root, ".holt", "commands"), "review.md", "PROJECT version")
