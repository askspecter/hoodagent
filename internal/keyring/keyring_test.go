...
func TestKeyringGetSurfacesNonNotFoundError(t *testing.T) {
	...
	if _, ok, err := k.Get("holt", "tokens"); err == nil || ok {
...
func TestKeyringRoundTripDarwin(t *testing.T) {
	...
	if err := k.Set("holt", "tokens", "blob-AAA"); err != nil {
		t.Fatalf("Set: %v", err)
	}
	got, ok, err := k.Get("holt", "tokens")
	if err != nil || !ok {
	...
	existed, err := k.Delete("holt", "tokens")
	if err != nil || !existed {
		t.Fatalf("Delete: existed=%v err=%v", existed, err)
	}
	if _, ok, _ := k.Get("holt", "tokens"); ok {
...
func TestKeyringRoundTripLinuxUsesStdin(t *testing.T) {
	f := newFake("linux")
	k := f.keyring()
	if err := k.Set("holt", "tokens", "blob-BBB"); err != nil {
		t.Fatalf("Set: %v", err)
	}
...
func TestKeyringGetMissingIsNotError(t *testing.T) {
	for _, goos := range []string{"darwin", "linux"} {
		...
		if _, ok, err := k.Get("holt", "absent"); err != nil || ok {
			t.Fatalf("[%s] Get(absent) = ok=%v err=%v, want false/nil", goos, ok, err)
		}
		if existed, err := k.Delete("holt", "absent"); err != nil || existed {
...
func TestKeyringUnsupportedPlatform(t *testing.T) {
	...
	if err := k.Set("holt", "tokens", "x"); err == nil {
		t.Fatal("Set on unsupported platform should error")
	}
	if _, _, err := k.Get("holt", "tokens"); err == nil {
		t.Fatal("Get on unsupported platform should error")
	}
	if _, err := k.Delete("holt", "tokens"); err == nil {
...
func TestKeyringMissingBinaryError(t *testing.T) {
	...
	if err := k.Set("holt", "tokens", "x"); err == nil || !strings.Contains(err.Error(), "secret-tool") {
