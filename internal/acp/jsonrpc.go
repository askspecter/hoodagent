// Package acp implements the Agent Client Protocol (ACP) surface for HOLT: a
// JSON-RPC 2.0 peer spoken over stdio (newline-delimited JSON) so editors such
// as Zed, JetBrains, and Neovim can drive HOLT's agent core as a backend.
//
...
// here is written originally against HOLT's own interfaces.
package acp
