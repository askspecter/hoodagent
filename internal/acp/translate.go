...
// translate.go maps HOLT's agent events onto ACP session/update payloads. The
...
// toolKindFor maps a HOLT tool name to the closest ACP ToolKind so editors can
...
// toolCallStart maps an advertised HOLT tool call to the initial ACP "tool_call"
// update (status in_progress — HOLT executes immediately after advertising).
...
// toolCallResult maps a finished HOLT tool result to a "tool_call_update".
...
// planUpdate maps HOLT's plan items to an ACP "plan" update.
...
// planStatusToACP maps HOLT's plan status (pending/in_progress/completed/failed)
