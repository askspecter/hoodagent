...
// Deps are the HOLT capabilities the ACP Agent drives. The CLI fills these with
...
// means the editor only hosts the thread — HOLT owns BYOK and telemetry-free
// operation.
type Deps struct {
	ResolveConfig func(workspaceRoot string, overrides config.Overrides) (config.ResolvedConfig, error)
	NewProvider   func(profile config.ProviderProfile) (holtruntime.Provider, error)
	RunAgent      func(ctx context.Context, prompt string, provider holtruntime.Provider, opts agent.Options) (agent.Result, error)
...
func NewAgent(conn *Conn, deps Deps) *Agent {
	...
	conn.Handle(MethodHoltSetModel, a.handleHoltSetModel)
...
func (a *Agent) handleInitialize(_ context.Context, params json.RawMessage) (any, error) {
	...
	return InitializeResult{
		...
		AgentCapabilities: AgentCapabilities{
			// Only advertise what HOLT actually implements: session/load (loadSession)
		...
		AgentInfo: &info,
		// HOLT owns credentials (BYOK) and does not delegate auth to the editor.
...
func (a *Agent) runTurn(ctx context.Context, sess *acpSession, userText string, images []holtruntime.ImageBlock) (string, error) {
...
// requestPermission forwards a HOLT permission prompt to the client as an ACP
...
func (a *Agent) handleHoltSetModel(_ context.Context, params json.RawMessage) (any, error) {
	var p HoltSetModelParams
	if err := json.Unmarshal(params, &p); err != nil {
	...
	return HoltSetModelResult{Model: p.Model}, nil
}
...
func promptImages(blocks []ContentBlock) []holtruntime.ImageBlock {
	var images []holtruntime.ImageBlock
	for _, blk := range blocks {
		...
		images = append(images, holtruntime.ImageBlock{MediaType: blk.MimeType, Data: data})
	}
	return images
}
