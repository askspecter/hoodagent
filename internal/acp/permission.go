...
// permission.go maps HOLT's permission prompt model onto ACP's
// session/request_permission request/option/outcome model. The option id on the
// wire carries the HOLT decision action verbatim, so mapping the client's
// selection back to a HOLT decision is exact and lossless.

// buildPermissionOptions turns the decisions HOLT offers for a tool call into ACP
// PermissionOptions. Only the actions HOLT actually presented (AvailableDecisions)
// are surfaced; the optionId is the HOLT action string for a clean round-trip.
func buildPermissionOptions(req agent.PermissionRequest) []PermissionOption {
	actions := req.AvailableDecisions
	if len(actions) == 0 {
		// Sensible default if HOLT didn't enumerate: allow once / reject.
...
// optionKindFor maps a HOLT decision action to an ACP PermissionOptionKind and a
...
// decisionFromOutcome maps the client's permission outcome back to a HOLT
// decision. A cancelled outcome cancels the run; a selected option id is the HOLT
...
// session/request_permission request from a HOLT permission request.
