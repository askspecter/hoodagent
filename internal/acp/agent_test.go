...
func (f fakeProvider) StreamCompletion(_ context.Context, _ holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	ch := make(chan holtruntime.StreamEvent, 4)
	go func() {
		defer close(ch)
		ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventText, Content: f.text}
...
func testDeps(t *testing.T) Deps {
	...
	return Deps{
		...
		NewProvider: func(config.ProviderProfile) (holtruntime.Provider, error) {
			return fakeProvider{text: "Hello from HOLT"}, nil
		},
		RunAgent: agent.Run,
		...
		Store:                store,
		AgentInfo:            Implementation{Name: "holt", Version: "test"},
	}
}
...
func TestACPEndToEndPrompt(t *testing.T) {
	...
	if got := drainText(t, h.updates); !strings.Contains(got, "Hello from HOLT") {
...
func TestACPRunTurnWiresSandboxAndScopedRegistry(t *testing.T) {
	...
	deps.RunAgent = func(_ context.Context, _ string, _ holtruntime.Provider, opts agent.Options) (agent.Result, error) {
		captured = opts
...
func drainText(t *testing.T, ch <-chan string) string {
	...
	for {
		...
		case s := <-ch:
			b.WriteString(s)
			if strings.Contains(b.String(), "Hello from HOLT") {
				return b.String()
			}
		case <-deadline:
			return b.String()
