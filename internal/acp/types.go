...
// ProtocolVersion is the ACP protocol version HOLT speaks. Wire compatibility is
...
const (
	...
	// Vendor-prefixed HOLT extensions (clients that don't support them ignore the
	// method and degrade cleanly, per the spec's _-prefixed convention).
	MethodHoltSetModel = "_zero/set_model"
)
...
// ContentBlock is the polymorphic content type. HOLT emits "text" (and "image"
...
// directions since the field names do not collide across the variants HOLT uses.
...
// McpServer mirrors the editor-provided MCP server entry. HOLT owns its own MCP
// configuration (BYOK), so these are accepted for spec compliance; HOLT's
...
// ToolCallContent is a tool call's rendered output. HOLT emits "content" (a
// text/image block) and "diff" (a file change); "terminal" is part of the spec
// but unused because HOLT executes locally.
...
type HoltSetModelParams struct {
	SessionID string `json:"sessionId"`
	Model     string `json:"model"`
}

type HoltSetModelResult struct {
	Model string `json:"model"`
}

// configIDModel is the SessionConfigOption id HOLT uses to expose model choice
