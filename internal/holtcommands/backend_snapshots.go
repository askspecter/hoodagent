...
// `holt mcp` command, and PR/CI automation. The snapshot strips
...
// is exposed to the TUI render path, the headless `holt hooks`
...
// is exposed to the TUI render path, the headless `holt plugins`
...
// lane: MCP servers, hooks, and plugins. `holt doctor` and the
// headless `holt config` command can return one of these to give
