...
// holt), then every Every interval, until MaxRuns successful spawns is reached
...
type Schedule struct {
	...
	Every time.Duration
	// FirstDelay delays the first fire. Holt => the first fire happens after Every.
	FirstDelay time.Duration
	// MaxRuns bounds successful spawns. Holt => unbounded (until cancelled).
	MaxRuns int
