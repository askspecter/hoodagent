...
// HOLT must satisfy. The set is recorded by ID with every result so a published
...
// default runner executes to decide pass/fail after HOLT finishes.
...
type TaskConfig struct {
	...
	// production runner (NewExecRunner) invokes headless `holt exec`.
	Runner TaskRunner
}
...
func FormatTaskSummary(result TaskRunResult) string {
	...
	lines := []string{
		"Holt task benchmark: " + displayOrUnknown(result.Suite),
...
// NewExecRunner builds the production runner: it invokes headless `holt exec`
// with stream-json output for each task and decides pass/fail. binary is the path
// to the `holt` binary; extraArgs are appended to every invocation (e.g. sandbox
...
//
// Pass/fail is decided from the stream-json run_end event's exit code: a holt
...
func NewExecRunner(binary string, extraArgs ...string) TaskRunner {
	return func(ctx context.Context, task BenchTask, rc RunContext) TaskOutcome {
		...
		if !haveExit {
			...
			return TaskOutcome{Err: fmt.Errorf("holt exec failed: %s", detail)}
		}
...
// returns its exit code. HOLT emits one JSON object per line; the last run_end
