...
func TestFormatSummaryIncludesWarnings(t *testing.T) {
	...
	result := Result{
		...
		ColdStartCommand:    []string{"/repo/holt", "--version"},
		FirstOutputCommand:  []string{"/repo/holt", "--version"},
		Iterations:          2,
		WarmupIterations:    1,
...
func TestPerfBenchHelperProcess(t *testing.T) {
	if os.Getenv("HOLT_PERF_HELPER") != "1" {
		return
	}
	fmt.Println("holt 0.1.0")
	os.Exit(0)
}
