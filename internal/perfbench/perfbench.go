...
func ResolveColdStartCommand(rootDir string) ([]string, error) {
	return resolveHoltVersionCommand(rootDir)
}
...
func FormatSummary(result Result) string {
	lines := []string{
		fmt.Sprintf("Holt performance benchmark (%s/%s, Go %s)", result.Platform.OS, result.Platform.Arch, result.Platform.GoVersion),
...
func EmitWarnings(w io.Writer, result Result) {
	for _, warning := range result.Warnings {
		_, _ = fmt.Fprintf(w, "::warning title=Holt performance::%s\n", EscapeActionCommand(warning.Message))
	}
}
...
func resolveHoltVersionCommand(rootDir string) ([]string, error) {
	...
	binaryName := release.HoltArtifactName(runtime.GOOS)
	binaryPath := filepath.Join(rootDir, binaryName)
	if _, err := os.Stat(binaryPath); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return nil, fmt.Errorf("no %s binary found; run `go run ./cmd/holt-release build` before running the performance benchmark", binaryName)
		}
		return nil, err
	}
