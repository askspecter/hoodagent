...
func TestBrowserRunUsesHelperManifest(t *testing.T) {
	...
	t.Setenv(EnvHelperManifest, `{"version":1,"helpers":{"agent-browser":{"command":`+quoteJSON(helper)+`,"pathPrepend":[`+quoteJSON(binDir)+`],"env":{"HOLT_HELPER_TEST":"1"}}}}`)
	...
	if !envContains(runner.env, "HOLT_HELPER_TEST=1") {
		t.Fatalf("env = %#v, want HOLT_HELPER_TEST", runner.env)
	}
...
func TestBrowserRunUsesManifestPrefixArgs(t *testing.T) {
	runner := &fakeCommandRunner{}
	t.Setenv(EnvHelperManifest, `{"version":1,"helpers":{"agent-browser":{"command":"cmd.exe","prefixArgs":["/d","/s","/c","C:\\holt\\agent-browser.cmd"]}}}`)
	browser := NewBrowser(BrowserOptions{
		Enabled: true,
		Runner:  runner,
	})
	...
	want := []string{"/d", "/s", "/c", `C:\holt\agent-browser.cmd`, "open", "https://example.com"}
...
func TestMergeEnvReplacesPathCaseInsensitivelyOnWindows(t *testing.T) {
	...
	env := mergeEnv([]string{`Path=C:\Windows`, "HOLT=1"}, []string{`PATH=C:\Holt`})
	pathCount := 0
	for _, item := range env {
		...
		pathCount++
		if value != `C:\Holt` {
			t.Fatalf("PATH value = %q, want C:\\Holt", value)
		}
	}
	if pathCount != 1 {
...
func TestBrowserRunUsesAdjacentPackagedHelper(t *testing.T) {
	root := t.TempDir()
	native := filepath.Join(root, "holt")
