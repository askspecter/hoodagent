...
func TestSHA256FileHashesArchiveBytes(t *testing.T) {
	dir := t.TempDir()
	archivePath := filepath.Join(dir, "holt-v0.1.0-linux-x64.tar.gz")
	archiveBytes := []byte("holt archive bytes")
...
func TestWriteAndVerifyReleaseChecksums(t *testing.T) {
	dir := t.TempDir()
	archiveName := "holt-v0.1.0-linux-x64.tar.gz"
	archivePath := filepath.Join(dir, archiveName)
	if err := os.WriteFile(archivePath, []byte("holt archive bytes"), 0o644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
func TestChecksumParsingRejectsMalformedAndUnsafeNames(t *testing.T) {
	...
	if _, err := FormatSHA256Checksum("abc", "holt.tar.gz"); err == nil || !strings.Contains(err.Error(), "64 hexadecimal") {
		t.Fatalf("FormatSHA256Checksum invalid checksum error = %v", err)
	}
	if _, err := ParseSHA256Checksum(strings.Repeat("a", 64) + "  ../holt.tar.gz\n"); err == nil || !strings.Contains(err.Error(), "same-directory") {
...
func TestVerifyReleaseChecksumsRequiresMatchingFiles(t *testing.T) {
	...
	strayChecksum := filepath.Join(dir, "holt-v0.1.0-macos-arm64.tar.gz.sha256")
	if err := os.WriteFile(strayChecksum, []byte(strings.Repeat("a", 64)+"  holt-v0.1.0-macos-arm64.tar.gz\n"), 0o644); err != nil {
...
func TestReleaseArchiveNamesMatchInstallerContracts(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			goos:        "linux",
			goarch:      "amd64",
			packageName: "holt-v0.1.0-linux-x64",
			archiveName: "holt-v0.1.0-linux-x64.tar.gz",
		},
		{
			name:        "macos arm64",
			...
			goos:        "darwin",
			goarch:      "arm64",
			packageName: "holt-v0.1.0-macos-arm64",
			archiveName: "holt-v0.1.0-macos-arm64.tar.gz",
		},
		{
			name:        "windows amd64",
			version:     "0.1.0",
			goos:        "windows",
			goarch:      "amd64",
			packageName: "holt-v0.1.0-windows-x64",
			archiveName: "holt-v0.1.0-windows-x64.zip",
		},
	}
...
func TestBuildHelpersMatchScriptContracts(t *testing.T) {
	...
	if got := DefaultBuildOutput(root, "windows"); got != filepath.Join(root, "holt.exe") {
		t.Fatalf("DefaultBuildOutput(windows) = %q", got)
	}
	if got := DefaultBuildOutput(root, "linux"); got != filepath.Join(root, "holt") {
		t.Fatalf("DefaultBuildOutput(linux) = %q", got)
	}
	if got := WindowsSandboxCommandRunnerArtifactName("windows"); got != "holt-windows-command-runner.exe" {
		t.Fatalf("WindowsSandboxCommandRunnerArtifactName(windows) = %q", got)
	}
	if got := WindowsSandboxSetupArtifactName("windows"); got != "holt-windows-sandbox-setup.exe" {
		t.Fatalf("WindowsSandboxSetupArtifactName(windows) = %q", got)
	}
	if got := BuildLdflags(version); !strings.Contains(got, "-X github.com/askspecter/holt/internal/cli.version=0.1.0") {
...
func TestSmokeRejectsMissingDefaultArtifact(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "build artifact not found: holt") {
...
func TestCreateArchivesWithRootPackageFiles(t *testing.T) {
	t.Run("tar gz", func(t *testing.T) {
		stagingDir := packageStagingFixture(t, "holt")
		archivePath := filepath.Join(t.TempDir(), "holt-v0.1.0-linux-x64.tar.gz")
		...
		for _, want := range []string{"holt", "README.md", "bin/holt.js", "VERSION"} {
			if !names[want] {
	...
	t.Run("zip", func(t *testing.T) {
		stagingDir := packageStagingFixture(t, "holt.exe")
		archivePath := filepath.Join(t.TempDir(), "holt-v0.1.0-windows-x64.zip")
		if err := createArchive(stagingDir, archivePath, "windows"); err != nil {
			t.Fatalf("createArchive returned error: %v", err)
		}
		names := zipArchiveNames(t, archivePath)
		for _, want := range []string{"holt.exe", "README.md", "bin/holt.js", "VERSION"} {
			if !names[want] {
...
func TestCopyPackageFilesStagesLinuxSandboxHelper(t *testing.T) {
	root := t.TempDir()
	staging := t.TempDir()
	artifact := filepath.Join(root, "holt")
	helper := filepath.Join(root, "holt-linux-sandbox")
	seccomp := filepath.Join(root, "holt-seccomp")
	for path, content := range map[string]string{
		...
		filepath.Join(root, "bin", "holt.js"): "wrapper",
	} {
		mustWriteFile(t, path, content)
	}
	if err := copyPackageFiles(root, staging, artifact, filepath.Join(staging, "holt"), "linux", "0.1.0", map[string]string{
		"holt-linux-sandbox": helper,
		"holt-seccomp":       seccomp,
	}); err != nil {
		t.Fatalf("copyPackageFiles: %v", err)
	}
	if _, err := os.Stat(filepath.Join(staging, "holt-linux-sandbox")); err != nil {
		t.Fatalf("staged helper missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(staging, "holt-seccomp")); err != nil {
...
func TestCopyPackageFilesStagesWindowsSandboxHelpers(t *testing.T) {
	root := t.TempDir()
	staging := t.TempDir()
	artifact := filepath.Join(root, "holt.exe")
	runner := filepath.Join(root, "holt-windows-command-runner.exe")
	setup := filepath.Join(root, "holt-windows-sandbox-setup.exe")
	for path, content := range map[string]string{
		artifact:                              "holt",
		runner:                                "runner",
		setup:                                 "setup",
	...
	if err := copyPackageFiles(root, staging, artifact, filepath.Join(staging, "holt.exe"), "windows", "0.1.0", map[string]string{
		"holt-windows-command-runner.exe": runner,
		"holt-windows-sandbox-setup.exe":  setup,
	}); err != nil {
		t.Fatalf("copyPackageFiles: %v", err)
	}
	if _, err := os.Stat(filepath.Join(staging, "holt-windows-command-runner.exe")); err != nil {
		t.Fatalf("staged runner missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(staging, "holt-windows-sandbox-setup.exe")); err != nil {
