...
func Build(ctx context.Context, options BuildOptions) (BuildResult, error) {
	...
	if err := buildHolt(ctx, rootDir, output, version, goos, goarch); err != nil {
		return BuildResult{}, err
	}
	return BuildResult{
		OutputPath: output,
...
func Package(ctx context.Context, options PackageOptions) (PackageResult, error) {
	...
	artifactPath := filepath.Join(rootDir, HoltArtifactName(goos))
	stagedBinaryPath := filepath.Join(stagingDir, HoltArtifactName(goos))
	...
	if err := buildHolt(ctx, rootDir, artifactPath, version, goos, goarch); err != nil {
		return PackageResult{}, err
	}
...
func HoltArtifactName(goos string) string {
	if goos == "windows" {
		return "holt.exe"
	}
	return "holt"
...
func LinuxSandboxHelperArtifactName(goos string) string {
	if goos == "windows" {
		return "holt-linux-sandbox.exe"
	}
	return "holt-linux-sandbox"
}

func LinuxSeccompHelperArtifactName(goos string) string {
	if goos == "windows" {
		return "holt-seccomp.exe"
	}
	return "holt-seccomp"
}
...
func WindowsSandboxCommandRunnerArtifactName(goos string) string {
	if goos == "windows" {
		return "holt-windows-command-runner.exe"
	}
	return "holt-windows-command-runner"
}

func WindowsSandboxSetupArtifactName(goos string) string {
	if goos == "windows" {
		return "holt-windows-sandbox-setup.exe"
	}
	return "holt-windows-sandbox-setup"
}

func DefaultBuildOutput(rootDir string, goos string) string {
	return filepath.Join(rootDir, HoltArtifactName(goos))
}

func BuildLdflags(version string) string {
	return "-s -w -X github.com/askspecter/holt/internal/cli.version=" + version
}
...
func ReleasePackageName(version string, goos string, goarch string) (string, error) {
	...
	return fmt.Sprintf("holt-v%s-%s-%s", version, platform, arch), nil
}
...
func buildHolt(ctx context.Context, rootDir string, output string, version string, goos string, goarch string) error {
	return buildGoPackage(ctx, rootDir, output, version, goos, goarch, "./cmd/holt")
}
...
func buildLinuxSandboxHelper(ctx context.Context, rootDir string, output string, goos string, goarch string) error {
	...
	return buildGoPackage(ctx, rootDir, output, "", goos, goarch, "./cmd/holt-linux-sandbox")
}
...
func buildLinuxSeccompHelper(ctx context.Context, rootDir string, output string, goos string, goarch string) error {
	...
	return buildGoPackage(ctx, rootDir, output, "", goos, goarch, "./cmd/holt-seccomp")
}
...
func buildWindowsSandboxCommandRunner(ctx context.Context, rootDir string, output string, goos string, goarch string) error {
	...
	return buildGoPackage(ctx, rootDir, output, "", goos, goarch, "./cmd/holt-windows-command-runner")
}
...
func buildWindowsSandboxSetup(ctx context.Context, rootDir string, output string, goos string, goarch string) error {
	...
	return buildGoPackage(ctx, rootDir, output, "", goos, goarch, "./cmd/holt-windows-sandbox-setup")
}
...
func copyPackageFiles(rootDir string, stagingDir string, artifactPath string, stagedBinaryPath string, goos string, version string, helperArtifacts map[string]string) error {
	...
	if err := copyFile(filepath.Join(rootDir, "bin", "holt.js"), filepath.Join(stagingDir, "bin", "holt.js"), 0o755); err != nil {
		return err
	}
