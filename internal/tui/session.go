...
func (m model) resolveResumeSession(args string) (*sessions.Metadata, error) {
	if strings.EqualFold(args, "latest") {
		...
		if latest == nil {
			return nil, errors.New("no holt sessions available to resume")
		}
		return latest, nil
	}
	...
	if session == nil {
		return nil, fmt.Errorf("holt session not found: %s", args)
	}
	return session, nil
}
...
// the CLI's `holt exec --resume` (readExecContextEvents) and the in-TUI /compact
