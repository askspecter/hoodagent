...
// tuiTheme is the resolved terminal palette Holt renders with. It is produced by
// buildTheme from a palette, so the same renderers serve both the dark default
// and the light variant; the active theme lives in the package var holtTheme and
// may be swapped at startup (background detection / HOLT_THEME / --theme) or live
...
// holtTheme is the active palette every renderer reads. It defaults to dark and is
...
// subsequent render.
var holtTheme = buildTheme(darkPalette)
