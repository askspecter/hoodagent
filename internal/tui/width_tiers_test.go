...
func TestWidthTierSegments(t *testing.T) {
	...
	for _, tc := range cases {
		m := newModel(context.Background(), Options{
			Cwd:          "/Users/dev/holt-project-workspace",
		...
		if got := strings.Contains(title, "holt-project-workspace"); got != tc.wantCwd {
...
func TestTitleBarKeepsWorkspaceWithLongBranchAndModel(t *testing.T) {
	...
	for _, want := range []string{"", "/workspace/holt", "feat/tui-assistant-response-cleanup", "ollama-cloud/cogito-2.1:671b-extra-long-model-name"} {
...
func TestComposerDividerRendersMetaAtExactFit(t *testing.T) {
	...
	meta := holtTheme.muted.Render("gpt-4o")
...
func TestViewNeverExceedsTerminalWidth(t *testing.T) {
	...
	for _, width := range []int{24, 40, 58, 70, 80, 100, 120} {
		...
		m.transcript = append(m.transcript,
			...
			transcriptRow{kind: rowToolResult, id: "c1", tool: "grep", status: tools.StatusOK, detail: "internal/cli/root.go:41: fs := flag.NewFlagSet(\"holt\", flag.ContinueOnError)"},
...
var permissionEventLongDetailFixture = agent.PermissionEvent{
	...
	Permission:     "prompt",
	PermissionMode: agent.PermissionModeAsk,
	SideEffect:     "runs `go test ./... -timeout 600s` in /Users/dev/holt-project-workspace with network access",
