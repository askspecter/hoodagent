...
func renderRecapRow(row transcriptRow, width int) string {
	return fitStyledLine(holtTheme.faint.Render("※ recap: "+strings.TrimSpace(row.text)), width)
}
...
//	under $HOME    → ~/projects/holt/main.go
...
func renderUserRow(row transcriptRow, width int) string {
	...
	for _, line := range wrapped {
		lines = append(lines, renderUserPromptStyledLine(holtTheme.ink.Bold(true).Render(line), contentWidth))
	}
...
func renderUserPromptStyledLine(styledText string, contentWidth int) string {
	if contentWidth <= 0 {
		return holtTheme.userPrompt.Render("▌")
	}
	fitted := fitStyledLine(styledText, contentWidth)
	return holtTheme.userPrompt.Render("▌") + "  " + fitted
}
...
func renderAssistantRow(row transcriptRow, width int) string {
	...
	if !row.final {
		...
		for _, line := range narr {
			styled := styleAssistantMarkdownLine(line, holtTheme.ink)
	...
	for index := range lines {
		lines[index] = styleAssistantMarkdownLine(lines[index], holtTheme.ink)
	}
...
func renderReasoningBlock(text string, expanded bool, width int, running bool, elapsed time.Duration, maxBodyLines int) string {
	...
	for _, line := range body {
		lines = append(lines, fitStyledLine("  "+styleAssistantMarkdownLine(line, holtTheme.sayText), width))
	}
	return strings.Join(lines, "\n")
}
...
func reasoningHiddenMarker(hidden int) string {
	return holtTheme.faint.Render(fmt.Sprintf("… %d earlier lines · Ctrl+O for all", hidden))
}
...
func reasoningHeaderLine(text string, expanded bool, running bool, elapsed time.Duration) string {
	return holtTheme.faint.Render(reasoningHeaderText(text, expanded, running, elapsed))
}
...
func doneLine(row transcriptRow) string {
	return holtTheme.faint.Render("worked for " + formatElapsedSeconds(row.turnElapsed))
}
...
func renderSystemNote(text string, width int) string {
	...
	marker, style := holtTheme.faint.Render("·"), holtTheme.muted
	if isCancellationNotice(trimmed) {
		marker, style = holtTheme.amber.Render("⊘"), holtTheme.amber
	}
...
func renderCommandCardRow(text string, width int) string {
	...
	for _, line := range raw[1:] {
		...
		case isCommandCardActionsLine(trimmed):
			lines = append(lines, holtTheme.accent.Render("actions: ")+holtTheme.ink.Render(strings.TrimSpace(strings.TrimPrefix(trimmed, "actions:"))))
		case isCommandCardHintLine(trimmed):
			lines = append(lines, holtTheme.faint.Render(line))
		...
		default:
			...
			lines = append(lines, holtTheme.accent.Bold(true).Render(line))
		}
	}
	return styledBlockFillTitle(width, title, lines, holtTheme.accent, lipgloss.NewStyle())
}
...
func renderPlanCardRow(text string, width int) string {
	...
	for _, line := range raw[1:] {
		...
		default:
			...
			lines = append(lines, holtTheme.muted.Bold(true).Render(line))
		}
	}
	return styledBlockFillTitleStyled(width, title, lines, holtTheme.faintest, lipgloss.NewStyle(), planCardTitleStyle(title))
}
...
func styleCommandCardContentRow(line string) string {
	...
	if strings.HasPrefix(body, "/") {
		if name, desc, ok := strings.Cut(body, " - "); ok {
			return indent + holtTheme.ink.Bold(true).Render(name) + holtTheme.muted.Render(" — "+desc)
		}
		return indent + holtTheme.ink.Bold(true).Render(body)
	}
	// "- bullet" list item: keep it readable (not dim grey).
	if strings.HasPrefix(body, "- ") {
		return indent + holtTheme.ink.Render(body)
	}
...
func styledCommandCardStatus(trimmed string) string {
	...
	switch state {
	case "warning":
		return holtTheme.amber.Render(trimmed)
	case "blocked":
		return holtTheme.red.Render(trimmed)
...
func renderMCPManagerCard(text string, width int) string {
	...
	for index, line := range raw {
		...
		case index == 0:
			lines = append(lines, holtTheme.accent.Bold(true).Render(line))
		case index == 1:
			lines = append(lines, holtTheme.ink.Bold(true).Render(line))
		case isMCPManagerHeading(trimmed):
			lines = append(lines, holtTheme.accent.Bold(true).Render(line))
		case strings.Contains(trimmed, "holt mcp "):
			lines = append(lines, holtTheme.ink.Render(line))
		case strings.HasPrefix(trimmed, "›") || strings.HasPrefix(trimmed, "- "):
			lines = append(lines, holtTheme.ink.Render(line))
		default:
			lines = append(lines, holtTheme.muted.Render(line))
		}
	}
	return styledBlock(width, lines, holtTheme.accent)
}
...
func renderCompactRunningCard(text string, width int) string {
	...
	for index, line := range raw {
		switch index {
		case 0:
			lines = append(lines, holtTheme.amber.Bold(true).Render(line))
		case 1:
			lines = append(lines, holtTheme.muted.Render(line))
		case 2:
			lines = append(lines, holtTheme.amber.Bold(true).Render(line))
	...
	return styledBlock(width, lines, holtTheme.amber)
}
...
func renderCompactCompleteCard(text string, width int) string {
	...
	for index, line := range raw {
		switch index {
		case 0:
			lines = append(lines, holtTheme.green.Bold(true).Render(line))
		case 1:
			lines = append(lines, holtTheme.ink.Render(line))
		default:
			lines = append(lines, holtTheme.muted.Render(line))
	...
	return styledBlock(width, lines, holtTheme.green)
}
...
func renderDoctorResultCard(text string, width int) string {
	...
	for index, line := range raw {
		...
		case strings.HasPrefix(trimmed, "- [pass]"):
			lines = append(lines, holtTheme.green.Render(line))
		case strings.HasPrefix(trimmed, "- [warn]"):
			lines = append(lines, holtTheme.amber.Render(line))
		case strings.HasPrefix(trimmed, "- [fail]"):
			lines = append(lines, holtTheme.red.Render(line))
...
func doctorResultBorderStyle(text string) lipgloss.Style {
	for _, line := range strings.Split(strings.ReplaceAll(text, "\r\n", "\n"), "\n") {
		...
		case "status: blocked":
			return holtTheme.red
		case "status: warning":
			return holtTheme.amber
		}
	}
	return holtTheme.accent
}
...
func renderErrorRow(row transcriptRow, width int) string {
	note := noteBox(row.text, width, holtTheme.cardErr, holtTheme.red)
	return note
}
...
func renderAskUserRow(row transcriptRow, width int) string {
	line := fitStyledLine(holtTheme.accent.Render("ask holt")+"  "+holtTheme.ink.Render(strings.TrimPrefix(row.text, "ask_user: ")), width)
...
func renderPermissionRow(row transcriptRow, width int) string {
	event := row.permission
	if event == nil {
		return holtTheme.amber.Render("permission") + "  " + holtTheme.ink.Render(row.text)
	}
	...
	case agent.PermissionActionAllow:
		...
		line := holtTheme.green.Render(label) + dot + holtTheme.green.Render(displayName)
		if scope := strings.TrimSpace(event.Scope); scope != "" {
			line += dot + holtTheme.muted.Render(permissionEventScopeLabel(event)+":"+scope)
		}
		return fitStyledLine(line, width)
	case agent.PermissionActionDeny:
		line := holtTheme.red.Render("denied") + dot + holtTheme.red.Render(displayName)
		if scope := strings.TrimSpace(event.Scope); scope != "" {
			line += dot + holtTheme.muted.Render(permissionEventScopeLabel(event)+":"+scope)
		}
		if reason := permissionDisplayReason(event.Reason); reason != "" {
			line += holtTheme.faint.Render(" — " + truncateRunes(reason, maxInt(16, width-lipgloss.Width(displayName)-16)))
		}
		...
		return out
	case agent.PermissionActionCancel:
		line := holtTheme.red.Render("cancelled") + dot + holtTheme.red.Render(displayName)
		if scope := strings.TrimSpace(event.Scope); scope != "" {
			line += dot + holtTheme.muted.Render(permissionEventScopeLabel(event)+":"+scope)
		}
		if reason := permissionDisplayReason(event.Reason); reason != "" {
		...
		return out
	default:
		line := holtTheme.amber.Render("permission") + "  " + holtTheme.ink.Render(displayName) + "  " + holtTheme.amber.Render("prompt")
		if scope := strings.TrimSpace(event.Scope); scope != "" {
			line += "  " + holtTheme.muted.Render(permissionEventScopeLabel(event)+":"+scope)
		}
		out := fitStyledLine(line, width)
		if detail := strings.TrimSpace(row.detail); detail != "" {
			out += "\n" + wrapDetailBlock(detail, width)
...
func wrapDetailBlock(detail string, width int) string {
	...
	for index := range lines {
		lines[index] = "  " + holtTheme.muted.Render(lines[index])
	}
	return strings.Join(lines, "\n")
}
...
func renderFocusedPermissionPrompt(request agent.PermissionRequest, cursor int, width int) (string, []int) {
	...
	fill := holtTheme.onPerm

	top := holtTheme.permBadge.Render(" PERMISSION ")

	body := fill(holtTheme.amber).Bold(true).Render(name)
	if request.ToolName == tools.RequestPermissionsToolName {
		body = fill(holtTheme.amber).Bold(true).Render("Grant requested permissions?")
	} else if request.SideEffect != "" {
		body += fill(holtTheme.ink).Render("  " + request.SideEffect)
	}
	lines := []string{top, body}
	if reason := permissionDisplayReason(request.Reason); reason != "" {
		lines = append(lines, fill(holtTheme.muted).Render(reason))
	}
	...
	if scope := strings.TrimSpace(request.Scope); scope != "" {
		lines = append(lines, fill(holtTheme.muted).Render(permissionScopeLine(request, scope)))
	}
	...
	for index, option := range options {
		...
		hotkey := fill(holtTheme.faint).Render(" [" + option.hotkey + "]")
		optionLabel := permissionOptionLabel(option, request)
		if index == cursor {
			marker := fill(holtTheme.accent).Render("▸ ")
			label := holtTheme.badge.Render(" " + optionLabel + " ")
			lines = append(lines, marker+label+hotkey)
		} else {
			label := fill(holtTheme.ink).Render(optionLabel)
	...
	lines = append(lines, fill(holtTheme.faint).Render(footer))

	return styledBlockFill(width, lines, holtTheme.permBorder, holtTheme.permBg), offsets
}
...
func permissionOptionLabel(option permissionOption, request agent.PermissionRequest) string {
	...
	case permissionDecisionCancel:
		return "No, and tell Holt what to do differently"
	default:
		return option.label
	}
}
...
func renderAskUserQuestionnaire(prompt pendingAskUserPrompt, input string, width int) string {
	...
	if len(questions) == 0 {
		return styledBlockFill(width, []string{holtTheme.badge.Render(" ASK ")}, holtTheme.lineStrong, lipgloss.NewStyle())
	}
	...
	if header := strings.TrimSpace(prompt.request.Header); header != "" {
		lines = append(lines, fill(holtTheme.ink).Render(header))
	}
	...
	if multi {
		...
		for index, question := range questions {
			...
			switch {
			case index == active:
				tabs = append(tabs, holtTheme.badge.Render(" "+title+" "))
			case prompt.states[index].answered:
				tabs = append(tabs, fill(holtTheme.muted).Render(" ✓ "+title+" "))
			default:
				tabs = append(tabs, fill(holtTheme.faint).Render(" "+title+" "))
			}
		}
		if active == confirm {
			tabs = append(tabs, holtTheme.badge.Render(" Confirm "))
		} else {
			tabs = append(tabs, fill(holtTheme.faint).Render(" Confirm "))
		}
	...
	if active == confirm {
		lines = append(lines, "")
		lines = append(lines, fill(holtTheme.ink).Render("Review and submit:"))
		for index, question := range questions {
			answer := strings.TrimSpace(prompt.states[index].answer)
			rendered := fill(holtTheme.ink).Render(answer)
			if answer == "" {
				rendered = fill(holtTheme.faint).Render("(no answer)")
			}
			lines = append(lines, "  "+fill(holtTheme.muted).Render(askUserTabTitle(question, index)+": ")+rendered)
		}
		lines = append(lines, "")
		lines = append(lines, fill(holtTheme.faint).Render("⇆ tab · enter submit · esc dismiss"))
		return styledBlockFill(width, lines, holtTheme.lineStrong, lipgloss.NewStyle())
	}

	question := questions[active]
	state := prompt.states[active]
	lines = append(lines, "")
	lines = append(lines, fill(holtTheme.ink).Render(question.Question))
	...
	if len(question.Options) > 0 && !state.typing {
		...
		for index, option := range question.Options {
			...
			if index == cursor {
				lines = append(lines, fill(holtTheme.accent).Render("▸ ")+holtTheme.badge.Render(" "+label+" "))
			} else {
				lines = append(lines, "  "+fill(holtTheme.ink).Render(label))
			}
			if index < len(question.OptionDescriptions) && strings.TrimSpace(question.OptionDescriptions[index]) != "" {
				lines = append(lines, "     "+fill(holtTheme.faint).Render(question.OptionDescriptions[index]))
			}
		}
		typeOwn := fmt.Sprintf("%d. %s", len(question.Options)+1, askUserTypeMyOwnLabel)
		if cursor >= len(question.Options) {
			lines = append(lines, fill(holtTheme.accent).Render("▸ ")+holtTheme.badge.Render(" "+typeOwn+" "))
		} else {
			lines = append(lines, "  "+fill(holtTheme.muted).Render(typeOwn))
		}
		lines = append(lines, "")
	...
	if question.MultiSelect && len(question.Options) > 0 {
		lines = append(lines, fill(holtTheme.muted).Render("suggested: "+strings.Join(question.Options, ", ")))
	}
	lines = append(lines, holtTheme.userPrompt.Render("❯ ")+fill(holtTheme.ink).Render(input)+fill(holtTheme.accent).Render("▌"))
...
func (m model) renderRunningToolCard(row transcriptRow, width int, rc rowContext, opts cardRenderOptions) string {
	glyph := holtTheme.faintest.Render("…")
	if m.pending && row.runID != 0 && row.runID == m.activeRunID {
		glyph = holtTheme.accent.Render(m.spinnerGlyph())
	}
	...
	head := toolCardHead(toolRowName(row), hint, arg, "", "", "", true, holtTheme.ink, rc.auto[rcKey(row.runID, row.id)], width, opts)
	return toolCard(head, glyph, nil, "", holtTheme.cardRun, width)
}
...
func renderToolResultCard(row transcriptRow, width int, rc rowContext, opts cardRenderOptions) string {
	...
	glyph := holtTheme.green.Render("•")
	nameStyle := holtTheme.green
	borderStyle := holtTheme.line
	if opts.fileSelected {
		...
		borderStyle = holtTheme.accent
	}
	if failed {
		glyph = holtTheme.red.Render("•")
		nameStyle = holtTheme.red
		borderStyle = holtTheme.cardErr
	}
...
func toolCardHead(name string, target string, arg string, headTag string, detail string, actionDetail string, running bool, nameStyle lipgloss.Style, auto bool, width int, opts cardRenderOptions) string {
	...
	if target = singleLineToolHeadText(toolCardHeadTarget(name, target, detail)); target != "" {
		...
		styled := holtTheme.toolTarget.Render(middleTruncate(shown, maxInt(16, width/2)))
		if looksLikePath(target) {
	...
	if arg = singleLineToolHeadText(arg); arg != "" && widthTier(width) == tierFull {
		head += "  " + holtTheme.toolArg.Render(truncateRunes(arg, maxInt(12, width/3)))
	}
	if headTag != "" {
		if strings.Contains(headTag, "\x1b") {
			head += "  " + headTag
		} else {
			head += "  " + holtTheme.faint.Render(headTag)
		}
	}
...
func capCardLines(lines []string, cap int) []string {
	...
	lines = lines[:cap]
	return append(lines, holtTheme.faint.Render(fmt.Sprintf("… %d more lines", hidden)))
}
...
func diffCountTag(adds int, dels int) string {
	...
	return holtTheme.faint.Render("(") +
		holtTheme.diffAdd.Render(fmt.Sprintf("+%d", adds)) +
		holtTheme.faint.Render(" ") +
		holtTheme.diffDel.Render(fmt.Sprintf("-%d", dels)) +
		holtTheme.faint.Render(")")
}
...
func diffCardBody(detail string, width int, opts cardRenderOptions) cardBody {
	...
	for i := 0; i < len(rawLines); i++ {
		...
		case !inHunk, strings.HasPrefix(line, `\`):
			...
			lines = append(lines, holtTheme.diffMeta.Render(truncateRunes(line, innerWidth)))
		case strings.HasPrefix(line, "+"):
		...
		default:
			text := truncateRunes(strings.TrimPrefix(line, " "), textBudget)
			row := "   " + holtTheme.muted.Render(text)
			if gutter {
				row = holtTheme.faintest.Render(fmt.Sprintf("%4d", newLine)) + row
			}
...
func highlightedAddedDiffLines(rawLines []string, meta diffMetadata, textBudget int) []string {
	...
	highlighted, ok := highlightCodeForPath(content, meta.path, textBudget, holtTheme.addLine.GetBackground())
...
func diffBodyLine(number int, sign string, text string, added bool, textBudget int, gutter bool) string {
	...
	if gutter {
		...
		if added {
			numCol = holtTheme.addLineNum.Render(num)
		} else {
			numCol = holtTheme.delLineNum.Render(num)
		}
	}
	if added {
		return numCol + holtTheme.addSign.Render(" "+sign+" ") + holtTheme.addLine.Render(text)
	}
	return numCol + holtTheme.delSign.Render(" "+sign+" ") + holtTheme.delLine.Render(text)
}

func diffBodyStyledLine(number int, sign string, styledText string, added bool, textBudget int, gutter bool) string {
	lineStyle, signStyle, numStyle := holtTheme.delLine, holtTheme.delSign, holtTheme.delLineNum
	if added {
		lineStyle, signStyle, numStyle = holtTheme.addLine, holtTheme.addSign, holtTheme.addLineNum
	}
...
func diffBodyLineSpanned(number int, sign string, text []rune, added bool, spanStart, spanEnd, textBudget int, gutter bool) string {
	...
	lineStyle, wordStyle, signStyle, numStyle := holtTheme.delLine, holtTheme.delLineWord, holtTheme.delSign, holtTheme.delLineNum
	if added {
		lineStyle, wordStyle, signStyle, numStyle = holtTheme.addLine, holtTheme.addLineWord, holtTheme.addSign, holtTheme.addLineNum
	}
...
func exploreCardBody(name string, hint string, arg string, detail string, width int, opts cardRenderOptions, failed bool) cardBody {
	...
	if !opts.expanded && opts.bodyCap > 0 {
		footer := ""
		if strings.TrimSpace(detail) != "" {
			footer = holtTheme.faint.Render("▸ details")
		}
	...
	if opts.expanded && opts.bodyCap > 0 && footer == "" {
		footer = holtTheme.faint.Render("▾ collapse")
	}
...
func exploreCardLine(name string, hint string, arg string, detail string, width int, opts cardRenderOptions, marker string) string {
	...
	line := holtTheme.faint.Render("  "+marker+" ") + holtTheme.green.Render(action)
	if target != "" {
		shown := target
		...
		styled := holtTheme.toolTarget.Render(middleTruncate(shown, maxInt(8, width-lipgloss.Width(action)-6)))
		if isPath {
			styled = hyperlink(fileURL(opts.cwd, target), styled)
		}
		line += " " + styled
...
func localControlChildLine(text string, width int) string {
	...
	return holtTheme.faint.Render(prefix) + holtTheme.muted.Render(truncateDisplayWidth(text, budget))
}
...
func bashCardBody(command string, detail string, width int, opts cardRenderOptions) cardBody {
	...
	for _, line := range strings.Split(detail, "\n") {
		...
		case strings.HasPrefix(line, "exit_code: "):
			...
			if code != "0" {
				footer = holtTheme.red.Render("exit " + code)
			}
		default:
			style := holtTheme.muted
			if section == "stderr" {
				style = holtTheme.delText
			}
...
func execCommandCardBody(command string, detail string, width int, opts cardRenderOptions) cardBody {
	...
	for _, line := range strings.Split(detail, "\n") {
		...
		case line == "interrupted: true":
			interrupted = true
			footer = holtTheme.green.Render("interrupted")
		case strings.HasPrefix(line, "exit_code: "):
			code := strings.TrimPrefix(line, "exit_code: ")
			if code == "0" {
		...
		case strings.HasPrefix(line, "session_id: "):
			footer = holtTheme.faint.Render("session " + strings.TrimSpace(strings.TrimPrefix(line, "session_id: ")))
		case strings.HasPrefix(line, "Use write_stdin "):
			continue
		default:
			style := holtTheme.muted
			if section == "" && strings.HasPrefix(line, "Command is still running.") {
				style = holtTheme.faint
			}
			output = append(output, commandOutputLine{text: line, style: style})
		}
	}
...
func renderCommandOutputLines(output []commandOutputLine, width int, opts cardRenderOptions) []string {
	...
	if len(output) == 1 {
		...
		return capCardLines([]string{holtTheme.faint.Render(prefix) + output[0].style.Render(text)}, opts.bodyCap)
	}
	...
	for _, item := range output {
		...
		lines = append(lines, holtTheme.faint.Render(prefix)+item.style.Render(text))
	}
...
func renderSessionsCards(payload string, width int) string {
	...
	for _, record := range strings.Split(payload, "\n") {
		...
		if len(fields) < 4 {
			blocks = append(blocks, fitStyledLine(holtTheme.faint.Render(record), width))
			continue
		}
		id, age, title, meta := fields[0], fields[1], fields[2], fields[3]
		innerWidth := width - 4
		top := joinHeaderLine(holtTheme.onPanel(holtTheme.accent).Render(id), holtTheme.onPanel(holtTheme.faint).Render(age), innerWidth)
		metaParts := strings.Split(meta, " · ")
		for index := range metaParts {
			metaParts[index] = holtTheme.onPanel(holtTheme.faint).Render(metaParts[index])
		}
		lines := []string{
			top,
			holtTheme.onPanel(holtTheme.ink).Render(title),
			strings.Join(metaParts, holtTheme.onPanel(holtTheme.faintest).Render(" · ")),
		}
		blocks = append(blocks, styledBlockFill(width, lines, holtTheme.line, holtTheme.panel))
	}
...
func grepCardBody(detail string, width int, opts cardRenderOptions) cardBody {
	...
	for _, line := range raw {
		if match := grepMatchPattern.FindStringSubmatch(line); match != nil {
			matches++
			location := holtTheme.grepLoc.Render(match[1])
			...
			lines = append(lines, location+"  "+holtTheme.muted.Render(truncateDisplayWidth(match[2], budget)))
			continue
		}
		lines = append(lines, holtTheme.muted.Render(line))
	}
	footer := ""
	if matches > 0 {
		footer = holtTheme.faint.Render(fmt.Sprintf("%d matches", matches))
	}
	return cardBody{lines: capCardLines(lines, opts.bodyCap), footer: footer}
}
