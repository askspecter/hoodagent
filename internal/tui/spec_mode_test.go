...
func TestSpecCommandCreatesDraftReview(t *testing.T) {
	...
	if !strings.Contains(next.pendingSpecReview.RelativePath, ".holt/specs/") {
...
func TestSpecReviewBlocksShiftTabModeCycle(t *testing.T) {
	...
	m.pendingSpecReview = &pendingSpecReviewPrompt{SpecID: "spec", SpecFilePath: ".holt/specs/spec.md"}
...
func newSpecModeTestModel(root string, provider holtruntime.Provider, store *sessions.Store) model {
...
func submitSpecScript(callID string, title string, plan string) []holtruntime.StreamEvent {
	args, _ := json.Marshal(map[string]string{
		"title": title,
		"plan":  plan,
	})
	return []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventToolCallStart, ToolCallID: callID, ToolName: specmode.SubmitToolName},
		{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: callID, ArgumentsFragment: string(args)},
		{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: callID},
		{Type: holtruntime.StreamEventDone},
	}
}

func providerRequestIncludesTool(request holtruntime.CompletionRequest, name string) bool {
...
func providerRequestsContain(requests []holtruntime.CompletionRequest, text string) bool {
...
func TestSpecLaunchesSeedElapsedClock(t *testing.T) {
	...
	next.turnStartedAt = time.Time{} // holt it to prove the impl path re-seeds independently
