...
func (provider *fakeProvider) StreamCompletion(
	...
	ch := make(chan holtruntime.StreamEvent, len(provider.events))
...
func TestPromptSubmitInjectsLiveSessionModelContext(t *testing.T) {
	...
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: "I am using the active session model."},
...
func TestPromptSubmitStoresReasoningSeparatelyFromAnswer(t *testing.T) {
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventReasoning, Content: "private "},
		{Type: holtruntime.StreamEventReasoning, Content: "thought"},
...
func TestParseCommand(t *testing.T) {
	cases := []struct {
		...
		{input: "hello holt", kind: commandPrompt, text: "hello holt"},
	}
...
func TestModelCommandPersistsSelectedModelToUserConfig(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), "holt.json")
...
func TestResumePickerSelectionHydratesSession(t *testing.T) {
	...
	if !transcriptContains(next.transcript, "Resumed Holt session") || !transcriptContains(next.transcript, target.SessionID) {
...
func TestResumeCommandWithUnknownIDReportsMissingSession(t *testing.T) {
	...
	if !transcriptContains(next.transcript, "holt session not found: zero_123") {
...
func TestPromptSubmitAppendsUserAndAssistantRows(t *testing.T) {
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		...
		{Type: holtruntime.StreamEventText, Content: " back"},
		{Type: holtruntime.StreamEventDone},
	}}
	m := newModel(context.Background(), Options{
		Provider:     provider,
...
func TestAgentEventRenderingMappingCoversRuntimeContract(t *testing.T) {
	surfaces := map[holtruntime.AgentEventType]string{
		holtruntime.AgentEventText:       "assistant transcript row",
		holtruntime.AgentEventToolCall:   "tool call transcript row",
		holtruntime.AgentEventToolResult: "tool result transcript row",
		holtruntime.AgentEventThinking:   "deferred: no transcript row until runtime emits thinking deltas",
		holtruntime.AgentEventUsage:      "usage tracker footer segment",
		holtruntime.AgentEventPlanUpdate: "system transcript row from /plan",
		holtruntime.AgentEventError:      "error transcript row",
		holtruntime.AgentEventTurnEnd:    "control boundary, no transcript row",
	}
	for _, eventType := range []holtruntime.AgentEventType{
		holtruntime.AgentEventText,
		holtruntime.AgentEventToolCall,
		holtruntime.AgentEventToolResult,
		holtruntime.AgentEventThinking,
		holtruntime.AgentEventUsage,
		holtruntime.AgentEventPlanUpdate,
		holtruntime.AgentEventError,
		holtruntime.AgentEventTurnEnd,
	} {
	...
	renderedRows := map[holtruntime.AgentEventType]struct {
		row   transcriptRow
		wants []string
	}{
		holtruntime.AgentEventText: {
	...
	m.width = 96
	m, usageRows := m.recordUsageEvent("gpt-4.1", holtruntime.Usage{InputTokens: 100, OutputTokens: 20})
	if len(usageRows) != 0 {
...
func TestSelectionHighlightUsesGutterShiftedCoordinate(t *testing.T) {
	...
	if !strings.Contains(styled, holtTheme.selection.Render("world")) {
...
func TestTranscriptSelectionPaintsHighlightOnceNotTwice(t *testing.T) {
	...
	sentinel := holtTheme.selection.Render("\x00")
