...
func TestEnterWithNoSuggestionStillSubmits(t *testing.T) {
	m := newModel(context.Background(), Options{})
	m.input.SetValue("hello holt") // plain prompt, no suggestions
	...
	if !transcriptContains(next.transcript, "hello holt") {
...
func TestSuggestionsSuppressedDuringSpecReview(t *testing.T) {
	...
	m.pendingSpecReview = &pendingSpecReviewPrompt{SpecID: "spec-1", SpecFilePath: ".holt/specs/spec-1.md"}
