...
func (m model) requestPlanStepExplanation(stepIndex int, step planStep) tea.Cmd {
	...
	return func() tea.Msg {
		...
		defer cancel()
		events, err := provider.StreamCompletion(ctx, holtruntime.CompletionRequest{
			Messages:        holtruntime.SeedMessages(system, user),
			ReasoningEffort: effort,
		})
		if err != nil {
			return planStepExplanationMsg{stepIndex: stepIndex, key: key, gen: gen, err: err}
		}
		collected := holtruntime.CollectStream(ctx, events)
		if collected.Error != "" {
...
// footer clock: seconds under a minute, "Nm Ns" above. A holt or negative span
