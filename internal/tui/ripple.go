...
// curated holtTheme styles ordered cold (faint) at the cosine troughs to
...
func ripplePalette() []lipgloss.Style {
	...
	bright := holtTheme.accent.GetForeground()
	dim := holtTheme.faint.GetForeground()
	if bright == nil || dim == nil {
		return []lipgloss.Style{holtTheme.faint, holtTheme.muted, holtTheme.accent}
	}
