...
type model struct {
	ctx                         context.Context
	cwd                         string
	userCommands                []usercommands.Command // file-sourced /commands (.holt/commands)
	...
	// like a frozen terminal (for ANY provider, not just slow ones). Holt = idle.
	...
	fadeActive         bool
	fadeDisabled       bool // streaming fade off (HOLT_NO_FADE / SSH / tmux / low-color / reduced motion)
	reducedMotion      bool // HOLT_REDUCED_MOTION / no-TTY: static spinner glyph, no fade
	...
	// row. commandPaletteOpen keeps a holt-match command search active so invalid
	...
	pendingImages      []holtruntime.ImageBlock
	...
	captureRunImages func([]holtruntime.ImageBlock)
}
...
type agentUsageMsg struct {
	runID   int
	modelID string
	usage   holtruntime.Usage
}

type agentResponseMsg struct {
	runID         int
	rows          []transcriptRow
	usageEvents   []holtruntime.Usage
	usageModelID  string
...
func newModel(ctx context.Context, options Options) model {
	...
	// Opt-in webhook fan-out (HOLT_NOTIFY_WEBHOOK_URL). Delivery failures stay
	...
	m := model{
		...
		themeMode:                   resolveThemeMode(options.Theme, os.Getenv("HOLT_THEME"), options.SavedTheme),
		hasDarkBg:                   true,
...
const (
	composerPlaceholder     = "describe a task for holt…"
...
func (m model) Init() tea.Cmd {
	...
	// Explicitly requesting it here means Holt doesn't depend solely on the
...
func (m model) updateModel(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case agentTextMsg:
		...
		// When the fade is disabled (HOLT_NO_FADE / SSH / tmux / low-color),
...
func (m model) View() tea.View {
	...
	// Paint the whole frame with the active theme's surface. Holt never paints the
	...
	if m.altScreen {
		view.BackgroundColor = holtTheme.bgPanel
	}
...
func (m model) footerView(width int) string {
	...
	if copyStatus := strings.TrimSpace(m.copyStatus); copyStatus != "" {
		footer.WriteString(rightAlignedLine(holtTheme.ink.Render(copyStatus), width))
...
func (m model) composerIdleHint() string {
	...
	return holtTheme.faint.Render(hint)
}
...
func (m model) jumpToBottomHint() string {
	...
	return holtTheme.faint.Render(fmt.Sprintf("↓ %d more · PgDn", m.chatScrollOffset))
}
...
func scrimViewportLine(line string, width int) string {
	...
	return holtTheme.faint.Render(plain)
}
...
func (m model) workingStatusLine() string {
	...
	line := holtTheme.accent.Render(m.spinnerGlyph()) + " " + working
	...
	line += holtTheme.faint.Render("  ·  " + m.workingActivity())
	if !m.turnStartedAt.IsZero() {
		line += holtTheme.faint.Render("  ·  " + formatWorkingElapsed(m.now().Sub(m.turnStartedAt)))
	}
	...
	line += holtTheme.faint.Render("  ·  " + m.workingTokenIndicator())
	...
	if !m.sidebarActive() {
		if hint := m.quietGenerationHint(); hint != "" {
			line += holtTheme.amber.Render("  ·  " + hint)
		}
	}
...
func (m model) workingPlanLine() string {
	...
	return "  " + holtTheme.faint.Render(text)
}
...
// actually happening and when Holt will act on its own: a heartbeating-but-
...
func (m model) quietGenerationHint() string {
	...
	if idleTimeout := providerio.ResolveStreamIdleTimeout(0); idleTimeout > 0 && quiet >= idleTimeout/2 {
		...
		return fmt.Sprintf("still generating… %s — unusually quiet, Holt will auto-recover by ~%s if it doesn't resume", formatWorkingElapsed(quiet), formatWorkingElapsed(ceiling))
	}
...
func reasoningPreviewLines(reasoning string, width int) []string {
	...
	for _, line := range lines {
		out = append(out, "  "+holtTheme.faint.Render(previewTail(line, avail)))
	}
	return out
}
...
func renderComposerInput(input textinput.Model, state composerState, width int, cursorVisible bool, selection composerSelectionState) string {
	...
	if state.text == "" {
		...
		return fitStyledLine(composerVisualLinePrefix(input, true)+cursor+holtTheme.faint.Render(input.Placeholder), width)
	}
...
func renderComposerVisualLine(input textinput.Model, state composerState, segment composerVisualLine, hasCursor bool, cursorVisible bool, selection composerSelectionState) string {
	...
	textStyle := holtTheme.ink.Inline(true)
	...
	for index := segment.start; index < segment.end; index++ {
		...
		case hasSelection && index >= selectionStart && index < selectionEnd:
			line.WriteString(holtTheme.selection.Render(cell))
		default:
...
func composerVisualLinePrefix(input textinput.Model, first bool) string {
	if first {
		return holtTheme.userPrompt.Render(input.Prompt)
	}
	return "  "
}
...
func commandArgumentHintComposerLine(input textinput.Model, argumentHint string) string {
	...
	return holtTheme.userPrompt.Render(input.Prompt) +
		holtTheme.ink.Inline(true).Render(displayValue) +
		holtTheme.faint.Render(" ") +
		composerCursor(holtTheme.faint.Render(string(hintRunes[0]))) +
		holtTheme.faint.Render(string(hintRunes[1:]))
}

func composerCursor(char string) string {
	return holtTheme.selection.Render(char)
}
...
func (m model) composerBox(width int) string {
	...
	rendered = append(rendered, holtTheme.lineStrong.Render("╭"+strings.Repeat("─", width-2)+"╮"))
	...
	if chips := renderAttachmentChips(m.pendingImageLabels, m.pendingDocuments); chips != "" {
		fitted := fitStyledLine(holtTheme.muted.Render(chips), innerWidth)
		pad := strings.Repeat(" ", maxInt(0, innerWidth-lipgloss.Width(fitted)))
		rendered = append(rendered, holtTheme.lineStrong.Render("│ ")+fitted+pad+holtTheme.lineStrong.Render(" │"))
	}
	for _, line := range lines {
...
func (m model) composerDescriptionHint(width int) string {
	...
	return fitStyledLine(holtTheme.muted.Render(desc), width)
}
...
func (m model) handleSubmit() (tea.Model, tea.Cmd) {
	...
	case commandSelfCorrect:
		...
		return m, nil
	case commandTurns:
		// Changing the budget mid-run would mutate the inherited HOLT_MAX_TURNS env
	...
	case commandUnknown:
		...
		// from .holt/commands/<name>.md — expand its template and run it as a
...
func (m model) launchPrompt(prompt string) (model, tea.Cmd) {
	...
	if m.provider == nil {
		m.transcript = reduceTranscript(m.transcript, transcriptAction{
			...
			text: "No provider configured. Run `holt setup` (guided) or `holt auth` (OAuth) from a shell, or set a provider API key env var, then relaunch.",
		})
		return m, nil
	}
...
func (m model) runAgent(runID int, runCtx context.Context, prompt string, images []holtruntime.ImageBlock) tea.Cmd {
...
func (m model) runAgentWithOptions(runID int, runCtx context.Context, prompt string, images []holtruntime.ImageBlock, runOptions tuiAgentRunOptions) tea.Cmd {
	return func() tea.Msg {
		started := m.now()
		toolCalls := 0
		rows := []transcriptRow{}
		...
		options.OnUsage = func(event holtruntime.Usage) {
...
func (m model) sendAgentUsage(runID int, modelID string, event holtruntime.Usage) {
	if m.runtimeMessageSink == nil {
		return
	}
