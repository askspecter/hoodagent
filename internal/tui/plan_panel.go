// plan_panel.go renders the sticky plan panel for the Holt TUI. The panel
...
func (s *planPanelState) completeRemaining(now time.Time) {
	...
	for i := range s.steps {
		...
		case "completed", "failed":
			...
			// per-step duration doesn't render a holt span.
...
func (m model) renderPlanSummaryLine(width int) string {
	...
	if state.isComplete() {
		return holtTheme.green.Render(fmt.Sprintf("✓ PLAN · %d/%d complete", done, total))
	}
	...
	return holtTheme.accent.Render(label + truncateStep(current, room))
}
...
func renderPlanHeader(state planPanelState, spinnerView string, done, total int, elapsed time.Duration) string {
	...
	if state.isComplete() {
		return holtTheme.green.Render(fmt.Sprintf("✓ PLAN COMPLETE · %d/%d · %s", done, total, formatElapsedSeconds(elapsed)))
	}
	return holtTheme.accent.Render(fmt.Sprintf("%s PLAN · %s · %d/%d · %s", spinnerView, current, done, total, formatElapsedSeconds(elapsed)))
}
...
func renderPlanStepLine(step planStep, now time.Time, maxContent int) string {
	...
	case "failed":
		...
		body = holtTheme.red.Render(content)
		timeStr = formatElapsedSeconds(step.completedAt.Sub(step.startedAt))
	default: // pending
		icon = holtTheme.faint.Render("○")
		body = holtTheme.faint.Render(content)
	}
	line := "  " + icon + " " + body
	if timeStr != "" {
		line += " " + holtTheme.faint.Render(timeStr)
	}
	return line
}
