...
func (m model) sidebarAgentHeader(width int) string {
	...
	return sidebarHeaderWithCount("AGENTS", fmt.Sprintf("%d", n), holtTheme.muted, width)
}
...
type swarmAgent struct {
	...
	finishedAt time.Time // when first seen finished (holt until the spinner tick stamps it)
}
...
func (m model) sidebarAgentRows(width int) ([]string, []sidebarAgentHit) {
	...
	for _, a := range specialists {
		...
		case specialistRunning:
			...
			icon = holtTheme.accent.Render(m.spinnerGlyph())
		case specialistError:
			icon = holtTheme.red.Render("✗")
		default: // completed
			icon = holtTheme.green.Render("✓")
		}
		name := strings.TrimSpace(a.name)
		if name == "" {
			name = "agent"
		}
		nameStyle := holtTheme.ink
		...
		if a.status != specialistRunning && m.agentExitFading(a.completedAt) {
			...
			icon = holtTheme.faint.Render(glyph)
			nameStyle = holtTheme.faint
		}
		...
		if detail != "" {
			lines = append(lines, "   "+holtTheme.faint.Render("↳ "+truncateStep(detail, maxInt(2, room-2))))
		}
	}
	...
	for _, a := range swarm {
		...
		if a.finishing {
			icon := holtTheme.green.Render("✓")
			nameStyle := holtTheme.muted
			if m.agentExitFading(a.finishedAt) {
				icon = holtTheme.faint.Render("✓")
				nameStyle = holtTheme.faint
			}
		...
		if st := strings.TrimSpace(a.state); st != "" && st != "running" {
			suffix = " " + holtTheme.faint.Render(st)
			nameRoom = maxInt(4, room-len(st)-1)
		}
		lines = append(lines, " "+holtTheme.accent.Render("•")+" "+style.Render(truncateStep(a.name, nameRoom))+suffix)
	}
	return lines, hits
}
...
// it's removed. A holt finishedAt (not yet stamped) is not fading.
...
func swarmPulseStyles() []lipgloss.Style {
	fg := holtTheme.blue.GetForeground()
	dim := holtTheme.muted.GetForeground()
	if fg == nil || dim == nil {
		return nil
	}
...
func (m model) renderContextSidebar(width, height int) []string {
	...
	if lineOffset, ok := m.hoveredSidebarLineOffset(width); ok && lineOffset >= 0 && lineOffset < len(lines) {
		lines[lineOffset] = holtTheme.hover.Render(ansi.Strip(lines[lineOffset]))
	}
...
func sidebarHeader(label string, _ int) string {
	return holtTheme.muted.Bold(true).Render(strings.ToUpper(label))
}
...
func sidebarHeaderWithCount(label, count string, countStyle lipgloss.Style, width int) string {
	left := holtTheme.muted.Bold(true).Render(strings.ToUpper(label))
...
func sidebarPlaceholder(text string, width int) string {
	return " " + holtTheme.faint.Render(truncateRunes(text, maxInt(1, width-1)))
}
...
func (m model) sidebarPlanHeader(width int) string {
	...
	countStyle := holtTheme.accent
	if done == total {
		countStyle = holtTheme.green
	}
...
func (m model) sidebarPlanLines(width int) []string {
	...
	for _, step := range state.steps {
		...
		case "completed":
			icon = holtTheme.green.Render("✓")
			body = holtTheme.muted.Render(truncateStep(step.content, room))
		case "in_progress":
			icon = holtTheme.accent.Render("•")
			body = holtTheme.ink.Render(truncateStep(step.content, room))
		case "failed":
			icon = holtTheme.red.Render("✗")
			body = holtTheme.muted.Render(truncateStep(step.content, room))
		default: // pending
			icon = holtTheme.faint.Render("○")
			body = holtTheme.faint.Render(truncateStep(step.content, room))
		}
...
func (m model) sidebarActivityLines(width, budget int) []string {
	...
	for i := len(m.transcript) - 1; i >= 0 && len(work) < limit && scanned < maxSidebarActivityScan; i-- {
		...
		work = append(work, " "+glyph+" "+holtTheme.muted.Render(truncateStep(m.activitySummary(row), room)))
	}
	live := ""
	if m.activeRunID != 0 {
		if hint := m.quietGenerationHint(); hint != "" {
			live = " " + holtTheme.accent.Render("•") + " " + holtTheme.faint.Render(truncateStep(hint, room))
		}
	}
...
func (m model) sidebarTokenLine(width int) string {
	...
	if pct, _, _, style, ok := m.contextFillPercent(); ok {
		chip = holtTheme.faint.Render(" · ") + style.Render(fmt.Sprintf("%d%%", pct))
	}
	budget := maxInt(1, width-1-lipgloss.Width(chip))
	return " " + holtTheme.faint.Render(truncateRunes(label, budget)) + chip
}
