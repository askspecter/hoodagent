...
func renderFocusedSpecReviewPrompt(review pendingSpecReviewPrompt, width int) string {
	fill := holtTheme.onPanel
	actions := holtTheme.badge.Render(" [a] approve ") +
		fill(holtTheme.ink).Render(" ") +
		fill(holtTheme.red).Render("[r]") + fill(holtTheme.ink).Render(" reject ") +
		fill(holtTheme.accent).Render("[e]") + fill(holtTheme.ink).Render(" edit file ") +
		fill(holtTheme.faint).Render("[esc] cancel")
	lines := []string{
		holtTheme.badge.Render(" SPEC REVIEW "),
		fill(holtTheme.faint).Render("path: ") + fill(holtTheme.ink).Render(reviewDisplayPath(review)),
		actions,
	}
	return styledBlockFill(width, lines, holtTheme.line, holtTheme.panel)
}
