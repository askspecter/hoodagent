...
func TestDoctorOptionsIncludeConfigPaths(t *testing.T) {
	m := newModel(context.Background(), Options{
		UserConfigPath:    "C:/holt/user.json",
		ProjectConfigPath: "C:/repo/.holt/config.json",
	...
	if options.UserConfig != "C:/holt/user.json" {
		t.Fatalf("UserConfig = %q, want %q", options.UserConfig, "C:/holt/user.json")
	}
	if options.ProjectConfig != "C:/repo/.holt/config.json" {
		t.Fatalf("ProjectConfig = %q, want %q", options.ProjectConfig, "C:/repo/.holt/config.json")
	}
...
func TestDoctorOptionsConnectivityInvokesConfiguredProviderHealthProbe(t *testing.T) {
	...
	if gotOptions.UserAgent != "holt-test" {
		t.Fatalf("probe UserAgent = %q, want %q", gotOptions.UserAgent, "holt-test")
	}
...
func TestDoctorFixLinesUseSandboxRemedy(t *testing.T) {
	...
	text := strings.Join(lines, "\n")
	if !strings.Contains(text, "holt sandbox setup") {
