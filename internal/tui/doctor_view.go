...
func doctorCommandOutput(report doctor.Report, backend *holtcommands.BackendLifecycleSnapshot) commandOutput {
...
func doctorBackendSection(backend holtcommands.BackendLifecycleSnapshot) commandSection {
...
func doctorActions(checks []doctor.Check, backend *holtcommands.BackendLifecycleSnapshot) []string {
	...
	for _, check := range checks {
		...
		case "sandbox.backend":
			if remedy := doctorCheckDetailString(check, "remedy"); remedy != "" {
				...
				add("holt sandbox policy --effective - inspect sandbox backend and enforcement status")
			}
		case "lsp.servers":
