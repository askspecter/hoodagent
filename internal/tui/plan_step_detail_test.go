...
func TestPlanStepDetailAIExplanation(t *testing.T) {
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: "We added the new button and wired up its click."},
...
func TestPlanStepDetailSecondClickHides(t *testing.T) {
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: "late explanation"},
		{Type: holtruntime.StreamEventDone},
	}}
	t0 := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	m := model{now: func() time.Time { return t0 }, provider: provider}
...
func TestPlanStepDetailToggle(t *testing.T) {
	...
	if len(m.transcript) != base {
		t.Errorf("re-click should net holt growth: got %d, base %d", len(m.transcript), base)
	}
