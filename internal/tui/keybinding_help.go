// keybinding_help.go renders the `?` keyboard-shortcut overlay. Holt has a
...
func (m model) renderKeybindingHelpLines(innerWidth int) []string {
	...
	for index, group := range keybindingGroups {
		...
		lines = append(lines, holtTheme.accent.Render(group.title))
		for _, binding := range group.bindings {
			lines = append(lines, formatKeybindingLine(binding, keyColumn, innerWidth))
		}
	}
	lines = append(lines, "")
	lines = append(lines, holtTheme.faint.Render(keybindingHelpFooter))
	return lines
}
...
func formatKeybindingLine(binding keybinding, keyColumn int, innerWidth int) string {
	...
	keyCell := holtTheme.ink.Render(keys) + strings.Repeat(" ", pad)
	// Indent(2) + keyCell + gap(2) consumed before the description.
	descBudget := innerWidth - 2 - keyColumn - 2
	if descBudget < 4 {
		descBudget = 4
	}
	desc := holtTheme.muted.Render(truncateRunes(binding.desc, descBudget))
	return "  " + keyCell + "  " + desc
}
...
func (m model) renderKeybindingHelpOverlay(width int) string {
	...
	block := styledBlockFillTitle(overlayWidth, "Keyboard Shortcuts", lines, holtTheme.line, holtTheme.panel)
