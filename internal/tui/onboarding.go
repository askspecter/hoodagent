...
func (m model) completeSetup() (tea.Model, tea.Cmd) {
	...
	if result.Provider.Name != "" {
		...
		// Export HOLT_PROVIDER alongside the committed profile fields (and the
		...
		// them unconditionally too: applyEnv makes HOLT_PROVIDER WIN over config,
...
func (m model) setupView(width int) string {
	...
	if m.setup.err != "" {
		content = append(content, "", holtTheme.red.Render("error: "+m.setup.err))
	}
...
func (m model) setupStageLines(width int, height int) []string {
	...
	case setupStageSafety:
		return []string{
			holtTheme.ink.Bold(true).Render("Safety"),
			"",
			"Holt asks before running shell commands or changing files.",
			"Unsafe mode stays off unless you explicitly enable it.",
			"",
			holtTheme.faint.Render("Default: ask before risky work."),
		}
	case setupStageReady:
		return m.setupReadyLines(width)
	default:
		return []string{
			holtTheme.accent.Render("Welcome to Holt"),
			"",
			holtTheme.ink.Render("A terminal agent for changing real code."),
			holtTheme.faint.Render("Plan changes, edit with approval, run checks, and resume sessions."),
		}
	}
}
...
func (m model) setupReadyLines(width int) []string {
	...
	lines := []string{
		holtTheme.ink.Bold(true).Render("Ready"),
		"",
		"Holt will save this setup and open chat.",
		"",
	}
	...
	for _, row := range rows {
		...
		line := "  " + holtTheme.faint.Render(label) + "  " + holtTheme.ink.Render(row.value)
		lines = append(lines, padSetupLine(line, rowWidth))
	}

	lines = append(lines,
		"",
		holtTheme.faint.Render("Later, use /provider, /doctor, or /help anytime."),
	)
	return lines
}
...
func (m model) setupModelLines(width int, height int) []string {
	...
	start := selectableListStart(len(models), maxVisible, m.setup.modelIndex)
	lines := []string{
		padSetupLine("  "+holtTheme.ink.Bold(true).Render("Choose a model"), rowWidth),
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+m.setupModelSearchLine(rowWidth-2), rowWidth),
	}
	if status := m.setupModelStatus(); status != "" {
		lines = append(lines, padSetupLine("  "+holtTheme.faint.Render(status), rowWidth))
	}
	lines = append(lines, blankSetupBlockLine(rowWidth))
	if len(models) == 0 {
		lines = append(lines, padSetupLine("  "+holtTheme.faint.Render("No matching models"), rowWidth))
		return lines
	}
	...
	lines = append(lines,
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+holtTheme.faint.Render(detail), rowWidth),
	)
	return lines
}
...
func (m model) setupTypedModelLines(width int) []string {
	...
	return []string{
		padSetupLine("  "+holtTheme.ink.Bold(true).Render("Choose a model"), rowWidth),
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+holtTheme.ink.Render("Enter the model ID this endpoint expects."), rowWidth),
		padSetupLine("  "+holtTheme.faint.Render("Examples: gpt-4.1, claude-sonnet-4-5, llama-3.3-70b"), rowWidth),
		blankSetupBlockLine(rowWidth),
...
func (m model) setupModelLoadingLines(width int) []string {
	...
	return []string{
		padSetupLine("  "+holtTheme.ink.Bold(true).Render("Choose a model"), rowWidth),
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+holtTheme.faint.Render("Checking available models..."), rowWidth),
		padSetupLine("  "+holtTheme.faint.Render("Built-in models will be used if discovery fails."), rowWidth),
	}
}
...
func (m model) setupModelSearchLine(width int) string {
	...
	if query == "" {
		return fitStyledLine(prompt+cursor+holtTheme.faint.Render("model name..."), width)
	}
	return fitStyledLine(prompt+holtTheme.ink.Render(query)+cursor, width)
}
...
func (m model) setupModelRow(width int, index int, model providerWizardModel) string {
	...
	if selected {
		marker = "❯ "
		style = holtTheme.accent.Bold(true)
	}
...
func (m model) setupMethodLines(width int) []string {
	...
	lines := []string{
		padSetupLine("  "+holtTheme.ink.Bold(true).Render("How do you want to connect?"), rowWidth),
		blankSetupBlockLine(rowWidth),
	}
	for index, option := range options {
		...
		lines = append(lines, padSetupLine("    "+holtTheme.faint.Render(option.subtitle), rowWidth))
	}
	return lines
}
...
func (m model) setupOAuthWaitingLines(width int) []string {
	...
	if m.setup.oauthDevice {
		lines = []string{holtTheme.ink.Bold(true).Render("Device-code sign-in for " + name), ""}
		if m.setup.deviceUserCode == "" {
			lines = append(lines, holtTheme.faint.Render("Requesting a device code..."))
		} else {
			lines = append(lines,
				"1. On any device, visit:  "+holtTheme.accent.Render(m.setup.deviceVerificationURI),
				"2. Enter the code:  "+holtTheme.accent.Bold(true).Render(m.setup.deviceUserCode),
				"",
				holtTheme.faint.Render("Waiting for authorization..."),
			)
		}
	} else {
		lines = []string{
			holtTheme.ink.Bold(true).Render("Signing in with " + name),
			"",
			"Opening your browser — approve there, then return here.",
			holtTheme.faint.Render("Waiting for authorization..."),
			"",
			holtTheme.faint.Render("If your browser didn't open, run:  " + providerWizardOAuthCLIHint(provider)),
		}
	}
	if m.setup.oauthErr != "" {
		lines = append(lines, "", holtTheme.red.Render("Sign-in failed: "+m.setup.oauthErr))
	}
	return lines
}
...
func (m model) setupProviderLines(width int, height int) []string {
	...
	lines := []string{
		padSetupLine("  "+holtTheme.ink.Bold(true).Render("Choose a provider"), rowWidth),
		blankSetupBlockLine(rowWidth),
	}
	for index, option := range visibleProviders {
		...
		if option.Recommended {
			line += holtTheme.faint.Render("  (recommended)")
		}
		lines = append(lines, padSetupLine(line, rowWidth))
	}
	...
	if m.setup.oauthMode && m.setup.oauthErr != "" {
		lines = append(lines,
			blankSetupBlockLine(rowWidth),
			padSetupLine("  "+holtTheme.red.Render("Sign-in failed: "+m.setup.oauthErr), rowWidth),
		)
	}
	return lines
}
...
func (m model) setupEndpointLines(width int) []string {
	...
	return []string{
		padSetupLine("  "+holtTheme.ink.Bold(true).Render("Endpoint URL"), rowWidth),
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+holtTheme.ink.Render("Enter the API base URL for "+displayValue(option.Name, option.ID)+"."), rowWidth),
		padSetupLine("  "+holtTheme.faint.Render(providerWizardEndpointHint(provider)), rowWidth),
		blankSetupBlockLine(rowWidth),
...
func (m model) setupNameLines(width int) []string {
	...
	return []string{
		padSetupLine("  "+holtTheme.ink.Bold(true).Render("Provider name"), rowWidth),
		blankSetupBlockLine(rowWidth),
		padSetupLine("  "+holtTheme.ink.Render("Choose the short label shown in Holt."), rowWidth),
		padSetupLine("  "+holtTheme.faint.Render("Leave blank to use "+name+"."), rowWidth),
		blankSetupBlockLine(rowWidth),
...
func (m model) setupCredentialLines(width int) []string {
	option := m.setupProvider()
	lines := []string{
		holtTheme.ink.Bold(true).Render("Credentials"),
		"",
	}
	...
	lines = append(lines,
		...
		"",
		holtTheme.faint.Render("Saved keys stay in your user config."),
		holtTheme.faint.Render("Blank uses "+envVar+" from your shell."),
	)
	return lines
}
...
func (m model) setupAPIKeyInputLine(width int) string {
	...
	if strings.TrimSpace(input.Value()) == "" {
		return holtTheme.faint.Render(input.Placeholder)
	}
...
func (m model) setupFooter() string {
	if m.setup.oauthPending {
		return holtTheme.faint.Render("Esc cancel")
	}
	switch m.setup.stage {
	case setupStageMethod:
		return holtTheme.faint.Render("↑/↓ choose   ") + holtTheme.accent.Render("Enter") + holtTheme.faint.Render(" continue   q quit")
	case setupStageReady:
		return holtTheme.accent.Render("Enter") + holtTheme.faint.Render(" to save and start chat")
	case setupStageEndpoint:
		return holtTheme.accent.Render("Enter") + holtTheme.faint.Render(" continue   left back")
	case setupStageName:
		return holtTheme.faint.Render("name optional   ") + holtTheme.accent.Render("Enter") + holtTheme.faint.Render(" continue   left back")
	case setupStageCredentials:
		if m.setupCredentialInputActive() {
			return holtTheme.faint.Render("paste key optional   ") + holtTheme.accent.Render("Enter") + holtTheme.faint.Render(" continue   left back")
		}
		return holtTheme.accent.Render("Space") + holtTheme.faint.Render(" to continue")
	case setupStageProvider:
		if m.setup.oauthMode && m.setupProviderDescriptor().OAuthDeviceFlow {
			return holtTheme.faint.Render("↑/↓ choose   ") + holtTheme.accent.Render("Enter") + holtTheme.faint.Render(" sign in   ") + holtTheme.accent.Render("d") + holtTheme.faint.Render(" device code   q quit")
		}
		return holtTheme.faint.Render("↑/↓ choose   ") + holtTheme.accent.Render("Enter") + holtTheme.faint.Render(" continue   q quit")
	case setupStageModel:
		if m.setup.modelLoad {
			return holtTheme.faint.Render("checking models...")
		}
		return holtTheme.faint.Render("↑/↓ choose   type search   ") + holtTheme.accent.Render("Enter") + holtTheme.faint.Render(" continue")
	case setupStageWelcome:
		return holtTheme.accent.Render("Space") + holtTheme.faint.Render(" to set up Holt")
	default:
		return holtTheme.accent.Render("Space") + holtTheme.faint.Render(" to continue")
	}
}
...
func (m model) setupProgressText() string {
	...
	return holtTheme.faint.Render(fmt.Sprintf("%d/%d", position+1, len(stages)))
}
