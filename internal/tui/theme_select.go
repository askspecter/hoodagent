...
// HOLT_THEME, then the persisted config theme. A value is accepted if it is `auto`
...
// ValidThemeArg reports whether s is an acceptable --theme / HOLT_THEME value
...
// applyTheme swaps the active palette (holtTheme) and the globals derived from it
...
// program starts), like every other holtTheme access.
func applyTheme(mode themeMode, hasDarkBackground bool) themeMode {
	...
	// falls back to the dark built-in so a bad value can never leave holtTheme unset.
	entry, ok := lookupTheme(string(resolved))
	if !ok {
		entry, _ = lookupTheme(string(themeDark))
	}
	holtTheme = buildTheme(entry.Palette)
...
// and never strands on a stale preview. It only swaps the global holtTheme via
