...
func (m model) detailedTranscriptView() string {
	...
	builder.WriteString("\n")
	builder.WriteString(holtTheme.line.Render(strings.Repeat("-", width)))
	builder.WriteString("\n")
	...
	for _, row := range m.transcript {
		...
		if row.kind == rowWelcome {
			rendered = fitStyledLine(holtTheme.faint.Render(row.text), width)
		} else {
	...
	if !shownAny {
		builder.WriteString(holtTheme.faint.Render("No transcript rows."))
		builder.WriteString("\n")
	}

	builder.WriteString(holtTheme.line.Render(strings.Repeat("-", width)))
	builder.WriteString("\n")
	builder.WriteString(fitStyledLine(holtTheme.faint.Render("Esc close | Ctrl+O toggle"), width))
	return builder.String()
}

func detailedTranscriptHeader(width int) string {
	title := holtTheme.ink.Bold(true).Render("Transcript")
	hint := holtTheme.faint.Render("detailed")
