...
func TestAltScreenTranscriptScrollKeepsFooterFixed(t *testing.T) {
	...
	if !strings.Contains(bottom, "describe a task for holt") || !strings.Contains(bottom, "openai") {
	...
	if !strings.Contains(scrolled, "describe a task for holt") || !strings.Contains(scrolled, "openai") {
