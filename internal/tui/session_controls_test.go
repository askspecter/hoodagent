...
func TestCompactCommandUsesProviderSummaryWhenAvailable(t *testing.T) {
	...
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: "Provider summary keeps the actual old decisions."},
...
func TestUsageEventsUpdateFooterAndContext(t *testing.T) {
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		...
		{Type: holtruntime.StreamEventUsage, Usage: holtruntime.Usage{InputTokens: 100, CachedInputTokens: 25, OutputTokens: 20}},
		{Type: holtruntime.StreamEventDone},
	}}
	m := newModel(context.Background(), Options{
		ModelName:    "gpt-4.1",
...
func TestUsageRuntimeMessageUpdatesFooterBeforeFinalResponse(t *testing.T) {
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventUsage, Usage: holtruntime.Usage{InputTokens: 10, OutputTokens: 5}},
		{Type: holtruntime.StreamEventText, Content: "done"},
		{Type: holtruntime.StreamEventDone},
	}}
...
func TestUsageEventsForCustomModelUseTokenOnlyFallback(t *testing.T) {
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: "done"},
		{Type: holtruntime.StreamEventUsage, Usage: holtruntime.Usage{InputTokens: 100, OutputTokens: 20}},
		{Type: holtruntime.StreamEventDone},
	}}
	m := newModel(context.Background(), Options{
...
func TestUnpricedUsageStatusUsesLatestEventNotCumulative(t *testing.T) {
	...
	m, rows = m.recordUsageEvent("custom-coder", holtruntime.Usage{InputTokens: 100, OutputTokens: 20})
	if len(rows) != 0 {
		t.Fatalf("unpriced usage should not append transcript rows, got %#v", rows)
	}
	m, rows = m.recordUsageEvent("custom-coder", holtruntime.Usage{InputTokens: 10, OutputTokens: 5})
	if len(rows) != 0 {
		t.Fatalf("unpriced usage should not append transcript rows, got %#v", rows)
	}
...
func TestStatusLineDropsTokenFigureWhenSidebarShowsIt(t *testing.T) {
	m := sidebarTestModel()
	m, _ = m.recordUsageEvent("test-model", holtruntime.Usage{InputTokens: 100, OutputTokens: 20})
	if !m.sidebarActive() {
...
func TestInvalidUsageEventsAppendTranscriptError(t *testing.T) {
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: "done"},
		{Type: holtruntime.StreamEventUsage, Usage: holtruntime.Usage{InputTokens: -1, OutputTokens: 20}},
		{Type: holtruntime.StreamEventDone},
	}}
	m := newModel(context.Background(), Options{
		ModelName:    "gpt-4.1",
...
func TestStaleAgentUsageResponseIsIgnored(t *testing.T) {
	...
	updated, _ := m.Update(agentResponseMsg{
		...
		usageEvents:  []holtruntime.Usage{{InputTokens: 100, OutputTokens: 20}},
	})
	next := updated.(model)
...
func TestModelSwitchUnknownModelReportsError(t *testing.T) {
	...
	if !transcriptContains(next.transcript, "unknown Holt model") {
