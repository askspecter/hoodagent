...
// holtWordmarkPrefixLines is the white `H` part of the empty-state ANSI
// Shadow-style wordmark. holtWordmarkOLines keeps the lime `O` glyph, and
// holtWordmarkSuffixLines renders the white `LT` that follows it.
var holtWordmarkPrefixLines = []string{
	`██╗  ██╗`,
	`██║  ██║`,
	`███████║`,
	`██╔══██║`,
...
var holtWordmarkSuffixLines = []string{
	`██╗     ████████╗`,
	`██║     ╚══██╔══╝`,
	`██║        ██║   `,
	`██║        ██║   `,
...
const emptyStateTagline = "Any model. Every tool. No limits."
...
func (m model) emptyStateLines(width int) []string {
	lines := []string{}
	for _, glyph := range holtWordmarkLines() {
		lines = append(lines, centerLine(glyph, width))
	}
	lines = append(lines, "")
	lines = append(lines, centerLine(holtTheme.muted.Render(emptyStateTagline), width))
	// Orientation: where HOLT is pointed (cwd · branch · model) so a returning user
	...
	lines = append(lines, "")
	lines = append(lines, centerLine(holtTheme.faint.Render(emptyStateExamples), width))
	lines = append(lines, "")
	lines = append(lines, centerLine(holtTheme.faint.Render("Press ? for keyboard shortcuts · / for commands"), width))
...
func (m model) emptyStateOrientation() string {
	...
	return holtTheme.faint.Render(strings.Join(parts, "  ·  "))
}

func holtWordmarkLines() []string {
	rows := minInt(minInt(len(holtWordmarkPrefixLines), len(holtWordmarkOLines)), len(holtWordmarkSuffixLines))
	lines := make([]string, 0, rows)
	for index := 0; index < rows; index++ {
		lines = append(lines, holtTheme.ink.Render(holtWordmarkPrefixLines[index])+
			holtTheme.accent.Render(holtWordmarkOLines[index])+
			holtTheme.ink.Render(holtWordmarkSuffixLines[index]))
	}
	return lines
}

func borderedBlock(width int, lines []string) string {
	return styledBlock(width, lines, holtTheme.line)
}
