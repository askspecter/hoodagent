...
func (m model) streamingToolCallView(width int) string {
	...
	head := toolCardHead(m.streamCallName, d.path, "", headTag, "", "", true, holtTheme.ink, false, width, cardRenderOptions{cwd: m.cwd})
	lines := []string{holtTheme.accent.Render(m.spinnerGlyph()) + " " + head}
