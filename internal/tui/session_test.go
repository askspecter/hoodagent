...
type scriptedProvider struct {
	scripts  [][]holtruntime.StreamEvent
...
func (provider *scriptedProvider) StreamCompletion(
	...
	if len(provider.scripts) == 0 {
		ch := make(chan holtruntime.StreamEvent)
		close(ch)
		return ch, nil
	}
	...
	ch := make(chan holtruntime.StreamEvent, len(provider.scripts[index]))
...
func TestPromptSubmitPersistsTUISessionEvents(t *testing.T) {
	...
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: "saved"},
		{Type: holtruntime.StreamEventUsage, Usage: holtruntime.Usage{InputTokens: 10, OutputTokens: 4}},
...
func TestPromptSubmitPersistsToolSessionEvents(t *testing.T) {
	...
	provider := &scriptedProvider{scripts: [][]holtruntime.StreamEvent{
		{
			{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call_1", ToolName: "read_file"},
			{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call_1", ArgumentsFragment: `{"path":"notes.txt"}`},
			{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call_1"},
			{Type: holtruntime.StreamEventDone},
		},
		{
			{Type: holtruntime.StreamEventText, Content: "read complete"},
			{Type: holtruntime.StreamEventDone},
		},
	}}
	registry := tools.NewRegistry()
...
func TestPromptSubmitPersistsPermissionSessionEvents(t *testing.T) {
	...
	provider := &scriptedProvider{scripts: [][]holtruntime.StreamEvent{
		{
			{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call_write", ToolName: "write_file"},
			{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call_write", ArgumentsFragment: `{"path":"notes.txt","content":"hello"}`},
			{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call_write"},
			{Type: holtruntime.StreamEventDone},
		},
		{
			{Type: holtruntime.StreamEventText, Content: "write blocked"},
			{Type: holtruntime.StreamEventDone},
		},
	}}
	registry := tools.NewRegistry()
...
func newPermissionTestModel(root string, provider holtruntime.Provider, registry *tools.Registry, store *sessions.Store, grantStore *sandbox.GrantStore, runtimeMessages chan<- tea.Msg) model {
...
func writeFileToolScript(callID string, path string, content string) []holtruntime.StreamEvent {
	return []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventToolCallStart, ToolCallID: callID, ToolName: "write_file"},
		{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: callID, ArgumentsFragment: `{"path":` + jsonString(path) + `,"content":` + jsonString(content) + `,"overwrite":true}`},
		{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: callID},
		{Type: holtruntime.StreamEventDone},
	}
}

func textScript(text string) []holtruntime.StreamEvent {
	return []holtruntime.StreamEvent{
...
func TestResumeCommandHydratesSessionTranscript(t *testing.T) {
	...
	appendTestEvent(t, store, session.SessionID, sessions.EventToolCall, map[string]any{"id": "call_1", "name": "grep", "arguments": `{"pattern":"Holt"}`})
	...
	for _, want := range []string{"Resumed Holt session", session.SessionID, "previous request", "tool call: grep", "permission: grep allow", "tool result: grep error matches", "previous answer", "old error"} {
		if !transcriptContains(next.transcript, want) {
	...
	if !ok || toolCall.tool != "grep" || toolCall.detail != "Holt" {
	...
	if transcriptContains(next.transcript, "holt exec --resume") {
...
func TestEscCancelRecordsSessionError(t *testing.T) {
	store := testSessionStore(t)
	provider := &fakeProvider{events: []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: "ignored"},
		{Type: holtruntime.StreamEventDone},
	}}
	m := newModel(context.Background(), Options{
		Provider:     provider,
...
func TestResumedPromptIncludesSessionContext(t *testing.T) {
	...
	appendTestEvent(t, store, session.SessionID, sessions.EventMessage, map[string]any{"role": "assistant", "content": "previous answer"})
	provider := &scriptedProvider{scripts: [][]holtruntime.StreamEvent{{
		{Type: holtruntime.StreamEventText, Content: "continued"},
		{Type: holtruntime.StreamEventDone},
	}}}
	m := newModel(context.Background(), Options{
		Provider:     provider,
	...
	for _, want := range []string{"Continuing Holt session", session.SessionID, "previous request", "previous answer", "Current user request:", "continue"} {
...
func TestResumeCommandReportsMissingSession(t *testing.T) {
	...
	if !transcriptContains(next.transcript, "holt session not found: missing_session") {
