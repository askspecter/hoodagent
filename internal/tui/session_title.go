...
func generateSessionTitle(ctx context.Context, provider holtruntime.Provider, digest string) (string, error) {
	if provider == nil {
	...
	request := holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{Role: holtruntime.MessageRoleSystem, Content: sessionTitleSystemPrompt},
			{Role: holtruntime.MessageRoleUser, Content: "Conversation:\n\n" + digest + "\n\nTitle:"},
		},
	}
