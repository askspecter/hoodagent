...
// resolveThemeMode precedence: explicit flag > HOLT_THEME env > auto.
...
func TestApplyThemeResolution(t *testing.T) {
	...
	for _, c := range cases {
		...
		gotR, gotG, gotB, _ := holtTheme.inkColor.RGBA()
		if gotR != wantR || gotG != wantG || gotB != wantB {
			t.Errorf("applyTheme(%q,%v): holtTheme.inkColor not the %q ink", c.mode, c.darkBg, c.want)
		}
	}
}
