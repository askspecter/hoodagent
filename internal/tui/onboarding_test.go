...
func TestSetupTakeoverRendersAndCompletes(t *testing.T) {
	...
	m := newModel(context.Background(), Options{
		...
		Setup: SetupOptions{
			Visible:    true,
			Required:   true,
			ConfigPath: "/tmp/holt/config.json",
	...
	if view := plainRender(t, m.View()); !strings.Contains(view, "Welcome to Holt") || !strings.Contains(view, "Space to set up Holt") || !strings.Contains(view, "terminal agent for changing real code") {
...
// Completing setup switches the live provider, so it must export HOLT_PROVIDER
