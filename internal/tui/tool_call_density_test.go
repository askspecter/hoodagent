...
func TestDisplayPathAbbreviatesHome(t *testing.T) {
	...
	abs := filepath.Join(home, "projects", "holt", "main.go")
	if got := displayPath(t.TempDir(), abs); got != "~/projects/holt/main.go" {
		t.Fatalf("displayPath under home = %q, want ~/projects/holt/main.go", got)
	}
}
...
func TestDisplayPathLeavesRelativeInputUntouched(t *testing.T) {
	if got := displayPath("/work/holt", "examples/calc.go"); got != "examples/calc.go" {
...
func TestRedundantConfirmationCollapsesSuccessBody(t *testing.T) {
	...
	row := transcriptRow{
		...
		detail: "Successfully created directory /work/holt/examples/calc",
	}
