...
func (m model) titleBar(width int) string {
	...
	cwdOnly := holtTheme.faint.Render(shortenPath(m.cwd))
	compactLeft := cwdOnly
	if branchOnly != "" {
	...
	if window := m.modelContextWindow(m.modelName); window > 0 {
		ctx = holtTheme.faint.Render(" · " + formatContextWindow(window))
	}
	...
	rule := holtTheme.line.Render(strings.Repeat("─", width))
	return line + "\n" + rule
}

func (m model) titleWorkspaceSegment() string {
	cwd := holtTheme.faint.Render(shortenPath(m.cwd))
	parts := []string{}
...
func (m model) titleWorkspaceSegmentShort() string {
	...
	if branch != "" {
		icon := holtTheme.muted.Render("")
		parts = append(parts, icon+" "+holtTheme.muted.Render(middleTruncate(branch, 22)))
	}
	if pr := m.titlePRSegment(); pr != "" {
		parts = append(parts, pr)
	}
...
func (m model) titleBranchSegment() string {
	...
	return holtTheme.muted.Render("") + " " + holtTheme.muted.Render(branch)
}
...
func (m model) titleModelSegment() string {
	...
	switch {
	case provider == "" && model == "":
		return holtTheme.muted.Render("no provider")
	case model == "":
		return holtTheme.ink.Render(provider)
	case provider == "":
		return holtTheme.ink.Render(model)
	default:
		return holtTheme.ink.Render(provider + "/" + model)
	}
}
...
func (m model) composerDividerLine(width int) string {
	...
	meta := holtTheme.muted.Render(model)
	metaWidth := lipgloss.Width(meta)
	if width < 8 {
		return holtTheme.lineStrong.Render(strings.Repeat("─", width))
	}
	if width < metaWidth+4 {
		return holtTheme.lineStrong.Render("╰" + strings.Repeat("─", width-2) + "╯")
	}
	rule := strings.Repeat("─", width-metaWidth-4)
	return holtTheme.lineStrong.Render("╰"+rule+" ") + meta + holtTheme.lineStrong.Render(" ╯")
}
...
func (m model) statusLine(width int) string {
	tier := widthTier(width)
	separator := holtTheme.line.Render(" │ ")
	prefix := "  "
	...
	left := prefix + holtTheme.accent.Render("●") + " " + modeStyle.Render(modeText)

	if tier == tierTiny {
		if m.exitConfirmActive {
			return fitStyledLine(prefix+holtTheme.amber.Render("●")+" "+holtTheme.amber.Render(ctrlCExitConfirmText), width)
		}
		if m.cancelConfirmActive {
			return fitStyledLine(prefix+holtTheme.amber.Render("●")+" "+holtTheme.amber.Render(escCancelConfirmText), width)
		}
	...
	if m.reasoningEffort != "" {
		left += holtTheme.muted.Render(" · ") + holtTheme.accent.Render(string(m.reasoningEffort))
	}
	if m.exitConfirmActive {
		left = prefix + holtTheme.amber.Render("●") + " " + holtTheme.amber.Render(ctrlCExitConfirmText)
	} else if m.cancelConfirmActive {
		left = prefix + holtTheme.amber.Render("●") + " " + holtTheme.amber.Render(escCancelConfirmText)
	} else if summary := m.backgroundTerminalSummary(); summary != "" {
		left += separator + holtTheme.muted.Render(summary)
	}
	...
	if usage != "" {
		rightGroups = append(rightGroups, holtTheme.muted.Render(usage))
	}
...
func (m model) modeLabel() (string, lipgloss.Style) {
	...
	case agent.PermissionModeAuto:
		return "auto-approve", holtTheme.modeAuto
	case agent.PermissionModeAsk:
		return "ask", holtTheme.modeAsk
	case agent.PermissionModeUnsafe:
		return "unsafe", holtTheme.modeUnsafe
	default:
		mode := strings.TrimSpace(string(m.permissionMode))
		if mode == "" {
			return "auto-approve", holtTheme.modeAuto
		}
		return mode, holtTheme.muted
	}
}
...
func (m model) contextFillPercent() (pct, used, window int, style lipgloss.Style, ok bool) {
	...
	style = holtTheme.green
	switch {
	case ratio >= 0.90:
		style = holtTheme.red
	case ratio >= 0.75:
		style = holtTheme.amber
	}
...
func renderSuggestionPalette(items []selectableListItem, selected, width int, title, query, footer string) string {
	...
	searchPrefix := transparentSurface(holtTheme.ink).Render(strings.Repeat(" ", searchInset))
	...
	lines = append(lines, holtTheme.line.Render(strings.Repeat("─", innerWidth)))
	...
	for index, item := range visible {
		...
		label := surface(holtTheme.ink).Render(labelText)
		pad := surface(holtTheme.ink).Render(strings.Repeat(" ", maxInt(0, labelWidth-lipgloss.Width(labelText))))
		...
		if desc := strings.TrimSpace(item.Description); desc != "" {
			...
			if truncated := truncateRunes(desc, maxInt(0, descWidth)); truncated != "" {
				line += surface(holtTheme.faint).Render("  " + truncated)
			}
		}
	...
	if len(visible) == 0 {
		...
		lines = append(lines, fillPaletteLine(searchPrefix+holtTheme.faint.Render(message), innerWidth, transparentSurface))
	}

	if footer = strings.TrimSpace(footer); footer != "" {
		lines = append(lines, holtTheme.line.Render(strings.Repeat("─", innerWidth)))
		line := holtTheme.faint.Render(footer)
		lines = append(lines, fillPaletteLine(line, innerWidth, transparentSurface))
	}
	return styledBlockFillTitle(paletteWidth, strings.TrimSpace(title), lines, holtTheme.lineStrong, lipgloss.NewStyle())
}

func styledBlockFillTitle(width int, title string, lines []string, borderStyle lipgloss.Style, fill lipgloss.Style) string {
	return styledBlockFillTitleStyled(width, title, lines, borderStyle, fill, holtTheme.ink.Bold(true))
}
...
func renderSuggestionSearchLine(query string, width int) string {
	...
	label := holtTheme.userPrompt.Render("search > ")
	valueWidth := maxInt(1, width-lipgloss.Width(label))
	value := holtTheme.ink.Render(truncateRunes(query, valueWidth))
...
func fillPaletteLine(line string, width int, surface func(lipgloss.Style) lipgloss.Style) string {
	...
	if pad > 0 {
		line += surface(holtTheme.ink).Render(strings.Repeat(" ", pad))
	}
	return line
}
...
func (m model) pickerOverlay(width int) string {
	...
	for index, item := range visible {
		...
		if item.Group != "" && item.Group != lastGroup {
			lines = append(lines, holtTheme.accent.Render(item.Group))
		...
		switch {
		case item.Local:
			left += surface(holtTheme.blue).Render("● ")
		case item.Remote:
			left += surface(holtTheme.accent).Render("● ")
		}
		if item.Favorite {
			left += surface(holtTheme.accent).Render("* ")
		}
		left += surface(holtTheme.ink).Render(item.Label)
		right := ""
		if item.Meta != "" {
			right = surface(holtTheme.faintest).Render(item.Meta)
		}
		...
		line := left + surface(holtTheme.ink).Render(strings.Repeat(" ", maxInt(1, gap))) + right
		lines = append(lines, fitStyledLine(line, innerWidth))
	}
	if len(visible) == 0 {
		lines = append(lines, holtTheme.faint.Render("  no matching items"))
	}
	return centerRenderedBlock(styledBlockFillTitle(overlayWidth, title, lines, holtTheme.lineStrong, lipgloss.NewStyle()), width)
}
...
func (m model) modelPickerOverlay(width int) string {
	...
	if status := strings.TrimSpace(m.modelPickerLoadError); status != "" {
		lines = append(lines, fillPaletteLine(searchPrefix+holtTheme.faint.Render(status), innerWidth, transparentSurface))
	}
	lines = append(lines, holtTheme.line.Render(strings.Repeat("─", innerWidth)))
	lastGroup := ""
	for index, item := range visible {
		if item.Group != "" && item.Group != lastGroup {
			lines = append(lines, fillPaletteLine(holtTheme.accent.Bold(true).Render(item.Group), innerWidth, transparentSurface))
			lastGroup = item.Group
		}
		lines = append(lines, renderModelPickerRow(innerWidth, start+index == m.picker.selected, item))
	}
	if len(visible) == 0 {
		lines = append(lines, fillPaletteLine(searchPrefix+holtTheme.faint.Render("no matching models"), innerWidth, transparentSurface))
	}
	if item, ok := m.picker.current(); ok {
		if detail := modelPickerItemDetail(item); detail != "" {
			lines = append(lines, holtTheme.line.Render(strings.Repeat("─", innerWidth)))
			lines = append(lines, fillPaletteLine(searchPrefix+holtTheme.faint.Render(detail), innerWidth, transparentSurface))
		}
	}
	lines = append(lines, holtTheme.line.Render(strings.Repeat("─", innerWidth)))
	footer := "↑/↓ move   Enter select   Ctrl+F favorite   Esc close"
	lines = append(lines, fillPaletteLine(holtTheme.faint.Render(footer), innerWidth, transparentSurface))
	title := strings.TrimSpace(m.picker.title)
	if title == "" {
...
func (m model) modelPickerLoadingOverlay(width int) string {
	...
	innerWidth := maxInt(1, overlayWidth-4)
	lines := []string{
		fillPaletteLine(holtTheme.faint.Render("Checking available models..."), innerWidth, transparentSurface),
		fillPaletteLine(holtTheme.faint.Render("Built-in models will be used if discovery fails."), innerWidth, transparentSurface),
		holtTheme.line.Render(strings.Repeat("─", innerWidth)),
		fillPaletteLine(holtTheme.faint.Render("Esc close"), innerWidth, transparentSurface),
	}
	title := strings.TrimSpace(m.picker.title)
	if title == "" {
		title = "Choose a model"
...
func renderModelPickerRow(width int, selected bool, item pickerItem) string {
	...
	left := marker + surface(holtTheme.ink).Render(prefix+label)
