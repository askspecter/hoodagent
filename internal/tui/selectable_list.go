...
func renderSelectableList(options selectableListOptions) string {
	...
	for index, item := range visible {
		...
		label := surface(holtTheme.ink).Render(item.Label)
		pad := surface(holtTheme.ink).Render(strings.Repeat(" ", maxInt(0, labelWidth-lipgloss.Width(item.Label))))
		...
		if strings.TrimSpace(item.Description) != "" {
			...
			if desc != "" {
				line += surface(holtTheme.faint).Render("  " + desc)
			}
		}
	...
	if hidden := len(options.Items) - len(visible); hidden > 0 {
		lines = append(lines, fitStyledLine(holtTheme.faint.Render(fmt.Sprintf("  %d more", hidden)), width))
	}
