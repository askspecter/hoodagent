...
func TestDiffCardBodyRendersCountsNumbersAndCap(t *testing.T) {
	...
	for _, want := range []string{holtTheme.diffAdd.Render("+3"), holtTheme.diffDel.Render("-0")} {
...
func TestLocalControlCardsUseFriendlyCompactLabels(t *testing.T) {
	...
	open := transcriptRow{kind: rowToolResult, id: "call_1", tool: "browser_open", status: tools.StatusOK, detail: "✓ HOLT - terminal agent\nhttp://localhost:8080/"}
	...
	for _, want := range []string{"Opened", "http://localhost:8080", "HOLT - terminal agent"} {
		if !strings.Contains(got, want) {
	...
	artifact := transcriptRow{kind: rowToolResult, id: "call_3", tool: "capture_artifact", status: tools.StatusOK, detail: "Artifact captured: /tmp/holt-artifacts/page.png\n\nhelper wrote screenshot"}
...
func TestBashCardBodyShowsShellIssueHint(t *testing.T) {
	...
	detail := strings.Join([]string{
		...
		"exit_code: 1",
		"[holt] shell issue: Windows cmd.exe rejected the command syntax.",
		"Suggestion: Use the cwd argument instead of cd.",
	}, "\n")
	row := transcriptRow{kind: rowToolResult, id: "call_1", tool: "bash", status: tools.StatusError, detail: detail}
	rc := buildRowContext([]transcriptRow{{kind: rowToolCall, id: "call_1", tool: "bash", detail: "cd /d/tmp/holt-pr-158 && ls -la"}})
...
func TestTitleBarShowsWorkspaceAndModel(t *testing.T) {
	m := limeTestModel()
	m.width = 120
	m.cwd = "/workspace/holt"
	m.gitBranch = "main"
	got := plainRender(t, m.titleBar(120))
	for _, want := range []string{" main", "/workspace/holt", "test-provider/test-model"} {
		if !strings.Contains(got, want) {
			t.Fatalf("title bar = %q, missing %q", got, want)
		}
	}
	for _, notWant := range []string{" 0 ", "holt /"} {
...
func TestTitleBarHighlightsBranchOverWorkspace(t *testing.T) {
	...
	highlightedBranch := holtTheme.muted.Render("") + " " + holtTheme.muted.Render("main")
	recessedWorkspace := holtTheme.faint.Render("/workspace/holt")
	...
	if faintBranch := holtTheme.faint.Render("") + " " + holtTheme.faint.Render("main"); strings.Contains(got, faintBranch) {
...
func TestSpecReviewCardShowsBadgePathAndKeys(t *testing.T) {
	got := plainRender(t, renderFocusedSpecReviewPrompt(pendingSpecReviewPrompt{
		SpecFilePath: "/repo/specs/holt-1.md",
		RelativePath: "specs/holt-1.md",
	}, 80))
	for _, want := range []string{"SPEC REVIEW", "specs/holt-1.md", "[a] approve", "[r] reject", "[e] edit file", "[esc] cancel"} {
		if !strings.Contains(got, want) {
