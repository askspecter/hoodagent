...
// resolveThemeMode precedence: flag > HOLT_THEME > persisted config > auto.
...
func assertPreviewMatchesSelection(t *testing.T, m model) {
	...
	if r, _, _, _ := holtTheme.inkColor.RGBA(); r != mustR(t, entry.Palette.ink) {
...
// Moving the cursor live-previews each palette: the global holtTheme swaps as the
...
func TestThemePickerCommitAppliesAndRecords(t *testing.T) {
	...
	if r, _, _, _ := holtTheme.inkColor.RGBA(); r != mustR(t, draculaPalette.ink) {
...
func TestThemePickerFilterRePreviews(t *testing.T) {
	...
	if r, _, _, _ := holtTheme.inkColor.RGBA(); r != mustR(t, lightPalette.ink) {
	...
	if r, _, _, _ := holtTheme.inkColor.RGBA(); r != mustR(t, darkPalette.ink) {
