...
func (m model) renderSpecialistCard(info specialistInfo, width int) string {
	...
	case specialistRunning:
		icon := m.spinnerGlyph()
		header = holtTheme.accent.Render(fmt.Sprintf("%s%s · %s · %s", icon, info.name, description, elapsedStr))
	case specialistCompleted:
		header = holtTheme.green.Render(fmt.Sprintf("✓ %s · %s · %s", info.name, description, elapsedStr))
	case specialistError:
		header = holtTheme.red.Render(fmt.Sprintf("✗ %s · %s · %s", info.name, description, elapsedStr))
	default:
		header = holtTheme.accent.Render(fmt.Sprintf("• %s · %s · %s", info.name, description, elapsedStr))
	}
	...
	var body string
	if info.status == specialistError {
		body = holtTheme.red.Render(bodyText)
	} else {
		body = holtTheme.muted.Render(bodyText)
	}
	...
	body += holtTheme.faint.Render("   · enter to open")
	...
	if info.status == specialistRunning && info.currentTool != "" {
		...
		lines = append(lines, holtTheme.muted.Render(progressLine))
	}
	...
	if info.status == specialistError && strings.TrimSpace(info.errorMsg) != "" {
		...
		lines = append(lines, holtTheme.red.Render("  "+errMsg))
	}
...
func specialistBorderStyle(status specialistStatus) lipgloss.Style {
	switch status {
	case specialistRunning:
		return holtTheme.cardRun
	case specialistError:
		return holtTheme.cardErr
	default:
		return holtTheme.line
	}
}
...
func renderSpecialistSummary(specialists []specialistInfo, spinnerView string) string {
	...
	return holtTheme.accent.Render(spinnerView) + holtTheme.muted.Render(summary[tailStart:])
}
