...
func (wizard *mcpAddWizardState) render(width int) string {
	...
	lines := []string{
		holtTheme.faint.Render(mcpAddWizardStepLine(wizard.step)),
	...
	block := styledBlockFillTitle(overlayWidth, "Add MCP Server", lines, holtTheme.lineStrong, lipgloss.NewStyle())
...
func (wizard *mcpAddWizardState) renderNameStep(width int) []string {
	...
	return []string{
		holtTheme.accent.Render("Server Name"),
		fitStyledLine(holtTheme.ink.Render("> "+value), width),
		holtTheme.faint.Render("Default: " + mcpAddWizardTypes[clampInt(wizard.selectedType, 0, len(mcpAddWizardTypes)-1)].Label),
		holtTheme.faint.Render("Use lowercase letters, numbers, dashes, or underscores."),
	}
}

func (wizard *mcpAddWizardState) renderTypeStep(width int) []string {
	lines := []string{holtTheme.accent.Render("Server Type")}
	for index, item := range mcpAddWizardTypes {
		...
		lines = append(lines, fillPaletteLine(surface(holtTheme.ink).Render(line), width, surface))
	}
	return lines
}
...
func (wizard *mcpAddWizardState) renderHeaderStep(width int) []string {
	...
	return []string{
		holtTheme.accent.Render("Add header"),
		fitStyledLine(holtTheme.ink.Render("> "+redaction.RedactString(value, redaction.Options{})), width),
		holtTheme.faint.Render(`Paste "Key: Value" or "Key=Value".`),
	}
}

func (wizard *mcpAddWizardState) renderConfirmStep(width int) []string {
	lines := []string{
		holtTheme.accent.Render("Review setup"),
		"server: " + holtTheme.ink.Render(wizard.serverName),
		"type: " + holtTheme.ink.Render(strings.ToUpper(wizard.serverType)),
	}
	if source := strings.TrimSpace(wizard.sourceLabel); source != "" {
		lines = append(lines, "source: "+holtTheme.ink.Render(source))
	}
	if sourceURL := strings.TrimSpace(wizard.sourceURL); sourceURL != "" {
		lines = append(lines, "docs: "+holtTheme.ink.Render(redactMCPWizardDisplayValue(sourceURL)))
	}
	if wizard.isRemote() {
		lines = append(lines, "url: "+holtTheme.ink.Render(redactMCPWizardDisplayValue(wizard.endpoint)))
		if wizard.headerKey != "" {
			lines = append(lines, "header: "+holtTheme.ink.Render(wizard.headerKey+"=[REDACTED]"))
		}
	} else {
		lines = append(lines, "command: "+holtTheme.ink.Render(redactMCPWizardCommand(wizard.endpoint)))
	}
	if len(wizard.prerequisites) > 0 {
		lines = append(lines, "", holtTheme.accent.Render("Needs"))
		for _, item := range wizard.prerequisites {
			lines = append(lines, "  - "+holtTheme.ink.Render(item))
		}
	}
	lines = append(lines, "", holtTheme.faint.Render("Enter saves and tests the server."))
...
func (wizard *mcpAddWizardState) renderResultStep(width int) []string {
	...
	lines := []string{
		holtTheme.accent.Render(title),
		holtTheme.ink.Bold(true).Render(displayValue(wizard.serverName, "unnamed")) + "  " + holtTheme.faint.Render(state),
		holtTheme.faint.Render(transport),
	}
	if result.Message != "" {
		lines = append(lines, fitStyledLine(holtTheme.red.Render(result.Message), width))
	}
	if !result.Saved {
		lines = append(lines, holtTheme.faint.Render("No config was saved yet."))
	} else {
