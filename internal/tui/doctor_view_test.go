...
func TestDoctorResultBorderStyleUsesSummaryStatusLine(t *testing.T) {
	...
	want := fmt.Sprint(holtTheme.red.GetForeground())
	if got != want {
...
func TestDoctorCommandOutputGroupsProviderAndPlatformChecks(t *testing.T) {
	output := doctorCommandOutput(doctor.Report{Checks: []doctor.Check{
		...
		doctorCheck("runtime.go", doctor.StatusPass, "Holt Go runtime is available."),
		doctorCheck("config.files", doctor.StatusPass, "Holt config file inputs are available."),
	}}, nil)
...
func TestDoctorCommandOutputIsProblemFirstDiagnosticCenter(t *testing.T) {
	output := doctorCommandOutput(doctor.Report{GeneratedAt: "2026-06-14T00:00:00Z", Checks: []doctor.Check{
		...
		doctorCheck("runtime.go", doctor.StatusPass, "Holt Go runtime is available."),
		doctorCheck("config.files", doctor.StatusPass, "Holt config files are available."),
	...
	output := doctorCommandOutput(doctor.Report{Checks: []doctor.Check{
		...
		doctorCheck("provider.connectivity", doctor.StatusWarn, "Provider connectivity was skipped."),
		{
			...
			Details: map[string]any{
				"remedy": "install the Windows sandbox command runner and setup helper together, then run `holt sandbox setup`",
			},
		},
...
func TestDoctorCommandOutputIncludesBackendCountsAndHints(t *testing.T) {
	backend := holtcommands.BackendLifecycleSnapshot{
		MCPServers: []holtcommands.MCPServerSnapshot{{Name: "filesystem"}, {Name: "github"}},
		Hooks:      []holtcommands.HookSnapshot{{ID: "lint", Enabled: true}},
		Plugins:    []holtcommands.PluginSnapshot{{ID: "browser"}, {ID: "github"}, {ID: "linear"}},
	}
