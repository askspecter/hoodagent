...
func renderMCPView(state MCPViewState, width int) string {
	...
	if !hasMCPViewContent(state) {
		lines = append(lines,
			...
			"  › Add MCP server      holt mcp add <name> --url <url>",
			"  › Add local stdio MCP holt mcp add <name> -- <command> [args...]",
			"  › List configured     holt mcp list",
			"",
			"Actions",
	...
	lines = append(lines,
		"",
		"Actions",
		"  add: holt mcp add <name> --url <url>",
		"  disconnect: holt mcp disable <name>",
		"  reconnect: holt mcp enable <name>",
		"  remove: holt mcp remove <name>",
	)
	return fitMCPManagerLines(lines, width)
}
...
func mcpServerActionLine(server MCPServerView) string {
	...
	actions := []string{"holt mcp check " + name}
	if strings.EqualFold(strings.TrimSpace(server.State), "disabled") {
		actions = append(actions, "holt mcp enable "+name)
	} else {
		actions = append(actions, "holt mcp disable "+name)
	}
	actions = append(actions, "holt mcp remove "+name)
	if strings.EqualFold(strings.TrimSpace(server.Auth), "oauth") {
		actions = append(actions, "holt mcp oauth login "+name)
	}
...
func mcpOAuthActionLine(server MCPOAuthServerView) string {
	...
	actions := []string{}
	if server.HasToken {
		actions = append(actions, "holt mcp oauth refresh "+name, "holt mcp oauth logout "+name)
	} else {
		actions = append(actions, "holt mcp oauth login "+name)
	}
	if server.Expired && !server.HasRefreshToken {
		actions = append(actions, "holt mcp oauth refresh "+name)
	}
