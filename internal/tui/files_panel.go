...
func (m model) sidebarFilesHeader(width int) string {
	...
	return sidebarHeaderWithCount("FILES", fmt.Sprintf("%d", n), holtTheme.muted, width)
}
...
func (m model) sidebarFileLines(width int) ([]string, []fileHit) {
	...
	if live != "" {
		lines = append(lines, " "+holtTheme.accent.Render("●")+" "+
			holtTheme.ink.Render(truncatePathLeft(live, room)))
	}
	...
	for _, f := range visible {
		if shown >= maxSidebarFiles {
			lines = append(lines, "   "+holtTheme.faint.Render(fmt.Sprintf("+%d more", len(visible)-shown)))
			break
		}
		shown++
...
func (m model) renderFileRow(f touchedFile, room int) string {
	badge := holtTheme.muted.Render("M")
	switch {
	case f.failed:
		badge = holtTheme.red.Render("✗")
	case f.created:
		badge = holtTheme.green.Render("A")
	}
	...
	pathStyle := holtTheme.muted
	lead := " "
	if m.selectedFile == f.path {
		lead = holtTheme.accent.Render("▸")
		pathStyle = holtTheme.ink
	}
	line := lead + badge + " " + pathStyle.Render(truncatePathLeft(f.path, pathRoom))
	if stat != "" {
		line += holtTheme.faintest.Render(stat)
	}
	return line
}
