...
// explicit accessibility/preference switch via HOLT_REDUCED_MOTION, and is also
...
// whereas HOLT_NO_FADE only governs the streaming-text fade.
func reducedMotionEnabled(env func(string) string, profile colorprofile.Profile) bool {
	if v := strings.TrimSpace(env("HOLT_REDUCED_MOTION")); v != "" && v != "0" && !strings.EqualFold(v, "false") {
		return true
	}
...
// init builds the palette once at package load (after holtTheme's var init), so
