...
func TestMCPManagerViewRendersServerActionHints(t *testing.T) {
	got := plainRender(t, renderMCPView(MCPViewState{
		Servers: []MCPServerView{
			{Name: "docs", Transport: "stdio", State: "enabled", Target: "holt-docs-mcp --workspace .", ToolCount: 2},
	...
	for _, want := range []string{
		"holt mcp check docs",
		"holt mcp disable docs",
		"holt mcp remove docs",
		"holt mcp enable linear",
		"holt mcp oauth login linear",
	} {
...
func TestMCPManagerViewRendersOAuthActionHints(t *testing.T) {
	...
	for _, want := range []string{
		"holt mcp oauth refresh linear",
		"holt mcp oauth logout linear",
		"holt mcp oauth login notion",
		"holt mcp oauth refresh expired",
	} {
		if !strings.Contains(got, want) {
	...
	state := BuildMCPViewState(MCPStateOptions{
		Config: config.MCPConfig{Servers: map[string]config.MCPServerConfig{
			"docs": {
				...
				Env:     map[string]string{"HOLT_DOCS_TOKEN": "env-secret"},
			},
			"linear": {
				Type: "http",
