...
// Run starts the Holt Bubble Tea shell and returns a process-style exit code.
func Run(ctx context.Context, options Options) int {
	...
	// arrive (e.g. `echo "" | holt`). Fail fast with guidance toward the headless
	...
	if !term.IsTerminal(os.Stdin.Fd()) {
		fmt.Fprintln(os.Stderr, "holt: the interactive shell needs a terminal (stdin is not a TTY). For non-interactive use, run: holt exec \"<prompt>\"")
		return 2
	}
	...
	if _, err := program.Run(); err != nil {
		...
		fmt.Fprintln(os.Stderr, "holt: tui error:", err)
		return 1
	}
	return 0
}
