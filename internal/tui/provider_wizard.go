...
// `holt` launch from a TUI session can use the baked-in ChatGPT client_id
// without requiring the user to export HOLT_OAUTH_ALLOW_PRESETS themselves.
func buildOAuthPresetEnv() map[string]string {
	...
	env["HOLT_OAUTH_ALLOW_PRESETS"] = "1"
	return env
}
...
func (m model) applyProviderWizard() (model, tea.Cmd) {
	...
	// and the HOLT_PROVIDER export (which pins spawned children) still point at
	// the old one — parent and children would silently run on different providers.
	var nextProvider holtruntime.Provider
	...
	// this, a HOLT_PROVIDER exported by an earlier switch stays pointing at the
...
func (wizard *providerWizardState) render(width int) string {
	...
	lines := []string{
		holtTheme.faint.Render(providerWizardStepLine(wizard)),
		holtTheme.line.Render(strings.Repeat("─", innerWidth)),
	}
	if wizard.err != "" {
		lines = append(lines, holtTheme.red.Render("error: "+wizard.err), "")
	}
	if wizard.oauthPending {
		lines = append(lines, wizard.renderOAuthWaiting(innerWidth)...)
		lines = append(lines,
			holtTheme.line.Render(strings.Repeat("─", innerWidth)),
			holtTheme.faint.Render("Esc cancel"),
		)
		block := styledBlockFillTitle(overlayWidth, "Provider setup", lines, holtTheme.lineStrong, lipgloss.NewStyle())
	...
	lines = append(lines,
		holtTheme.line.Render(strings.Repeat("─", innerWidth)),
		holtTheme.faint.Render(wizard.footer()),
	)
...
func (wizard *providerWizardState) renderMethodStep(width int) []string {
	options := providerWizardMethodOptions()
	wizard.selectedMethod = clampInt(wizard.selectedMethod, 0, maxInt(0, len(options)-1))
	lines := []string{holtTheme.accent.Render("How do you want to connect?")}
	for index, option := range options {
		surface := transparentSurface
		marker := surface(holtTheme.faintest).Render("  ")
		if index == wizard.selectedMethod {
			surface = holtTheme.onSel
			marker = surface(holtTheme.accent).Render("❯ ")
		}
		lines = append(lines, fitStyledLine(marker+surface(holtTheme.ink).Render(option.label), width))
		lines = append(lines, fitStyledLine("    "+holtTheme.faint.Render(option.subtitle), width))
	}
	return lines
}
...
func (wizard *providerWizardState) renderManageKeyStep(width int) []string {
	...
	lines := []string{holtTheme.accent.Render(name + " already has a saved key")}
	for index, option := range options {
		surface := transparentSurface
		marker := surface(holtTheme.faintest).Render("  ")
...
func (wizard *providerWizardState) renderOAuthWaiting(width int) []string {
	...
	if wizard.oauthDevice {
		lines := []string{
			holtTheme.accent.Render("Device-code sign-in for " + name),
			"",
		}
		if wizard.deviceUserCode == "" {
			return append(lines, fitStyledLine(holtTheme.faint.Render("Requesting a device code..."), width))
		}
		return append(lines,
			fitStyledLine(holtTheme.ink.Render("1. On any device, visit:  ")+holtTheme.accent.Render(wizard.deviceVerificationURI), width),
			fitStyledLine(holtTheme.ink.Render("2. Enter the code:  ")+holtTheme.accent.Bold(true).Render(wizard.deviceUserCode), width),
			"",
			fitStyledLine(holtTheme.faint.Render("Waiting for authorization..."), width),
		)
	}
	return []string{
		holtTheme.accent.Render("Signing in with " + name),
		"",
		fitStyledLine(holtTheme.ink.Render("Opening your browser — approve there, then return here."), width),
		fitStyledLine(holtTheme.faint.Render("Waiting for authorization..."), width),
		"",
		fitStyledLine(holtTheme.faint.Render("If your browser didn't open, run:  "+providerWizardOAuthCLIHint(provider)), width),
	}
}
...
func providerWizardOAuthCLIHint(provider providercatalog.Descriptor) string {
	if provider.OAuthMintsKey {
		return "holt auth openrouter"
	}
	return "holt auth login " + provider.ID
}
...
func (wizard *providerWizardState) renderProviderStep(width int) []string {
	...
	lines := []string{holtTheme.accent.Render(header)}
	...
	if wizard.oauthMode && wizard.oauthErr != "" {
		lines = append(lines, "", holtTheme.red.Render("OAuth login failed: "+wizard.oauthErr))
		if hint := providerWizardOAuthErrHint(wizard.currentProvider()); hint != "" {
			lines = append(lines, holtTheme.faint.Render(hint))
		}
	}
	return lines
}
...
func providerWizardOAuthErrHint(provider providercatalog.Descriptor) string {
	if strings.EqualFold(provider.ID, "huggingface") {
		return "Hugging Face needs your own app: create one at " +
			"https://huggingface.co/settings/applications/new, then set HOLT_OAUTH_HUGGINGFACE_CLIENT_ID."
	}
	return ""
}
...
func (wizard *providerWizardState) renderSelectableProvider(width int, index int, provider providercatalog.Descriptor) string {
	...
	left := marker + surface(holtTheme.ink).Render(name)
	if badge := providerWizardBadge(provider); badge != "" {
		left += surface(holtTheme.faint).Render("   " + badge)
	}
...
func (wizard *providerWizardState) renderEndpointStep(width int) []string {
	...
	return []string{
		holtTheme.accent.Render("Endpoint URL"),
		holtTheme.ink.Render("Enter the API base URL for " + provider.Name + "."),
		holtTheme.faint.Render(providerWizardEndpointHint(provider)),
		input,
	}
}
...
func (wizard *providerWizardState) renderNameStep(width int) []string {
	...
	return []string{
		holtTheme.accent.Render("Provider name"),
		holtTheme.ink.Render("Choose the short label shown in the status bar."),
		holtTheme.faint.Render("Leave blank to use " + name + "."),
...
func (wizard *providerWizardState) renderCredentialStep(width int) []string {
	...
	value := holtTheme.accent.Render("▌") + holtTheme.faint.Render("paste key here")
	if wizard.apiKey != "" {
		value = holtTheme.ink.Render(maskedProviderWizardKey(wizard.apiKey)) + holtTheme.accent.Render("▌")
	}
	input := holtTheme.userPrompt.Render("api key > ") + value
	lines := []string{
		holtTheme.accent.Render("Paste API key"),
		holtTheme.ink.Render(providerWizardCredentialInstruction(env)),
		input,
		holtTheme.faint.Render("Pasted keys are hidden and saved in your user config."),
	}
	if oauth {
		lines = append(lines, holtTheme.accent.Render("or  ctrl+o  to log in with OAuth in the browser (no key needed)"))
	}
	if wizard.oauthErr != "" {
		lines = append(lines, holtTheme.red.Render("OAuth login failed: "+wizard.oauthErr))
	}
...
func (wizard *providerWizardState) renderModelStep(width int) []string {
	...
	lines := []string{holtTheme.accent.Render("Choose a model")}
	if status := wizard.modelStatusText(); status != "" {
		lines = append(lines, holtTheme.faint.Render(status))
	}
	lines = append(lines, wizard.renderModelSearch(width))
	wizard.refreshModels()
	models := wizard.filteredModels()
	if len(models) == 0 {
		lines = append(lines, holtTheme.faint.Render("  no matching models"))
		return lines
	}
	...
	if detail := providerWizardModelDetail(wizard.currentModel()); detail != "" {
		lines = append(lines, fitStyledLine(holtTheme.faint.Render("  "+detail), width))
	}
	return lines
}
...
func (wizard *providerWizardState) renderModelLoadingStep(width int) []string {
	return []string{
		holtTheme.accent.Render("Choose a model"),
		"",
		fitStyledLine(holtTheme.faint.Render("Checking available models..."), width),
		fitStyledLine(holtTheme.faint.Render("Built-in models will be used if discovery fails."), width),
	}
}
...
func providerWizardInputLine(promptText string, value string, placeholder string, width int) string {
	prompt := holtTheme.userPrompt.Render("search > ")
	cursor := holtTheme.accent.Render("▌")
	if promptText != "" {
		prompt = holtTheme.userPrompt.Render(promptText)
	}
	if value == "" {
		return fitStyledLine(prompt+cursor+holtTheme.faint.Render(placeholder), width)
	}
	return fitStyledLine(prompt+holtTheme.ink.Render(value)+cursor, width)
}

func (wizard *providerWizardState) renderTypedModelStep(width int) []string {
	provider := wizard.currentProvider()
	return []string{
		holtTheme.accent.Render("Model name"),
		holtTheme.ink.Render("Enter the model ID this endpoint expects."),
		holtTheme.faint.Render("Examples: gpt-4.1, claude-sonnet-4-5, llama-3.3-70b"),
...
func (wizard *providerWizardState) renderSelectableModel(width int, index int, model providerWizardModel) string {
	...
	left := marker + surface(holtTheme.ink).Render(model.displayLabel())
	return fitStyledLine(left, width)
}
...
func (wizard *providerWizardState) renderDoneStep(width int) []string {
	...
	lines := []string{
		holtTheme.accent.Render("Ready to connect"),
		"",
		holtTheme.ink.Render("Provider    " + provider.Name),
	}
	if providerWizardNeedsEndpoint(provider) {
		lines = append(lines, holtTheme.ink.Render("Endpoint    "+strings.TrimSpace(wizard.baseURL)))
	}
	lines = append(lines,
		holtTheme.ink.Render("Name        "+providerWizardDisplayName(provider, wizard.baseURL, wizard.profileName)),
		holtTheme.ink.Render("Model       "+model.ID),
		holtTheme.ink.Render("Credential  "+providerWizardCredentialLabel(provider, wizard.apiKey)),
		"",
		holtTheme.faint.Render("Press Enter to save and start using this provider."),
	)
	return lines
}
