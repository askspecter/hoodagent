...
func (m model) permissionsTextWithStore(store grantLister) string {
	...
	snapshots := holtcommands.SandboxGrantSnapshots(grants)
		...
		return renderCommandOutput(commandOutput{
			...
			Sections: []commandSection{
				...
				{Title: "Next actions", Lines: []string{
					"holt providers catalog",
					"holt providers setup openai --set-active",
					"holt providers add openai --api-key-env OPENAI_API_KEY --set-active",
				}},
			},
		})
	}

	snapshot := holtcommands.ProviderSnapshotFromProfile(m.providerProfile, true)
...
func providerNextActionLines(profile config.ProviderProfile, snapshot holtcommands.ProviderSnapshot, activeName string) []string {
	...
	if providerCredentialRequired(profile, snapshot.ProviderKind) && !providerProfileHasCredential(profile) {
		if envName := providerCredentialEnvName(profile, snapshot.ProviderKind); envName != "" {
			lines = append(lines,
				...
				"holt providers add "+setupID+" --api-key-env "+envName+" --set-active",
			)
		} else {
	...
	return append(lines,
		"holt providers check "+providerName+" --connectivity",
		"holt providers catalog",
		"holt providers setup "+setupID+" --set-active",
	)
}
