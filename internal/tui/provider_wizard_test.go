...
func TestProviderWizardAdvancesProviderAPIKeyAndModelSteps(t *testing.T) {
	...
	assertNotContains(t, view, "holt providers add anthropic")
	...
	assertNotContains(t, view, "holt providers check")
}
...
func TestProviderWizardPersistsPastedKeyToUserConfig(t *testing.T) {
	...
	t.Setenv("HOLT_CRED_STORAGE", "encrypted-file")
...
// HOLT_PROVIDER exactly like the /model and /provider switch paths — a stale
...
func TestApplyProviderWizardExportsActiveProviderEnv(t *testing.T) {
	...
	m.newProvider = func(config.ProviderProfile) (holtruntime.Provider, error) {
		return &fakeProvider{}, nil
	}
	m.providerWizard = &providerWizardState{
		step:        providerWizardStepModel,
...
// status line and the HOLT_PROVIDER export (which pins spawned children) still
...
func TestApplyProviderWizardPersistFailureLeavesLiveStateUnchanged(t *testing.T) {
	t.Setenv("HOLT_CRED_STORAGE", "encrypted-file") // never touch the real OS keychain: apiKey is secured before the persist fails
	...
	m.newProvider = func(config.ProviderProfile) (holtruntime.Provider, error) { return newProvider, nil }
	m.providerWizard = &providerWizardState{
		step:        providerWizardStepModel,
