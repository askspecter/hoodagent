...
func TestRenderPromptIncludesRepoSummaryAndRelativePaths(t *testing.T) {
	root, err := filepath.Abs(filepath.Join("testdata", "Holt"))
	if err != nil {
		t.Fatalf("Abs: %v", err)
	}
	repo := RepoMap{
		...
		Files: []File{
			...
			{Path: filepath.Join(root, "cmd", "holt", "main.go")},
		},
	}
...
func TestRenderPromptReturnsEmptyForNonPositiveBudget(t *testing.T) {
	got := RenderPrompt(RepoMap{Root: "Holt", Files: []File{{Path: "main.go"}}}, 0)
	if got != "" {
