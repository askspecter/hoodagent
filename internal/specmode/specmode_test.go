...
func TestSaveDraftWritesPlainMarkdownWithCollisionSuffix(t *testing.T) {
	...
	if first.RelativePath != ".holt/specs/2026-06-08-oauth-redirect.md" {
...
func TestSaveDraftContainsAdversarialTitles(t *testing.T) {
	...
	for _, title := range []string{
		...
		t.Run(title, func(t *testing.T) {
			...
			if filepath.IsAbs(saved.RelativePath) || !strings.HasPrefix(saved.RelativePath, ".holt/specs/") {
...
func TestSubmitToolSavesSpecAndReturnsReviewControl(t *testing.T) {
	...
	if len(result.ChangedFiles) != 1 || result.ChangedFiles[0] != ".holt/specs/2026-06-08-implementation-plan.md" {
...
func TestLoadSpecFileRejectsPathsOutsideSpecDirectory(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected LoadSpecFile to reject a path outside .holt/specs")
	}
}

func TestLoadSpecFileRejectsNonRegularFiles(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, filepath.FromSlash(".holt/specs/not-a-file.md"))
