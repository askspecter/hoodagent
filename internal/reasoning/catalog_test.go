...
// TestCoversZeroShippedReasoningModels pins that every reasoning model Holt
// currently ships resolves in the catalog (by its api model id), so the
// models.dev fallback actually covers Holt's catalog rather than just well-known
