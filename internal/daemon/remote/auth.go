// Package remote adds an OPT-IN, TLS-only network bridge on top of holt's local
...
// the default; nothing here activates unless `holt daemon serve-remote` is run.
//
...
const (
	EnvToken     = "HOLT_DAEMON_REMOTE_TOKEN"
	EnvTokenFile = "HOLT_DAEMON_REMOTE_TOKEN_FILE"
)
