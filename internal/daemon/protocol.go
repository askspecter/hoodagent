// Package daemon implements holt's long-running daemon/server mode: a local
// control server that supervises a pool of headless `holt exec` worker processes
// and routes multiple agent sessions to them over an owner-only local socket.
//
// It reuses holt's existing building blocks rather than inventing new ones:
...
//   - internal/cli (exec) : a worker is a `holt exec -i/-o stream-json` process.
//
