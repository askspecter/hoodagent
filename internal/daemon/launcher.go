...
// reentrancyMarkers are the env vars holt sets on a command it has ALREADY
// wrapped in a sandbox. The daemon MUST strip them from every worker's
// environment: a worker that inherited them would trip holt's re-entrancy guard
...
// other variable is preserved — including the HOLT_SANDBOX_* policy config the
...
// execWorker is a WorkerHandle backed by a `holt exec` child process speaking
...
type ExecLauncherConfig struct {
	// Executable is the holt binary to spawn (defaults to os.Executable()).
	Executable string
...
// NewExecLauncher builds a Launcher that spawns `holt exec` workers which speak
...
func NewExecLauncher(cfg ExecLauncherConfig) (Launcher, error) {
	...
	if exe == "" {
		...
		if err != nil {
			return nil, fmt.Errorf("daemon: locate holt executable: %w", err)
		}
		exe = resolved
	}
