...
func TestNewCreatesOpenAIProviderWithFactoryOptions(t *testing.T) {
	...
	if transport.request.Header.Get("User-Agent") != "holt-factory-test" {
...
func TestNewThreadsCustomProviderHeaders(t *testing.T) {
	...
	if transport.request.Header.Get("HTTP-Referer") != "https://holt.dev" {
...
func TestNewRoutesChatGPTCatalogToCodexProvider(t *testing.T) {
	...
	t.Setenv("HOLT_OAUTH_STORAGE", "file")
	t.Setenv("HOLT_OAUTH_TOKENS_PATH", t.TempDir()+"/tokens.json")
...
// returns a Store on it. Pinning HOLT_OAUTH_STORAGE matters as much as the
// path: an inherited "keyring" value would send NewStore to the OS keychain
// and ignore HOLT_OAUTH_TOKENS_PATH entirely, making the test read/write the
