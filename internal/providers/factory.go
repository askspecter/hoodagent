...
import (
	...
	"github.com/askspecter/holt/internal/providers/anthropic"
...
func New(profile config.ProviderProfile, options Options) (holtruntime.Provider, error) {
...
// after Holt model aliases and provider-kind defaults are resolved.
...
func resolveProfile(profile config.ProviderProfile, options Options) (resolvedProfile, error) {
	...
	// user runs `holt` with a `catalogID: chatgpt` profile; the user can
	...
	if entry, ok := registry.Get(model); ok {
		...
		if providerKind == config.ProviderKindOpenAICompatible {
			if !entry.AllowsProvider(modelregistry.ProviderOpenAICompatible) {
				return resolvedProfile{}, fmt.Errorf("holt model %s belongs to %s, not %s", entry.ID, entry.Provider, modelregistry.ProviderOpenAICompatible)
			}
		} else if providerKind == config.ProviderKindAnthropicCompat {
			if !entry.AllowsProvider(modelregistry.ProviderAnthropic) {
				return resolvedProfile{}, fmt.Errorf("holt model %s belongs to %s, not %s", entry.ID, entry.Provider, providerKind)
			}
...
// per-candidate load error or a mid-session `holt auth login`, pick a different
// login than the bearer — a mismatch the backend rejects.
func newCodexProvider(profile config.ProviderProfile, resolved resolvedProfile, options Options) (holtruntime.Provider, error) {
