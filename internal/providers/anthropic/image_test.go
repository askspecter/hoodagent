...
func captureRequestBody(t *testing.T, request holtruntime.CompletionRequest) map[string]any {
	t.Helper()
...
func TestUserTextOnlyTurnUnchanged(t *testing.T) {
	body := captureRequestBody(t, holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{Role: holtruntime.MessageRoleUser, Content: "Describe this."},
		},
	})
		...
		Messages: []holtruntime.Message{
			{
				...
				Images:  []holtruntime.ImageBlock{{MediaType: "image/png", Data: raw}},
			},
		},
	})
	blocks := firstUserContentBlocks(t, body)
...
func TestUserImageOnlyTurnEmits(t *testing.T) {
	...
	body := captureRequestBody(t, holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{
				Role:   holtruntime.MessageRoleUser,
				Images: []holtruntime.ImageBlock{{MediaType: "image/jpeg", Data: raw}},
			},
		},
	})
