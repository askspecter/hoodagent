...
func TestStreamCompletionPostsMessagesRequest(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			...
			{Role: holtruntime.MessageRoleTool, Content: "file contents", ToolCallID: "toolu_1"},
		},
	...
	if sysBlock["type"] != "text" || sysBlock["text"] != "You are Holt." {
...
func TestStreamCompletionEmitsTextUsageAndDone(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSEEvent(w, "content_block_delta", `{"type":"content_block_delta","index":0,"delta":{"type":"text_delta","text":" Holt"}}`)
	...
	want := []holtruntime.StreamEvent{
		...
		{Type: holtruntime.StreamEventUsage, Usage: holtruntime.Usage{InputTokens: 25, OutputTokens: 15, PromptTokens: 25, CompletionTokens: 15}},
...
func TestStreamCompletionReportsCacheTokens(t *testing.T) {
	...
	var usage *holtruntime.Usage
	for _, event := range collectProviderEvents(t, provider) {
		if event.Type == holtruntime.StreamEventUsage {
			u := event.Usage
			usage = &u
		}
	}
...
func TestStreamCompletionCapturesThinkingBlocksForReplay(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	done := eventsOfType(events, holtruntime.StreamEventDone)
	if len(done) != 1 {
	...
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{Role: holtruntime.MessageRoleUser, Content: "go"},
			{
				Role:      holtruntime.MessageRoleAssistant,
				Content:   "I will grep.",
				ToolCalls: []holtruntime.ToolCall{{ID: "toolu_1", Name: "grep", Arguments: `{"pattern":"x"}`}},
				Reasoning: []holtruntime.ReasoningBlock{{Provider: providerName, Type: "thinking", Text: "reasoning", Signature: "sig-abc"}},
			},
			{Role: holtruntime.MessageRoleTool, Content: "result", ToolCallID: "toolu_1"},
		},
	})
	if err != nil {
		t.Fatalf("StreamCompletion returned error: %v", err)
...
func TestStreamCompletionEmitsToolUseBlocks(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	want := []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "toolu_1", ToolName: "read_file"},
		{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "toolu_1", ArgumentsFragment: `{"path":`},
		{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "toolu_1", ArgumentsFragment: `"src/index.ts"}`},
		{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "toolu_1"},
		{Type: holtruntime.StreamEventDone},
	}
	if !reflect.DeepEqual(events, want) {
		t.Fatalf("events = %#v, want %#v", events, want)
...
func TestStreamCompletionClosesOpenToolCallOnEOF(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSEEvent(w, "content_block_delta", `{"type":"content_block_delta","index":0,"delta":{"type":"input_json_delta","partial_json":"{\"pattern\":\"Holt\"}"}}`)
	})
...
func TestStreamCompletionRejectsMalformedHistoryBeforeDispatch(t *testing.T) {
	...
	_, err = provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			...
			{
				Role:      holtruntime.MessageRoleAssistant,
				ToolCalls: []holtruntime.ToolCall{{ID: "toolu_1", Name: "read_file", Arguments: `"src/index.ts"`}},
			},
		},
	})
