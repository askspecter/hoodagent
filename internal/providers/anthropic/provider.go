...
func New(options Options) (*Provider, error) {
	...
	maxTokens, err := providerio.PositiveOrDefault(options.MaxTokens, defaultMaxTokens, "holt Anthropic provider maxTokens")
	if err != nil {
		return nil, err
	}
	...
	case "content_block_delta":
		...
		case "text_delta":
			if payload.Delta.Text != "" {
				providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventText, Content: payload.Delta.Text})
			}
...
func (provider *Provider) anthropicRequest(request holtruntime.CompletionRequest) (messagesRequest, error) {
	system, messages, err := mapMessages(request.Messages)
	if err != nil {
		return messagesRequest{}, err
	}
	if len(messages) == 0 {
		return messagesRequest{}, errors.New("holt Anthropic provider requires at least one non-system message")
	}
...
func mapMessages(messages []holtruntime.Message) (string, []anthropicMessage, error) {
	systemParts := []string{}
	...
	for _, message := range messages {
		...
		case holtruntime.MessageRoleTool:
			if message.ToolCallID == "" {
				return "", nil, errors.New("holt Anthropic provider requires toolCallId on tool result messages")
			}
...
func parseToolArguments(argumentsJSON string, toolName string) (map[string]any, error) {
	...
	if err := json.Unmarshal([]byte(argumentsJSON), &parsed); err != nil {
		return nil, fmt.Errorf("holt Anthropic provider could not parse tool arguments for %s as JSON", toolName)
	}
	object, ok := parsed.(map[string]any)
	if !ok || object == nil {
		return nil, fmt.Errorf("holt Anthropic provider requires tool arguments for %s to be a JSON object", toolName)
	}
	return object, nil
}
...
type streamState struct {
	...
	reasoningBlocks     []holtruntime.ReasoningBlock
	inputTokens         int
	outputTokens        int
...
func (state *streamState) stopThinking(index int) {
	...
	state.reasoningBlocks = append(state.reasoningBlocks, holtruntime.ReasoningBlock{
...
func (state *streamState) startTool(ctx context.Context, index int, id string, name string, events chan<- holtruntime.StreamEvent) {
	state.tools[index] = toolBlock{id: id, name: name}
	providerio.SendEvent(ctx, events, holtruntime.StreamEvent{
		Type:       holtruntime.StreamEventToolCallStart,
		ToolCallID: id,
		ToolName:   name,
	})
}

func (state *streamState) deltaTool(ctx context.Context, index int, fragment string, events chan<- holtruntime.StreamEvent) {
	tool, ok := state.tools[index]
	if !ok || fragment == "" {
		return
	}
	providerio.SendEvent(ctx, events, holtruntime.StreamEvent{
		Type:              holtruntime.StreamEventToolCallDelta,
		ToolCallID:        tool.id,
		ArgumentsFragment: fragment,
	})
}

func (state *streamState) stopTool(ctx context.Context, index int, events chan<- holtruntime.StreamEvent) {
	tool, ok := state.tools[index]
	if !ok {
		return
	}
	providerio.SendEvent(ctx, events, holtruntime.StreamEvent{
		Type:       holtruntime.StreamEventToolCallEnd,
		ToolCallID: tool.id,
	})
	delete(state.tools, index)
}

func (state *streamState) closeOpen(ctx context.Context, events chan<- holtruntime.StreamEvent) {
