...
func TestClassifiedErrorCuratesAuthFailure(t *testing.T) {
	// AUDIT-H7: 401/403 must lead with an actionable instruction (run `holt auth`),
	...
	if !strings.Contains(msg, "holt auth") {
		t.Errorf("401 message should point at `holt auth`, got %q", msg)
	}
	...
	// 403 also curated.
	if got := ClassifiedError(http.StatusForbidden, "forbidden"); !strings.Contains(got, "holt auth") {
