...
type Options struct {
	...
	// When unset, Holt uses providerio.ResolveStreamIdleTimeout — the
	// HOLT_STREAM_IDLE_TIMEOUT override or providerio.DefaultStreamIdleTimeout.
...
func New(options Options) (*Provider, error) {
	...
	maxTokens, err := providerio.PositiveOrDefault(options.MaxTokens, defaultMaxTokens, "holt Gemini provider maxTokens")
	if err != nil {
		return nil, err
	}
...
func (provider *Provider) StreamCompletion(
	ctx context.Context,
	request holtruntime.CompletionRequest,
) (<-chan holtruntime.StreamEvent, error) {
	...
	events := make(chan holtruntime.StreamEvent, 16)
	go func() {
		defer close(events)
...
func (provider *Provider) stream(ctx context.Context, body []byte, events chan<- holtruntime.StreamEvent) {
	...
	if err != nil {
		providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventError, Error: provider.redact("provider stream error: " + err.Error())})
		return
	}
	defer func() {
	...
	if errors.Is(err, providerio.ErrStreamIdle) || errors.Is(err, providerio.ErrStreamStalled) {
		providerio.SendEvent(ctx, events, holtruntime.StreamEvent{
			Type:  holtruntime.StreamEventError,
...
func (provider *Provider) emitPayload(ctx context.Context, data string, state *streamState, events chan<- holtruntime.StreamEvent) bool {
	...
	for _, candidate := range payload.Candidates {
		...
		for _, part := range candidate.Content.Parts {
			...
			if part.Text != "" {
				providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventText, Content: part.Text})
			}
			if part.FunctionCall != nil {
				if part.FunctionCall.Name == "" {
					...
					providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallDropped})
					continue
				}
...
func (provider *Provider) emitDone(ctx context.Context, state *streamState, events chan<- holtruntime.StreamEvent) {
	if state.hasUsage {
		usage, err := holtruntime.NormalizeUsage(holtruntime.TokenUsage{
		...
		if err == nil {
			providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventUsage, Usage: usage})
		}
	}
	providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventDone, FinishReason: state.finishReason})
	state.done = true
}

func (provider *Provider) emitToolCall(ctx context.Context, functionCall functionCall, signature string, syntheticIndex int, events chan<- holtruntime.StreamEvent) bool {
	id := functionCall.ID
	if id == "" {
	...
	if err != nil {
		providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventError, Error: provider.redact("provider error: " + err.Error())})
		return false
	}
	...
	providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallStart, ToolCallID: id, ToolName: functionCall.Name, ToolCallSignature: signature})
	providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: id, ArgumentsFragment: string(encoded)})
	providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: id})
	return true
}

func (provider *Provider) emitHTTPError(ctx context.Context, response *http.Response, events chan<- holtruntime.StreamEvent) {
...
func (provider *Provider) geminiRequest(request holtruntime.CompletionRequest) (generateContentRequest, error) {
	systemInstruction, contents, err := mapMessages(request.Messages)
	if err != nil {
		return generateContentRequest{}, err
	}
	if len(contents) == 0 {
		return generateContentRequest{}, errors.New("holt Gemini provider requires at least one non-system message")
	}
...
// Anything outside it — most notably OpenAI's `additionalProperties`, which Holt
...
// keyword Holt (or an MCP server) adds later can't silently leak an unsupported
...
func mapMessages(messages []holtruntime.Message) (*geminiContent, []geminiContent, error) {
	...
	for _, message := range messages {
		...
		switch message.Role {
		case holtruntime.MessageRoleSystem:
			if hasContent {
				systemParts = append(systemParts, geminiPart{Text: content})
			}
		case holtruntime.MessageRoleTool:
			if message.ToolCallID == "" {
				return nil, nil, errors.New("holt Gemini provider requires toolCallId on tool result messages")
			}
		...
		case holtruntime.MessageRoleAssistant:
			parts := []geminiPart{}
			if hasContent {
...
func parseToolArguments(argumentsJSON string, toolName string) (map[string]any, error) {
	...
	if err := json.Unmarshal([]byte(argumentsJSON), &parsed); err != nil {
		return nil, fmt.Errorf("holt Gemini provider could not parse tool arguments for %s as JSON", toolName)
	}
	object, ok := parsed.(map[string]any)
	if !ok || object == nil {
		return nil, fmt.Errorf("holt Gemini provider requires tool arguments for %s to be a JSON object", toolName)
	}
	return object, nil
}
...
func normalizeFunctionCallArgs(value any, toolName string) (map[string]any, error) {
	...
	if !ok || object == nil {
		return nil, fmt.Errorf("holt Gemini provider requires streamed tool arguments for %s to be a JSON object", toolName)
	}
	return object, nil
}
...
func mapFinishReason(reason string) string {
	...
	case "MAX_TOKENS":
		return holtruntime.FinishReasonLength
	case "SAFETY", "PROHIBITED_CONTENT", "BLOCKLIST", "SPII", "RECITATION", "IMAGE_SAFETY":
		return holtruntime.FinishReasonContentFilter
	default:
