...
func TestMapFinishReasonNonNormal(t *testing.T) {
	...
	for _, cf := range []string{"SAFETY", "RECITATION", "IMAGE_SAFETY", "PROHIBITED_CONTENT", "BLOCKLIST", "SPII"} {
		if got := mapFinishReason(cf); got != holtruntime.FinishReasonContentFilter {
			t.Errorf("%q → %q, want content_filter (M3)", cf, got)
		}
	}
	if got := mapFinishReason("MAX_TOKENS"); got != holtruntime.FinishReasonLength {
...
func TestStreamCompletionSurfacesRecitationFinishReason(t *testing.T) {
	...
	for _, e := range events {
		if e.Type == holtruntime.StreamEventDone {
			sawDone = true
			if e.FinishReason != holtruntime.FinishReasonContentFilter {
				t.Fatalf("RECITATION done FinishReason = %q, want %q (M3)", e.FinishReason, holtruntime.FinishReasonContentFilter)
			}
		}
	}
	if !sawDone {
...
func TestStreamCompletionEmitsDroppedOnNamelessFunctionCallPart(t *testing.T) {
	...
	for _, e := range events {
		switch e.Type {
		case holtruntime.StreamEventToolCallDropped:
			dropped++
...
func TestStreamCompletionDoesNotDropValidFunctionCall(t *testing.T) {
	...
	for _, e := range events {
		...
		if e.Type == holtruntime.StreamEventToolCallStart && e.ToolName == "read_file" {
			sawStart = true
		}
	}
	if !sawStart {
