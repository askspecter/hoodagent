...
func TestStreamCompletionPostsGenerateContentRequest(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{Role: holtruntime.MessageRoleSystem, Content: "You are Holt."},
			{Role: holtruntime.MessageRoleUser, Content: "Read the file."},
			{
			...
			{Role: holtruntime.MessageRoleTool, Content: "file contents", ToolCallID: "call_1"},
			{Role: holtruntime.MessageRoleUser, Content: "Now grep for Holt."},
		},
	...
	if mergedUserParts[1].(map[string]any)["text"] != "Now grep for Holt." {
...
func TestStreamCompletionCapturesThoughtSignatureAndSkipsThoughtText(t *testing.T) {
	...
	for _, event := range events {
		if event.Type == holtruntime.StreamEventText && strings.Contains(event.Content, "internal reasoning") {
...
func TestGeminiRequestReplaysThoughtSignature(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{Role: holtruntime.MessageRoleUser, Content: "go"},
			{
				Role:      holtruntime.MessageRoleAssistant,
				ToolCalls: []holtruntime.ToolCall{{ID: "call_1", Name: "grep", Arguments: `{"pattern":"x"}`, Signature: "sig-xyz"}},
			},
			{Role: holtruntime.MessageRoleTool, Content: "result", ToolCallID: "call_1"},
		},
	})
	if err != nil {
		t.Fatalf("StreamCompletion returned error: %v", err)
...
func TestStreamCompletionAppliesCustomAuthAndHeaders(t *testing.T) {
	...
	provider, err := New(Options{
		...
		AuthScheme:    "Bearer",
		CustomHeaders: map[string]string{"X-Tenant": "holt"},
	})
	if err != nil {
		t.Fatalf("New returned error: %v", err)
	}
...
func TestStreamCompletionEmitsTextUsageAndReasoningTokens(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSE(w, `{"candidates":[{"content":{"parts":[{"text":" Holt"}]}}],"usageMetadata":{"promptTokenCount":25,"candidatesTokenCount":15,"thoughtsTokenCount":3,"cachedContentTokenCount":7}}`)
	})

	events := collectProviderEvents(t, provider)
	want := []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: "Hello"},
		{Type: holtruntime.StreamEventText, Content: " Holt"},
		{Type: holtruntime.StreamEventUsage, Usage: holtruntime.Usage{InputTokens: 25, OutputTokens: 18, PromptTokens: 25, CompletionTokens: 18, ReasoningTokens: 3, CachedInputTokens: 7}},
...
func TestStreamCompletionEmitsCandidateFunctionCalls(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		writeSSE(w, `{"candidates":[{"content":{"parts":[{"functionCall":{"id":"call_1","name":"read_file","args":{"path":"src/index.ts"}}},{"functionCall":{"id":"call_2","name":"grep","args":{"pattern":"Holt"}}}]}}]}`)
	})

	events := collectProviderEvents(t, provider)
	want := []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call_1", ToolName: "read_file"},
		{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call_1", ArgumentsFragment: `{"path":"src/index.ts"}`},
		{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call_1"},
		{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call_2", ToolName: "grep"},
		{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call_2", ArgumentsFragment: `{"pattern":"Holt"}`},
		{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call_2"},
		{Type: holtruntime.StreamEventDone},
	}
	if !reflect.DeepEqual(events, want) {
		t.Fatalf("events = %#v, want %#v", events, want)
...
func TestStreamCompletionUsesSyntheticToolIDsWhenMissing(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		writeSSE(w, `{"functionCalls":[{"name":"grep","args":{"pattern":"Holt"}}]}`)
	})
...
func TestStreamCompletionClassifiesHTTPAndPromptBlockErrors(t *testing.T) {
	...
	if len(events) != 1 || events[0].Type != holtruntime.StreamEventError || !strings.HasPrefix(events[0].Error, "auth error:") {
	...
	if len(events) != 1 || events[0].Type != holtruntime.StreamEventError || !strings.Contains(events[0].Error, "Content blocked: blocked by policy") {
...
func TestStreamCompletionStopsOnMalformedStreamToolArgs(t *testing.T) {
	...
	if len(eventsOfType(events, holtruntime.StreamEventDone)) != 0 {
...
func TestStreamCompletionRejectsMalformedHistoryBeforeDispatch(t *testing.T) {
	...
	_, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{{Role: holtruntime.MessageRoleTool, Content: "missing id"}},
	})
	...
	_, err = provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{Role: holtruntime.MessageRoleUser, Content: "call tool"},
			{
				Role:      holtruntime.MessageRoleAssistant,
				ToolCalls: []holtruntime.ToolCall{{ID: "call_1", Name: "read_file", Arguments: `"src/index.ts"`}},
			},
		},
	})
...
func validRequest() holtruntime.CompletionRequest {
	return holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{{Role: holtruntime.MessageRoleUser, Content: "hello"}},
	}
}
...
// parameters Holt emits (schemaToRuntimeMap always writes additionalProperties)
