...
type Options struct {
	...
	// MaxTokens caps the model's output tokens. Holt omits the cap (the model's
...
func (provider *Provider) stream(ctx context.Context, body []byte, events chan<- holtruntime.StreamEvent) {
	...
	if err != nil {
		...
		if ctxErr := ctx.Err(); ctxErr != nil {
			sendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventError, Error: provider.redact("provider stream error: " + ctxErr.Error())})
			return
		}
		...
		if humanized, ok := providerio.UpstreamUnreachable(err.Error()); ok {
			sendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventError, Error: provider.redact(humanized)})
			return
		}
		sendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventError, Error: provider.redact("provider stream error: " + err.Error())})
		return
	}
	defer func() {
	...
	if errors.Is(err, providerio.ErrStreamIdle) || errors.Is(err, providerio.ErrStreamStalled) {
		...
		sendEvent(ctx, events, holtruntime.StreamEvent{
	...
	if !state.done {
		...
		sendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventDone, FinishReason: state.finishReason})
	}
}
...
func (provider *Provider) emitPayload(ctx context.Context, data string, state *toolState, events chan<- holtruntime.StreamEvent) bool {
...
func (provider *Provider) emitChunk(
	...
	chunk streamChunk,
	state *toolState,
	events chan<- holtruntime.StreamEvent,
) {
	for _, choice := range chunk.Choices {
		if choice.Delta.ReasoningContent != "" {
			sendEvent(ctx, events, holtruntime.StreamEvent{
				Type:    holtruntime.StreamEventReasoning,
	...
	if chunk.Usage != nil {
		sendEvent(ctx, events, holtruntime.StreamEvent{
			Type: holtruntime.StreamEventUsage,
...
func sendEvent(ctx context.Context, events chan<- holtruntime.StreamEvent, event holtruntime.StreamEvent) {
	select {
	case <-ctx.Done():
		if event.Type == holtruntime.StreamEventError {
			select {
			case events <- event:
			default:
			}
...
func sendBufferedEvent(events chan<- holtruntime.StreamEvent, event holtruntime.StreamEvent) {
	select {
	case events <- event:
	default:
	}
...
func (provider *Provider) openAIRequest(request holtruntime.CompletionRequest) chatCompletionRequest {
...
func mapMessage(message holtruntime.Message) chatMessage {
	mapped := chatMessage{
	...
	if len(message.Images) == 0 || message.Role != holtruntime.MessageRoleUser {
