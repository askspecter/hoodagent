...
func TestToolCallKeyOutputIndexZero(t *testing.T) {
	...
	if got := p.toolCallKey(&responsesEvent{OutputIndex: &holt}); got != "output-0" {
	...
	if got := p.toolCallKey(&responsesEvent{ItemID: "call_x", OutputIndex: &holt}); got != "call_x" {
...
func TestHandleTerminalResponseNilPayload(t *testing.T) {
	...
	failed := make(chan holtruntime.StreamEvent, 4)
	st := &responsesState{}
	...
	completed := make(chan holtruntime.StreamEvent, 4)
...
func TestHandleTerminalResponseFailedPayloadWithoutError(t *testing.T) {
	...
	out := make(chan holtruntime.StreamEvent, 8)
	st := &responsesState{}
	...
	ok := make(chan holtruntime.StreamEvent, 8)
	p.handleTerminalResponse(context.Background(),
