...
func drainCodexEvents(t *testing.T, stream <-chan holtruntime.StreamEvent) {
	t.Helper()
	for ev := range stream {
		if ev.Type == holtruntime.StreamEventError {
...
func TestCodexProviderStreamsReasoningSummaryDeltas(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages:        []holtruntime.Message{{Role: holtruntime.MessageRoleUser, Content: "solve it"}},
		ReasoningEffort: "high",
	})
	if err != nil {
		t.Fatalf("StreamCompletion: %v", err)
	...
	for ev := range stream {
		switch ev.Type {
		case holtruntime.StreamEventReasoning:
			reasoning = append(reasoning, ev.Content)
		case holtruntime.StreamEventText:
			text = append(text, ev.Content)
		case holtruntime.StreamEventError:
			t.Fatalf("unexpected error event: %q", ev.Error)
		}
	}
...
func TestCodexProviderParsesResponsesTextDeltas(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{{Role: holtruntime.MessageRoleUser, Content: "say hi"}},
	})
	if err != nil {
		t.Fatalf("StreamCompletion: %v", err)
	}
	...
	for ev := range stream {
		...
		case holtruntime.StreamEventUsage:
			usage = &ev.Usage
		case holtruntime.StreamEventDone:
			gotDone = true
		case holtruntime.StreamEventError:
			t.Fatalf("unexpected error event: %q", ev.Error)
		}
...
func TestCodexProviderParsesResponsesToolCalls(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{{Role: holtruntime.MessageRoleUser, Content: "weather in SF?"}},
		Tools: []holtruntime.ToolDefinition{
	...
	for ev := range stream {
		switch ev.Type {
		case holtruntime.StreamEventToolCallStart:
			starts++
			startName = ev.ToolName
		case holtruntime.StreamEventToolCallDelta:
			deltas++
			fragments = append(fragments, ev.ArgumentsFragment)
		case holtruntime.StreamEventToolCallEnd:
			ends++
			endName = ev.ToolName
		case holtruntime.StreamEventError:
			t.Fatalf("unexpected error event: %q", ev.Error)
...
func TestCodexProviderSendsAssistantToolCallsAsInputItems(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{Role: holtruntime.MessageRoleUser, Content: "weather in SF?"},
			{
			...
			{Role: holtruntime.MessageRoleTool, ToolCallID: "call-1", Content: "72F and sunny"},
			{Role: holtruntime.MessageRoleUser, Content: "thanks"},
		},
	})
	if err != nil {
		t.Fatalf("StreamCompletion: %v", err)
...
func TestCodexProviderEmitsLengthFinishWhenStreamEndsWithoutCompletion(t *testing.T) {
	...
	var gotDone bool
	for ev := range stream {
		if ev.Type == holtruntime.StreamEventDone {
			gotDone = true
	...
	if finishReason != holtruntime.FinishReasonLength {
		t.Fatalf("finish reason = %q, want %q", finishReason, holtruntime.FinishReasonLength)
	}
}
