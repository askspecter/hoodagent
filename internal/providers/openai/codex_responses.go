...
func (p *CodexProvider) buildResponsesRequest(request holtruntime.CompletionRequest) (*responsesRequest, error) {
...
func (p *CodexProvider) userInputItem(msg holtruntime.Message) inputItem {
	parts := []contentItem{}
	if msg.Content != "" {
...
func (p *CodexProvider) assistantInputItems(msg holtruntime.Message) ([]inputItem, error) {
	items := []inputItem{}
	if msg.Content != "" {
...
func (p *CodexProvider) streamResponses(
	...
	if !state.done {
		...
		providerio.SendEvent(ctx, events, holtruntime.StreamEvent{
			...
			FinishReason: holtruntime.FinishReasonLength,
		})
	}
}
...
func (p *CodexProvider) emitResponsesEvent(
	...
	case responsesEventOutputTextDelta:
		if event.Delta != "" {
			providerio.SendEvent(ctx, events, holtruntime.StreamEvent{
				Type:    holtruntime.StreamEventText,
				Content: event.Delta,
			})
		}
		return true
...
func (p *CodexProvider) handleTerminalResponse(
	...
	if event.Response == nil {
		...
		if event.Type == responsesEventFailed {
			...
			providerio.SendEvent(ctx, events, holtruntime.StreamEvent{Type: holtruntime.StreamEventDone})
		}
		state.done = true
		return false
	}
