...
func TestStreamCompletionPostsChatCompletionRequest(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{Role: holtruntime.MessageRoleSystem, Content: "system"},
			{Role: holtruntime.MessageRoleUser, Content: "user"},
			{
				Role:    holtruntime.MessageRoleAssistant,
				Content: "using a tool",
				ToolCalls: []holtruntime.ToolCall{{
					ID:        "call_1",
					Name:      "read_file",
					Arguments: `{"path":"README.md"}`,
				}},
			},
			{Role: holtruntime.MessageRoleTool, Content: "contents", ToolCallID: "call_1"},
		},
		Tools: []holtruntime.ToolDefinition{{
			Name:        "read_file",
	...
	if gotUserAgent != "holt-test" {
		t.Fatalf("user agent = %q, want holt-test", gotUserAgent)
	}
...
func TestStreamCompletionOmitsAuthAndToolsWhenEmpty(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{{Role: holtruntime.MessageRoleUser, Content: "hi"}},
	})
	if err != nil {
		t.Fatalf("StreamCompletion returned error: %v", err)
	}
...
func TestStreamCompletionAppliesCustomAuthAndHeaders(t *testing.T) {
	...
	provider, err := New(Options{
		...
		AuthScheme:    "Token",
		CustomHeaders: map[string]string{"HTTP-Referer": "https://holt.dev"},
	})
	if err != nil {
		t.Fatalf("New returned error: %v", err)
	}
...
func TestStreamCompletionEmitsTextUsageAndDone(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSE(w, `{"choices":[{"delta":{"content":"holt"}}],"usage":{"prompt_tokens":12,"completion_tokens":5,"prompt_tokens_details":{"cached_tokens":3}}}`)
		writeSSE(w, `[DONE]`)
	})

	events := collectProviderEvents(t, provider)
	assertEvent(t, events[0], holtruntime.StreamEventText, "hello ")
	assertEvent(t, events[1], holtruntime.StreamEventText, "holt")
	if events[2].Type != holtruntime.StreamEventUsage || events[2].Usage.PromptTokens != 12 || events[2].Usage.CompletionTokens != 5 || events[2].Usage.CachedInputTokens != 3 {
		t.Fatalf("unexpected usage event: %#v", events[2])
	}
	if events[3].Type != holtruntime.StreamEventDone {
...
func TestStreamCompletionEmitsReasoningContentDeltas(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	reasoning := eventsOfType(events, holtruntime.StreamEventReasoning)
	if len(reasoning) != 2 {
	...
	if text := eventsOfType(events, holtruntime.StreamEventText); len(text) != 0 {
...
func TestStreamCompletionEmitsReasoningBeforeRegularContent(t *testing.T) {
	...
	assertEvent(t, events[0], holtruntime.StreamEventReasoning, "Thinking. ")
	assertEvent(t, events[1], holtruntime.StreamEventText, "Answer.")
	if events[2].Type != holtruntime.StreamEventDone {
...
func TestStreamCompletionPreservesLiteralThinkTagsByDefault(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	assertEvent(t, events[0], holtruntime.StreamEventText, "show <think>literal</think> markup")
	if reasoning := eventsOfType(events, holtruntime.StreamEventReasoning); len(reasoning) != 0 {
...
func TestStreamCompletionSplitsInlineThinkTagsFromContent(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	assertEvent(t, events[0], holtruntime.StreamEventReasoning, "private reasoning")
	assertEvent(t, events[1], holtruntime.StreamEventText, "public answer")
	if events[2].Type != holtruntime.StreamEventDone {
		t.Fatalf("third event = %#v, want done", events[2])
	}
}
...
func TestStreamCompletionSplitsInlineThinkTagsAcrossChunks(t *testing.T) {
	...
	text := eventsOfType(events, holtruntime.StreamEventText)
...
func TestStreamCompletionBuffersToolArgsUntilIDAndNameArrive(t *testing.T) {
	...
	if events[0].Type != holtruntime.StreamEventToolCallStart || events[0].ToolCallID != "call_1" || events[0].ToolName != "read_file" {
		t.Fatalf("unexpected start event: %#v", events[0])
	}
	if events[1].Type != holtruntime.StreamEventToolCallDelta || events[1].ToolCallID != "call_1" || events[1].ArgumentsFragment != `{"path":"README.md"}` {
		t.Fatalf("unexpected delta event: %#v", events[1])
	}
	if events[2].Type != holtruntime.StreamEventToolCallEnd || events[2].ToolCallID != "call_1" {
...
func TestStreamCompletionTracksMultipleToolCallsByIndex(t *testing.T) {
	provider := newTestProvider(t, func(w http.ResponseWriter, r *http.Request) {
		...
		writeSSE(w, `{"choices":[{"delta":{"tool_calls":[{"index":1,"id":"call_b","function":{"name":"grep","arguments":"\"holt\"}"}}]},"finish_reason":"tool_calls"}]}`)
		writeSSE(w, `[DONE]`)
	})

	events := collectProviderEvents(t, provider)
	starts := eventsOfType(events, holtruntime.StreamEventToolCallStart)
	deltas := eventsOfType(events, holtruntime.StreamEventToolCallDelta)
	ends := eventsOfType(events, holtruntime.StreamEventToolCallEnd)
	...
	if deltas[1].ToolCallID != "call_b" || deltas[1].ArgumentsFragment != `{"query":"holt"}` {
...
func TestStreamCompletionClassifiesHTTPErrorsAndRedactsToken(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(events) != 1 || events[0].Type != holtruntime.StreamEventError {
...
func TestStreamCompletionEmitsErrorWhenContextCancels(t *testing.T) {
	...
	stream, err := provider.StreamCompletion(ctx, holtruntime.CompletionRequest{})
	if err != nil {
		t.Fatalf("StreamCompletion returned setup error: %v", err)
	}
	<-requestStarted
...
func TestStreamCompletionFlushesBufferedContentWhenContextCancels(t *testing.T) {
	...
	events := []holtruntime.StreamEvent{}
	select {
	...
	if len(events) != 3 ||
		events[0].Type != holtruntime.StreamEventText || events[0].Content != "visible " ||
		events[1].Type != holtruntime.StreamEventText || events[1].Content != "<thi" ||
		events[2].Type != holtruntime.StreamEventError {
		t.Fatalf("events = %#v, want text, buffered text, then context error", events)
	}
	if done := eventsOfType(events, holtruntime.StreamEventDone); len(done) != 0 {
...
func collectProviderEvents(t *testing.T, provider *Provider) []holtruntime.StreamEvent {
	t.Helper()
	stream, err := provider.StreamCompletion(context.Background(), holtruntime.CompletionRequest{})
	if err != nil {
		t.Fatalf("StreamCompletion returned setup error: %v", err)
...
func readAll(stream <-chan holtruntime.StreamEvent) []holtruntime.StreamEvent {
	events := []holtruntime.StreamEvent{}
...
func drain(stream <-chan holtruntime.StreamEvent) {
	for range stream {
	}
}
...
func assertEvent(t *testing.T, event holtruntime.StreamEvent, eventType holtruntime.StreamEventType, content string) {
	t.Helper()
...
func eventsOfType(events []holtruntime.StreamEvent, eventType holtruntime.StreamEventType) []holtruntime.StreamEvent {
	matching := []holtruntime.StreamEvent{}
...
func TestStreamCompletionDoesNotHangOnEOFWithOpenToolCall(t *testing.T) {
	...
	for {
		...
		case event, ok := <-stream:
			if !ok {
				if len(eventsOfType(events, holtruntime.StreamEventToolCallEnd)) != 1 {
					t.Fatalf("events = %#v, want one tool-call-end on EOF", events)
				}
				if len(eventsOfType(events, holtruntime.StreamEventDone)) != 1 {
...
func TestStreamCompletionSkipsNamelessToolCallOnEOF(t *testing.T) {
	...
	events := collectProviderEvents(t, provider)
	if len(eventsOfType(events, holtruntime.StreamEventToolCallStart)) != 0 {
		t.Fatalf("events = %#v, want no start for nameless tool call", events)
	}
	if len(eventsOfType(events, holtruntime.StreamEventToolCallDelta)) != 0 {
		t.Fatalf("events = %#v, want no delta for nameless tool call", events)
	}
	if len(eventsOfType(events, holtruntime.StreamEventToolCallEnd)) != 0 {
...
func TestStreamCompletionIdleTimeoutAbortsStalledStream(t *testing.T) {
	...
	done := make(chan []holtruntime.StreamEvent, 1)
	go func() { done <- readAll(stream) }()
	var events []holtruntime.StreamEvent
	select {
	case events = <-done:
	...
	for _, e := range events {
		if e.Type == holtruntime.StreamEventText && e.Content == "hi" {
			gotText = true
		}
		if e.Type == holtruntime.StreamEventError && strings.Contains(strings.ToLower(e.Error), "idle") {
			gotIdleError = true
		}
	}
	if !gotText {
...
func TestStreamCompletionSurfacesLengthFinishReason(t *testing.T) {
	...
	var sawDone bool
	for _, e := range events {
		if e.Type == holtruntime.StreamEventDone {
			sawDone = true
	...
	if doneReason != holtruntime.FinishReasonLength {
		t.Fatalf("done FinishReason = %q, want %q", doneReason, holtruntime.FinishReasonLength)
	}

	// And it round-trips through the runtime collector as Truncated.
	collected := holtruntime.CollectStream(context.Background(), replay(events))
	if !collected.Truncated() || collected.FinishReason != holtruntime.FinishReasonLength {
...
func TestStreamCompletionSurfacesContentFilterFinishReason(t *testing.T) {
	...
	for _, e := range events {
		if e.Type == holtruntime.StreamEventDone {
			sawDone = true
			if e.FinishReason != holtruntime.FinishReasonContentFilter {
				t.Fatalf("done FinishReason = %q, want %q", e.FinishReason, holtruntime.FinishReasonContentFilter)
			}
		}
	}
	if !sawDone {
...
func TestStreamCompletionJoinsMultiLineDataFields(t *testing.T) {
	...
	var text string
	for _, e := range events {
		if e.Type == holtruntime.StreamEventText {
			text += e.Content
		}
		if e.Type == holtruntime.StreamEventError {
...
func replay(events []holtruntime.StreamEvent) <-chan holtruntime.StreamEvent {
...
func TestStreamCompletionEmitsDroppedOnNamelessToolCall(t *testing.T) {
	...
	for _, e := range events {
		if e.Type == holtruntime.StreamEventToolCallDropped {
			dropped = true
		}
		if e.Type == holtruntime.StreamEventToolCallStart {
			started = true
		}
	}
	if started {
...
func TestMapMessageBuildsImageURLContentParts(t *testing.T) {
	msg := mapMessage(holtruntime.Message{
		Role:    holtruntime.MessageRoleUser,
		Content: "describe these",
		Images: []holtruntime.ImageBlock{
...
func TestMapMessageImageOnlyOmitsTextPart(t *testing.T) {
	msg := mapMessage(holtruntime.Message{
		Role:    holtruntime.MessageRoleUser,
		Content: "",
		Images:  []holtruntime.ImageBlock{{MediaType: "image/png", Data: []byte("A")}},
	})
	parts, ok := msg.Content.([]contentPart)
	if !ok {
		t.Fatalf("Content type = %T, want []contentPart", msg.Content)
...
func TestMapMessageTextOnlyKeepsStringContent(t *testing.T) {
	msg := mapMessage(holtruntime.Message{Role: holtruntime.MessageRoleUser, Content: "hi"})
	if got, ok := msg.Content.(string); !ok || got != "hi" {
		t.Fatalf("Content = %#v, want string \"hi\"", msg.Content)
	}
	empty := mapMessage(holtruntime.Message{Role: holtruntime.MessageRoleAssistant, Content: ""})
...
func TestMapMessageNonUserRolesNeverCarryImages(t *testing.T) {
	images := []holtruntime.ImageBlock{{MediaType: "image/png", Data: []byte("ABC")}}
	for _, role := range []holtruntime.MessageRole{
		holtruntime.MessageRoleAssistant,
		holtruntime.MessageRoleTool,
		holtruntime.MessageRoleSystem,
	} {
		msg := mapMessage(holtruntime.Message{Role: role, Content: "plain", Images: images})
...
func TestOpenAIRequestEmptyContentHandling(t *testing.T) {
	...
	req := provider.openAIRequest(holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{Role: holtruntime.MessageRoleUser, Content: "hi"},
			// Degenerate empty assistant turn (e.g. a sub-agent that failed with no
			// output): must be dropped, not sent.
			{Role: holtruntime.MessageRoleAssistant, Content: "   "},
			// Assistant with tool calls but no text: kept, content present as "".
			{Role: holtruntime.MessageRoleAssistant, Content: "", ToolCalls: []holtruntime.ToolCall{{
				ID: "call_1", Name: "read_file", Arguments: "{}",
			}}},
			// Tool result with empty content: kept, content present as "".
			{Role: holtruntime.MessageRoleTool, Content: "", ToolCallID: "call_1"},
		},
	})
