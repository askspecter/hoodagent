...
func providerCallingAskUserThenAnswer(arguments string, answer string) *mockProvider {
	return &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "ask_user"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: arguments},
...
func TestRunAskUserReturnsHandlerAnswers(t *testing.T) {
	...
	if toolMessage.Role != holtruntime.MessageRoleTool || toolMessage.ToolCallID != "call-1" {
...
func TestRunAskUserCancellationAbortsRun(t *testing.T) {
	...
	for _, m := range result.Messages {
		if m.Role == holtruntime.MessageRoleTool {
			toolMsg = m.Content
		}
	}
