...
func TestToolResultRequestedModelFieldExists(t *testing.T) {
	...
	// Keep the holtruntime import load-bearing so the file compiles standalone.
	_ = holtruntime.MessageRoleAssistant
}
...
func escalateThenAnswerTurns(answer string) [][]holtruntime.StreamEvent {
	return [][]holtruntime.StreamEvent{
		{
			{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "c1", ToolName: "escalate"},
...
func TestRunSwitchesProviderOnEscalationRequest(t *testing.T) {
	...
	secondProvider := &mockProvider{turns: [][]holtruntime.StreamEvent{
		{
			{Type: holtruntime.StreamEventText, Content: "answered on the upgraded model"},
			{Type: holtruntime.StreamEventDone},
		},
	}}
...
func TestRunEscalationSwitcherErrorIsNonFatal(t *testing.T) {
	...
	for _, m := range provider.requests[1].Messages {
		if m.Role == holtruntime.MessageRoleUser && strings.Contains(strings.ToLower(m.Content), "could not switch") {
			sawNote = true
		}
	}
	if !sawNote {
...
func TestRunSwitchesAtMostOncePerTurn(t *testing.T) {
	...
	firstProvider := &mockProvider{turns: [][]holtruntime.StreamEvent{
		{
			{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "c1", ToolName: "escalate"},
			{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "c1"},
			{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "c2", ToolName: "escalate"},
			{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "c2"},
			{Type: holtruntime.StreamEventDone},
		},
	}}
	secondProvider := &mockProvider{turns: [][]holtruntime.StreamEvent{
