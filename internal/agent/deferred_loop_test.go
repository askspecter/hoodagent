...
func toolDefinitionByName(definitions []holtruntime.ToolDefinition, name string) *holtruntime.ToolDefinition {
...
func assertNoDeferredDiscoveryMessage(t *testing.T, request holtruntime.CompletionRequest) {
	t.Helper()
	for _, message := range request.Messages {
		if message.Role == holtruntime.MessageRoleUser && strings.Contains(message.Content, "Deferred tools:") {
...
func legacyToolDefinitions(registry *tools.Registry, permissionMode PermissionMode, options Options) []holtruntime.ToolDefinition {
	definitions := make([]holtruntime.ToolDefinition, 0)
...
func TestRunLoadsDeferredToolThenAdvertisesNextTurn(t *testing.T) {
	...
	provider := &mockProvider{turns: [][]holtruntime.StreamEvent{
		{ // turn 1: call load_signal
			{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "c1", ToolName: "load_signal"},
	...
	for _, m := range result.Messages {
		if m.Role == holtruntime.MessageRoleUser && strings.Contains(m.Content, "Tool discovery") {
...
func TestRunReactiveRetryKeepsLoadedDeferredToolAndDiscovery(t *testing.T) {
	...
	provider := &mockProvider{turns: [][]holtruntime.StreamEvent{
		...
		{ // turn 2: mid-stream context-limit error -> reactive compaction
			{Type: holtruntime.StreamEventError, Error: "prompt is too long: 250000 tokens > 200000 maximum"},
		},
	...
	var alpha *holtruntime.ToolDefinition
...
func (provider *connectErrorProvider) StreamCompletion(_ context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
