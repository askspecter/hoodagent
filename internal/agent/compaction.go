...
// The BPE tokenizers holt targets fold a run's leading space into the following
...
// staying allocation- and dependency-free. holt still receives the exact count
...
func estimateTokens(messages []holtruntime.Message) int {
	total := 0
...
func estimateToolDefTokens(tools []holtruntime.ToolDefinition) int {
	total := 0
...
func Compact(messages []holtruntime.Message, opts CompactionOptions) ([]holtruntime.Message, error) {
...
func CompactMessages(messages []holtruntime.Message, opts CompactionOptions) (CompactionResult, error) {
	...
	systemEnd := 0
	for systemEnd < len(messages) && messages[systemEnd].Role == holtruntime.MessageRoleSystem {
		systemEnd++
	}
	...
	compacted := make([]holtruntime.Message, 0, systemEnd+1+(len(messages)-boundary))
	compacted = append(compacted, messages[:systemEnd]...)
	compacted = append(compacted, holtruntime.Message{
...
// systemEnd.
func safeSuffixBoundary(messages []holtruntime.Message, systemEnd int, boundary int) int {
	...
	for boundary > systemEnd && messages[boundary].Role != holtruntime.MessageRoleAssistant {
		boundary--
	}
	return boundary
}
...
func (state *compactionState) recover(
	...
	tools []holtruntime.ToolDefinition,
	errorMessage string,
) (compacted []holtruntime.Message, retried bool, err error) {
	if !state.enabled {
...
func summarizeClosure(ctx context.Context, provider Provider, onUsage func(Usage)) func([]holtruntime.Message) (string, error) {
	return func(toSummarize []holtruntime.Message) (string, error) {
...
func summarizeWithFallback(ctx context.Context, provider Provider, messages []holtruntime.Message, onUsage func(Usage)) (string, error) {
	...
	reduced, reduceErr := summarizeMessagesOnce(ctx, provider, []holtruntime.Message{
		{Role: holtruntime.MessageRoleUser, Content: combined},
	}, onUsage)
	if reduceErr != nil {
...
func summarizeMessagesOnce(ctx context.Context, provider Provider, messages []holtruntime.Message, onUsage func(Usage)) (string, error) {
	request := holtruntime.CompletionRequest{
		Messages: []holtruntime.Message{
			{Role: holtruntime.MessageRoleSystem, Content: summaryInstructions},
			{Role: holtruntime.MessageRoleUser, Content: "Summarize this conversation:\n\n" + renderTranscript(messages)},
		},
	...
	collected := holtruntime.CollectStreamWithOptions(ctx, stream, holtruntime.CollectOptions{OnUsage: onUsage})
	if collected.Error != "" {
...
func renderTranscript(messages []holtruntime.Message) string {
