...
func Run(ctx context.Context, prompt string, provider Provider, options Options) (Result, error) {
	...
	messages := holtruntime.SeedMessagesWithImages(buildSystemPrompt(options), prompt, options.Images)
	...
	for turn := 0; turn < maxTurns; turn++ {
		...
		forwardingOpts := holtruntime.CollectOptions{OnUsage: options.OnUsage}
		...
		recoverStreamError := func(collected holtruntime.CollectedStream) (holtruntime.CollectedStream, error) {
			...
			if compacted, retried, retryErr := compactor.recover(ctx, provider, messages, request.Tools, collected.Error); retried {
				...
				retryRequest := holtruntime.CompletionRequest{
					Messages:        copyMessages(messages),
					Tools:           exposed,
					ReasoningEffort: options.ReasoningEffort,
				}
				...
				collected = holtruntime.CollectStreamWithOptions(ctx, retryStream, holtruntime.CollectOptions{
		...
		collected := holtruntime.CollectStreamWithOptions(ctx, stream, forwardingOpts)
		if collected.Error != "" {
		...
		for attempt := 1; attempt <= maxStreamStallRetries &&
			...
			collected = holtruntime.CollectStreamWithOptions(ctx, retryStream, forwardingOpts)
		}
		if collected.Error != "" {
		...
		messages = append(messages, holtruntime.Message{
		...
		for index, call := range collected.ToolCalls {
			...
			messages = append(messages, holtruntime.Message{
				Role:       holtruntime.MessageRoleTool,
...
func finalAnswerAfterMaxTurns(ctx context.Context, provider Provider, messages []holtruntime.Message, options Options) (string, []holtruntime.Message, string) {
	finalMessages := copyMessages(messages)
	finalMessages = append(finalMessages, holtruntime.Message{
		Role:    holtruntime.MessageRoleUser,
	...
	stream, err := streamWithReconnect(ctx, provider, holtruntime.CompletionRequest{
	...
	collected := holtruntime.CollectStreamWithOptions(ctx, stream, holtruntime.CollectOptions{
...
func executeToolCall(ctx context.Context, registry *tools.Registry, call ToolCall, permissionMode PermissionMode, options Options) (ToolResult, error) {
	...
	result := registry.RunWithOptions(ctx, call.Name, args, tools.RunOptions{
		...
		// a file that changed on disk outside Holt since it was last read.
...
func partitionTools(registry *tools.Registry, permissionMode PermissionMode, options Options, loaded map[string]bool) ([]holtruntime.ToolDefinition, string) {
	...
	definitions := make([]holtruntime.ToolDefinition, 0, len(visible))
	...
	for _, tool := range visible {
		...
		if !active {
			...
			definitions = append(definitions, holtruntime.ToolDefinition{
				Name:        name,
...
func copyMessages(messages []Message) []Message {
	...
	for index, message := range messages {
		...
		if message.Reasoning != nil {
			copied[index].Reasoning = append([]holtruntime.ReasoningBlock{}, message.Reasoning...)
		}
		...
		copied[index].Images = holtruntime.CloneImageBlocks(message.Images)
	}
	return copied
}
