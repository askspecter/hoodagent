...
func TestRequestPermissionsTurnGrantAllowsLaterToolAndCleansUp(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "grant-1", ToolName: tools.RequestPermissionsToolName},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "grant-1", ArgumentsFragment: `{"reason":"Need to write outside the workspace.","permissions":{"file_system":{"write":[` + quoteJSONString(target) + `]}}}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "grant-1"},
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "write-1", ToolName: "write_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "write-1", ArgumentsFragment: `{"path":` + quoteJSONString(target) + `,"content":"carrot\n","overwrite":true}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "write-1"},
...
func tempDirOutsideDefaultTemp(t *testing.T) string {
	t.Helper()
	dir, err := os.MkdirTemp(".", ".holt-sandbox-outside-")
	if err != nil {
...
func requestPermissionsOnlyProvider(arguments string, finalAnswer string) *mockProvider {
	...
	return &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "grant-1", ToolName: tools.RequestPermissionsToolName},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "grant-1", ArgumentsFragment: arguments},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "grant-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventText, Content: finalAnswer},
				{Type: holtruntime.StreamEventDone},
			},
		},
	}
