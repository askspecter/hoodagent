...
type Provider = holtruntime.Provider
type ToolCall = holtruntime.ToolCall
type Usage = holtruntime.Usage
...
type Options struct {
	...
	// can detect a file changed on disk outside Holt since it was last read. nil
...
type Result struct {
	...
	// that produced FinalAnswer: holtruntime.FinishReasonLength when the output
...
func (result Result) TruncationNotice() string {
	...
	case holtruntime.FinishReasonLength:
		return "Response was cut off at the output token limit and may be incomplete. " +
			"Raise the model's max output tokens or ask holt to continue."
	case holtruntime.FinishReasonContentFilter:
