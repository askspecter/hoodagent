...
func (p *stallProvider) StreamCompletion(_ context.Context, _ holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	n := atomic.AddInt32(&p.calls, 1)
	ch := make(chan holtruntime.StreamEvent, 5)
	if n <= p.stallBefore {
		if p.partialText != "" {
			ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventText, Content: p.partialText}
		}
		if p.reasoningText != "" {
			ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventReasoning, Content: p.reasoningText}
		}
		if p.partialToolCall != "" {
			...
			ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "tc_1", ToolName: p.partialToolCall}
			ch <- holtruntime.StreamEvent{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "tc_1", ArgumentsFragment: `{"path":"x.html","content":"<!doctype`}
		}
		ch <- holtruntime.StreamEvent{
