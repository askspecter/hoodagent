...
func pruneStaleToolOutput(messages []holtruntime.Message, preserveLast int) ([]holtruntime.Message, int) {
	if len(messages) == 0 {
		return messages, 0
	}
	if preserveLast < 0 {
	...
	for i := len(messages) - 1; i >= 0; i-- {
		msg := messages[i]
		if msg.Role != holtruntime.MessageRoleTool {
			continue
		}
	...
	out := make([]holtruntime.Message, len(messages))
	copy(out, messages)
	reclaimed := 0
...
func toolNameForResult(messages []holtruntime.Message, index int) string {
