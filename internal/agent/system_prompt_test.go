...
func TestBuildSystemPromptIncludesWorkspaceSeedFromCwd(t *testing.T) {
	cwd := t.TempDir()
	writeSystemPromptTestFile(t, cwd, "go.mod", "module example.test/holt\n")
	writeSystemPromptTestFile(t, cwd, "AGENTS.md", "Use Go commands.\n")
	writeSystemPromptTestFile(t, cwd, "cmd/holt/main.go", "package main\n")
...
func TestBuildSystemPromptProjectGuidelinesPathWalkingMonorepo(t *testing.T) {
	...
	// The user launches Holt from the sub-tree, so both files should be
...
func TestBuildSystemPromptProjectGuidelinesZeroFallback(t *testing.T) {
	// HOLT.md is the second-priority name at each level; the loader picks it
	// when no AGENTS.md is present.
	cwd := t.TempDir()
	if err := os.WriteFile(filepath.Join(cwd, "HOLT.md"), []byte("Brand-specific rule."), 0o644); err != nil {
		t.Fatal(err)
	}
	prompt := buildSystemPrompt(Options{Cwd: cwd})
	if !strings.Contains(prompt, "## Project guidelines (HOLT.md)") {
		t.Fatalf("expected HOLT.md fallback to be injected, got:\n%s", prompt)
	}
	if !strings.Contains(prompt, "Brand-specific rule.") {
		t.Fatalf("expected HOLT.md content, got:\n%s", prompt)
	}
}

func TestBuildSystemPromptProjectGuidelinesProjectLocalFallback(t *testing.T) {
	cwd := t.TempDir()
	dot := filepath.Join(cwd, ".holt")
	...
	// confirms the .holt/AGENTS.md file's content is the one injected, not
	// any other file.
	if !strings.Contains(prompt, "Personal: use dark theme.") {
		t.Fatalf("expected .holt/AGENTS.md content, got:\n%s", prompt)
	}
