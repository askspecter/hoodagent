...
type imageEchoProvider struct {
	seen []holtruntime.Message
}

func (p *imageEchoProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	if p.seen == nil {
		p.seen = append([]holtruntime.Message{}, request.Messages...)
	}
	events := make(chan holtruntime.StreamEvent)
	close(events)
	return events, nil
}
...
func TestRunSeedsImagesIntoUserTurn(t *testing.T) {
	...
	images := []holtruntime.ImageBlock{{MediaType: "image/png", Data: []byte{0x89, 0x50}}}
	...
	if user.Role != holtruntime.MessageRoleUser {
