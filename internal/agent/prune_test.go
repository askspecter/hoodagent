...
func bigToolResult(id string, tokens int) holtruntime.Message {
...
func toolCallMsg(id, name string) holtruntime.Message {
	return holtruntime.Message{
		Role:      holtruntime.MessageRoleAssistant,
		ToolCalls: []holtruntime.ToolCall{{ID: id, Name: name}},
	}
}
...
func TestPrunePreservesRecentAndPrunesOld(t *testing.T) {
	...
	msgs := []holtruntime.Message{
		...
		holtruntime.Message{Role: holtruntime.MessageRoleAssistant, Content: "thinking"},
		holtruntime.Message{Role: holtruntime.MessageRoleUser, Content: "next"},
	}
	...
	if out[1].ToolCallID != "old" || out[1].Role != holtruntime.MessageRoleTool {
...
func TestPruneIsIdempotent(t *testing.T) {
	msgs := []holtruntime.Message{
		...
		holtruntime.Message{Role: holtruntime.MessageRoleUser, Content: "go"},
	}
...
func TestPruneNeverTouchesNonToolMessages(t *testing.T) {
	msgs := []holtruntime.Message{
		{Role: holtruntime.MessageRoleUser, Content: strings.Repeat("u ", 60000)},
		{Role: holtruntime.MessageRoleAssistant, Content: strings.Repeat("a ", 60000)},
	}
