...
func emptyTurn() []holtruntime.StreamEvent {
	return []holtruntime.StreamEvent{{Type: holtruntime.StreamEventDone}}
}

// textTurn produces a turn with visible assistant text.
func textTurn(content string) []holtruntime.StreamEvent {
	return []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: content},
		{Type: holtruntime.StreamEventDone},
	}
}

// toolTurn produces a turn that calls a named tool with the given args JSON.
func toolTurn(callID string, toolName string, args string) []holtruntime.StreamEvent {
	return []holtruntime.StreamEvent{
...
func countUserMessagesContaining(messages []holtruntime.Message, needle string) int {
	count := 0
	for _, message := range messages {
		if message.Role == holtruntime.MessageRoleUser && strings.Contains(message.Content, needle) {
			count++
		}
	}
	return count
...
func TestGuardStateResetsToolOnlyStreakOnEmptyNonToolTurn(t *testing.T) {
	...
	toolOnly := holtruntime.CollectedStream{
		ToolCalls: []holtruntime.ToolCall{{ID: "call", Name: "read_file", Arguments: `{}`}},
	}

	for range toolOnlyProgressReminderAt - 1 {
		state.observeTurn(toolOnly)
	}
	state.observeTurn(holtruntime.CollectedStream{})
	state.observeTurn(toolOnly)
...
func TestRunInjectsToolOnlyProgressReminder(t *testing.T) {
	...
	turns := make([][]holtruntime.StreamEvent, 0, toolOnlyProgressReminderAt+1)
	...
	for _, message := range provider.requests[toolOnlyProgressReminderAt].Messages {
		if message.Role == holtruntime.MessageRoleUser && strings.Contains(message.Content, toolOnlyProgressReminderMarker) {
			found = true
		}
	}
	if !found {
...
func repeatedFlakyTurns(n int) [][]holtruntime.StreamEvent {
	turn := []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "c", ToolName: "flaky"},
		{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "c"},
		{Type: holtruntime.StreamEventDone},
	}
	turns := make([][]holtruntime.StreamEvent, 0, n)
	for i := 0; i < n; i++ {
...
func TestRunInjectsToolFailureHintWithSchema(t *testing.T) {
	...
	for _, m := range provider.requests[2].Messages {
		if m.Role == holtruntime.MessageRoleUser && strings.Contains(m.Content, toolFailureHintMarker) {
			found = true
