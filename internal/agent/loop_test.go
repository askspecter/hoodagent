...
type mockProvider struct {
	turns    [][]holtruntime.StreamEvent
	requests []holtruntime.CompletionRequest
}

func (provider *mockProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	provider.requests = append(provider.requests, request)

	events := []holtruntime.StreamEvent{{Type: holtruntime.StreamEventDone}}
	if len(provider.turns) >= len(provider.requests) {
		events = provider.turns[len(provider.requests)-1]
	}

	ch := make(chan holtruntime.StreamEvent, len(events))
...
func agentNativeBackendStub() sandbox.Backend {
	return sandbox.Backend{
		...
		Available:       true,
		Executable:      "/nonexistent/holt-linux-sandbox-stub",
...
func TestRunRetriesShellUnsandboxedAfterSandboxNamespaceLimitedOutput(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"ps aux"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventText, Content: "done"},
				{Type: holtruntime.StreamEventDone},
			},
		},
	}
...
func TestRunUsesEscalatedJustificationForPermissionPrompt(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"ps aux","sandbox_permissions":"require_escalated","justification":"Need host process visibility."}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
		},
...
func TestRunUsesUserFacingEscalatedFallbackForPermissionPrompt(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "exec_command"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"ps aux","sandbox_permissions":"require_escalated"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
		},
...
func TestRunRetriesNetworkDeniedShellWithNetworkGrant(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"node server.js"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunRetriesShellUnsandboxedAfterSandboxDeniedExit(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"npm install --save http-server"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunDoesNotRetryUnsandboxedWhenDeniedReadsActive(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"touch cache-file"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunReturnsProviderText(t *testing.T) {
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{{
			{Type: holtruntime.StreamEventText, Content: "hello"},
			{Type: holtruntime.StreamEventText, Content: " holt"},
			{Type: holtruntime.StreamEventDone},
		}},
	}
	...
	if result.FinalAnswer != "hello holt" {
	...
	assertMessage(t, provider.requests[0].Messages[0], holtruntime.MessageRoleSystem, "")
	assertMessage(t, provider.requests[0].Messages[1], holtruntime.MessageRoleUser, "say hi")
}

func TestRunDoesNotPersistReasoningAsAssistantText(t *testing.T) {
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{{
			{Type: holtruntime.StreamEventReasoning, Content: "private reasoning"},
			{Type: holtruntime.StreamEventText, Content: "public answer"},
			{Type: holtruntime.StreamEventDone},
		}},
	}
	...
	if last.Role != holtruntime.MessageRoleAssistant || last.Content != "public answer" {
...
func TestRunReportsTruncationFinishReason(t *testing.T) {
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{{
			{Type: holtruntime.StreamEventText, Content: "partial answer"},
			{Type: holtruntime.StreamEventDone, FinishReason: holtruntime.FinishReasonLength},
		}},
	}
	...
	if result.FinishReason != holtruntime.FinishReasonLength {
		t.Fatalf("FinishReason = %q, want %q", result.FinishReason, holtruntime.FinishReasonLength)
	}
	if !result.Truncated() {
...
func TestResultTruncationNotice(t *testing.T) {
	cases := map[string]struct {
		...
		"length":         {holtruntime.FinishReasonLength, true},
		"content_filter": {holtruntime.FinishReasonContentFilter, true},
...
func TestRunEmitsTextDeltas(t *testing.T) {
	...
	if strings.Join(deltas, "|") != "hello| holt" {
...
func TestRunEmitsUsageEvents(t *testing.T) {
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{{
			{Type: holtruntime.StreamEventUsage, Usage: holtruntime.Usage{PromptTokens: 12, CompletionTokens: 5, CachedInputTokens: 2}},
			{Type: holtruntime.StreamEventText, Content: "done"},
			{Type: holtruntime.StreamEventDone},
		}},
	}

	var usages []holtruntime.Usage
	_, err := Run(context.Background(), "track usage", provider, Options{
		Registry: tools.NewRegistry(),
		OnUsage:  func(usage holtruntime.Usage) { usages = append(usages, usage) },
	})
...
func TestRunRejectsLocalWebFetchBeforePermissionRequest(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "web_fetch"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"url":"http://localhost:8000/index.html"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunAdvertisesPromptedWebSearchInAutoMode(t *testing.T) {
	t.Setenv("HOLT_WEBSEARCH_BASE_URL", "https://search.example/api")
	registry := tools.NewRegistry()
...
func TestRunRequestsPermissionBeforeWebSearchExecution(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "web_search"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"query":"private workspace detail"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunRejectsFilteredToolCalls(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call_1", ToolName: "read_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call_1", ArgumentsFragment: `{"path":"README.md"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call_1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventText, Content: "done"},
...
func TestRunExecutesToolCallThroughRegistry(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "read_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"notes.txt"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventText, Content: "read done"},
				{Type: holtruntime.StreamEventDone},
			},
		},
	}
	...
	lastMessage := provider.requests[1].Messages[len(provider.requests[1].Messages)-1]
	assertMessage(t, lastMessage, holtruntime.MessageRoleTool, "alpha")
	if lastMessage.ToolCallID != "call-1" {
		t.Fatalf("expected tool_call_id call-1, got %q", lastMessage.ToolCallID)
	}
...
func TestRunSanitizesMalformedToolCallArgumentsBeforeRetry(t *testing.T) {
		...
		turns: [][]holtruntime.StreamEvent{
			{
				...
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"README.md"`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventText, Content: "recovered"},
				{Type: holtruntime.StreamEventDone},
			},
		},
	}
	...
	var assistantCall *holtruntime.ToolCall
	var toolParseError string
	for _, message := range provider.requests[1].Messages {
		if message.Role == holtruntime.MessageRoleAssistant && len(message.ToolCalls) > 0 {
			assistantCall = &message.ToolCalls[0]
		}
		if message.Role == holtruntime.MessageRoleTool && strings.Contains(message.Content, "Failed to parse arguments for read_file") {
...
func TestRunRecoversFirstObjectFromConcatenatedToolArgs(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "read_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"README.md"}{"path":"AGENTS.md"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
	...
	for _, message := range provider.requests[1].Messages {
		if message.Role == holtruntime.MessageRoleTool {
...
func TestRunDefersSelfCorrectFeedbackUntilAfterToolBatch(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "write_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"notes.txt","content":"hello"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "read_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"path":"existing.txt"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventText, Content: "done"},
	...
	for i, m := range msgs {
		switch m.Role {
		case holtruntime.MessageRoleTool:
			toolIdx = append(toolIdx, i)
		case holtruntime.MessageRoleUser:
...
func TestRunBatchesSelfCorrectOncePerTurn(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "write_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"a.txt","content":"hello"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "write_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"path":"b.txt","content":"hello"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: holtruntime.StreamEventDone},
			},
			{
	...
	for _, m := range provider.requests[1].Messages {
		if m.Role == holtruntime.MessageRoleUser && strings.Contains(m.Content, "Verification failed after your edit") {
			feedbackCount++
		}
	}
	if feedbackCount != 1 {
...
func TestRunSessionAllowSkipsMatchingPromptWithoutPersistentGrant(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "write_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":"notes.txt","content":"first","overwrite":true}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "write_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"path":"notes.txt","content":"second","overwrite":true}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunCommandPrefixApprovalSkipsLaterMatchingBashPrompt(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"echo prefix-ok"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"command":"echo prefix-ok again"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunCommandPrefixApprovalCoversSegmentedShellWithSafeTail(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			...
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"command":"ps aux | head -5"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunPersistentCommandPrefixApprovalSkipsFutureSessionPrompt(t *testing.T) {
	...
	firstProvider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"echo persist-ok"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventText, Content: "first done"},
				{Type: holtruntime.StreamEventDone},
			},
		},
	}
	...
	secondProvider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"command":"echo persist-ok again"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventText, Content: "second done"},
				{Type: holtruntime.StreamEventDone},
			},
		},
	}
...
func TestRunPersistentCommandPrefixStillPromptsForNetwork(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"curl https://example.com"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunApprovedNetworkBashPromptAppliesTurnNetworkGrant(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":` + quoteJSONString(command) + `}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunDoesNotOfferPrefixApprovalForUnsafeBashCommand(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"echo hi && npm install","prefix_rule":["echo"]}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunPromptsForDestructiveShellInsteadOfSandboxDeny(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"echo rm -rf /"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func (p cancelMidStreamProvider) StreamCompletion(_ context.Context, _ holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	p.cancel()
	return make(chan holtruntime.StreamEvent), nil
}
...
func TestRunSessionAllowsLaterOutsideWorkspaceWrite(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "write_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":` + quoteJSONString(outside) + `,"content":"first"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-2", ToolName: "write_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-2", ArgumentsFragment: `{"path":` + quoteJSONString(outside) + `,"content":"second","overwrite":true}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-2"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunRequestsFinalAnswerAfterMaxTurns(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			...
			{
				{Type: holtruntime.StreamEventText, Content: "I read notes.txt and found alpha."},
				{Type: holtruntime.StreamEventDone},
			},
		},
	}
	...
	if lastMessage.Role != holtruntime.MessageRoleUser || !strings.Contains(lastMessage.Content, "tool-turn limit") {
...
func providerCallingWritePathThenAnswer(path string, answer string) *mockProvider {
	return &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "write_file"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"path":` + quoteJSONString(path) + `,"content":"hello"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventText, Content: answer},
				{Type: holtruntime.StreamEventDone},
			},
		},
	}
...
func TestRunAppendsConfirmationPolicyToSystemPrompt(t *testing.T) {
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{{
			{Type: holtruntime.StreamEventText, Content: "ok"},
			{Type: holtruntime.StreamEventDone},
		}},
	}
	...
	if system.Role != holtruntime.MessageRoleSystem {
	...
	for _, marker := range []string{"You are Holt", "Testing gate"} {
...
func TestSystemPromptEmbedsConfirmationPolicy(t *testing.T) {
	...
	if !strings.HasPrefix(prompt, "You are Holt") {
...
func TestSpecDraftDeniesBashToolCalls(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "call-1", ToolName: "bash"},
				{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "call-1", ArgumentsFragment: `{"command":"printf ran > ran.txt"}`},
				{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "call-1"},
				{Type: holtruntime.StreamEventDone},
			},
			{
...
func TestRunStopsWhenSubmitSpecReturnsReviewControl(t *testing.T) {
	...
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{{
			{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "exit-1", ToolName: specmode.SubmitToolName},
			{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: "exit-1", ArgumentsFragment: `{"title":"Implementation Plan","plan":"# Goal\n\nAdd implementation plan."}`},
			{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "exit-1"},
			{Type: holtruntime.StreamEventDone},
		}},
	}
	...
	if !strings.Contains(result.FinalAnswer, ".holt/specs/") {
		t.Fatalf("final answer should mention saved spec, got %q", result.FinalAnswer)
	}
}

func assertMessage(t *testing.T, message holtruntime.Message, role holtruntime.MessageRole, contentContains string) {
	t.Helper()
...
func TestRunRetriesOnDroppedToolCall(t *testing.T) {
	provider := &mockProvider{
		turns: [][]holtruntime.StreamEvent{
			{
				{Type: holtruntime.StreamEventText, Content: "Let me write the files."},
				{Type: holtruntime.StreamEventToolCallDropped},
				{Type: holtruntime.StreamEventDone},
			},
			{
				{Type: holtruntime.StreamEventText, Content: "All done."},
				{Type: holtruntime.StreamEventDone},
			},
		},
	}
	...
	var fedback bool
	for _, m := range provider.requests[1].Messages {
		if m.Role == holtruntime.MessageRoleUser && strings.Contains(strings.ToLower(m.Content), "tool name") {
			fedback = true
		}
	}
	if !fedback {
...
func TestRunSurfacesDroppedToolCallAlongsideValidCall(t *testing.T) {
	...
	for _, m := range provider.requests[1].Messages {
		if m.Role == holtruntime.MessageRoleTool && m.ToolCallID == "call-1" {
			sawToolResult = true
		}
	}
	if !sawToolResult {
	...
	for _, m := range provider.requests[1].Messages {
		if m.Role == holtruntime.MessageRoleUser && strings.Contains(strings.ToLower(m.Content), "malformed") {
...
func TestRunAppendsAbortedPlaceholderForUnexecutedToolCallsOnGuardStop(t *testing.T) {
	...
	haltingTurn := []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "flaky-stop", ToolName: "flaky"},
		{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "flaky-stop"},
		{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "flaky-2", ToolName: "flaky"},
		{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "flaky-2"},
		{Type: holtruntime.StreamEventDone},
	}
	...
	for _, message := range result.Messages {
		if message.Role != holtruntime.MessageRoleAssistant {
			continue
		}
...
func TestRunScrubsSecretsFromToolOutput(t *testing.T) {
	...
	provider := &mockProvider{turns: [][]holtruntime.StreamEvent{
		{
			{Type: holtruntime.StreamEventToolCallStart, ToolCallID: "c1", ToolName: "leak"},
			{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: "c1"},
			{Type: holtruntime.StreamEventDone},
		},
		{
			{Type: holtruntime.StreamEventText, Content: "done"},
...
func TestRunDoesNotFlagCleanToolOutput(t *testing.T) {
	...
	provider := &mockProvider{turns: [][]holtruntime.StreamEvent{
		...
		{{Type: holtruntime.StreamEventText, Content: "done"}, {Type: holtruntime.StreamEventDone}},
	}}
	var captured ToolResult
