...
func TestCompactMessagesReturnsMetadataForManualCompaction(t *testing.T) {
	messages := []holtruntime.Message{
		...
		{Role: holtruntime.MessageRoleAssistant, Content: "recent answer"},
		{Role: holtruntime.MessageRoleUser, Content: "latest question"},
	}
	...
	result, err := CompactMessages(messages, CompactionOptions{
		...
		Summarize: func(toSummarize []holtruntime.Message) (string, error) {
			captured = append([]holtruntime.Message(nil), toSummarize...)
	...
	if result.Messages[1].Role != holtruntime.MessageRoleUser {
