...
func TestCompactKeepsSystemAndPreservedSuffix(t *testing.T) {
	messages := []holtruntime.Message{
		{Role: holtruntime.MessageRoleSystem, Content: "system prompt"},
		{Role: holtruntime.MessageRoleUser, Content: "first question"},
		{Role: holtruntime.MessageRoleAssistant, Content: "first answer"},
		{Role: holtruntime.MessageRoleUser, Content: "second question"},
		{Role: holtruntime.MessageRoleAssistant, Content: "second answer"},
		{Role: holtruntime.MessageRoleUser, Content: "most recent question"},
	}

	var captured []holtruntime.Message
	summarizeCalls := 0
	result, err := Compact(messages, CompactionOptions{
		PreserveLast: 2,
		Summarize: func(toSummarize []holtruntime.Message) (string, error) {
			summarizeCalls++
	...
	if result[0].Role != holtruntime.MessageRoleSystem || result[0].Content != "system prompt" {
		t.Fatalf("expected system message preserved at head, got %#v", result[0])
	}
	// Next is the injected summary as a single user message.
	if result[1].Role != holtruntime.MessageRoleUser {
...
func TestCompactSuffixNeverStartsWithToolResult(t *testing.T) {
	...
	messages := []holtruntime.Message{
		{Role: holtruntime.MessageRoleSystem, Content: "system prompt"},
		{Role: holtruntime.MessageRoleUser, Content: "do the thing"},
		{Role: holtruntime.MessageRoleAssistant, Content: "ok", ToolCalls: []holtruntime.ToolCall{{ID: "1", Name: "read_file"}}},
		{Role: holtruntime.MessageRoleTool, Content: "file contents", ToolCallID: "1"},
		{Role: holtruntime.MessageRoleAssistant, Content: "here is the result", ToolCalls: []holtruntime.ToolCall{{ID: "2", Name: "read_file"}}},
		{Role: holtruntime.MessageRoleTool, Content: "more contents", ToolCallID: "2"},
	}
...
func TestCompactNoopWhenTooFewMessages(t *testing.T) {
	...
	result, err := Compact(messages, CompactionOptions{
		PreserveLast: 8,
		Summarize: func([]holtruntime.Message) (string, error) {
			called = true
			return "x", nil
		},
	})
...
func TestCompactPropagatesSummarizeError(t *testing.T) {
	messages := []holtruntime.Message{
		{Role: holtruntime.MessageRoleSystem, Content: "system"},
		{Role: holtruntime.MessageRoleUser, Content: "a"},
		{Role: holtruntime.MessageRoleAssistant, Content: "b"},
		{Role: holtruntime.MessageRoleUser, Content: "c"},
		{Role: holtruntime.MessageRoleAssistant, Content: "d"},
		{Role: holtruntime.MessageRoleUser, Content: "e"},
	}
...
func (p *scriptedSummaryProvider) StreamCompletion(_ context.Context, _ holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	i := p.calls
	p.calls++
	if i < len(p.scripts) && p.scripts[i] != "" {
		return streamEvents([]holtruntime.StreamEvent{{Type: holtruntime.StreamEventError, Error: p.scripts[i]}}), nil
	}
	return streamEvents([]holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: "PARTIAL"},
		{Type: holtruntime.StreamEventDone},
	}), nil
}

func TestSummarizeWithFallbackPropagatesNonContextReduceError(t *testing.T) {
	msgs := []holtruntime.Message{
		{Role: holtruntime.MessageRoleUser, Content: "a"},
		{Role: holtruntime.MessageRoleAssistant, Content: "b"},
	}
...
func TestEstimateTokensMonotonic(t *testing.T) {
	small := []holtruntime.Message{{Role: holtruntime.MessageRoleUser, Content: "short"}}
	large := []holtruntime.Message{{Role: holtruntime.MessageRoleUser, Content: strings.Repeat("x", 4000)}}
	...
	grown := append([]holtruntime.Message{}, large...)
	grown = append(grown, holtruntime.Message{Role: holtruntime.MessageRoleAssistant, Content: "more"})
...
func TestEstimateTokensCountsToolCallsAndResults(t *testing.T) {
	plain := []holtruntime.Message{{Role: holtruntime.MessageRoleAssistant, Content: "hi"}}
	withCall := []holtruntime.Message{{
		Role:      holtruntime.MessageRoleAssistant,
		Content:   "hi",
		ToolCalls: []holtruntime.ToolCall{{ID: "1", Name: "read_file", Arguments: strings.Repeat("a", 400)}},
	}}
...
func TestEstimateTokensCountsImages(t *testing.T) {
	...
	plain := []holtruntime.Message{{Role: holtruntime.MessageRoleUser, Content: "look"}}
	withImage := []holtruntime.Message{{
		Role:    holtruntime.MessageRoleUser,
		Content: "look",
		Images:  []holtruntime.ImageBlock{{MediaType: "image/png", Data: []byte("tiny")}},
	}}
	twoImages := []holtruntime.Message{{
		Role:    holtruntime.MessageRoleUser,
		Content: "look",
...
func TestEstimateToolDefTokensCountsDefinitions(t *testing.T) {
	...
	one := []holtruntime.ToolDefinition{{
		Name:        "read_file",
	...
	two := append(append([]holtruntime.ToolDefinition{}, one...), holtruntime.ToolDefinition{
		Name:        "write_file",
...
func (provider *summarizeRecordingProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	...
	if len(request.Tools) == 0 {
		...
		return streamEvents([]holtruntime.StreamEvent{
			{Type: holtruntime.StreamEventText, Content: "COMPACTED SUMMARY"},
			{Type: holtruntime.StreamEventDone},
		}), nil
	}
...
func streamEvents(events []holtruntime.StreamEvent) <-chan holtruntime.StreamEvent {
...
func (provider *reactiveProvider) StreamCompletion(ctx context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	provider.requests = append(provider.requests, request)
	if len(request.Tools) == 0 {
		provider.summarizeCalls++
		return streamEvents([]holtruntime.StreamEvent{
			{Type: holtruntime.StreamEventText, Content: "SUMMARY"},
			{Type: holtruntime.StreamEventDone},
		}), nil
	}
	...
	case provider.turnRequests == 2 && !provider.failedOnce:
		...
		return streamEvents([]holtruntime.StreamEvent{
			{Type: holtruntime.StreamEventError, Error: "This model's maximum context length is 1000 tokens. Please reduce the length of the messages."},
		}), nil
	default:
...
func (provider *midStreamReactiveProvider) StreamCompletion(_ context.Context, request holtruntime.CompletionRequest) (<-chan holtruntime.StreamEvent, error) {
	if len(request.Tools) == 0 {
		provider.summarizeCalls++
		return streamEvents([]holtruntime.StreamEvent{
			{Type: holtruntime.StreamEventText, Content: "SUMMARY"},
	...
	case provider.turnRequests == 2 && !provider.failedOnce:
		...
		return streamEvents([]holtruntime.StreamEvent{
			{Type: holtruntime.StreamEventText, Content: provider.partialText},
			{Type: holtruntime.StreamEventError, Error: "This model's maximum context length is 1000 tokens. Please reduce the length of the messages."},
		}), nil
	default:
		return streamEvents(textTurn(provider.finalText)), nil
...
func toolTurnWithText(text string, callID string, toolName string, args string) []holtruntime.StreamEvent {
	return []holtruntime.StreamEvent{
		{Type: holtruntime.StreamEventText, Content: text},
		{Type: holtruntime.StreamEventToolCallStart, ToolCallID: callID, ToolName: toolName},
		{Type: holtruntime.StreamEventToolCallDelta, ToolCallID: callID, ArgumentsFragment: args},
		{Type: holtruntime.StreamEventToolCallEnd, ToolCallID: callID},
		{Type: holtruntime.StreamEventDone},
	}
}
...
func TestCompactNeverProducesConsecutiveUserMessages(t *testing.T) {
	msgs := []holtruntime.Message{
		{Role: holtruntime.MessageRoleSystem, Content: "sys"},
		{Role: holtruntime.MessageRoleUser, Content: "u1"},
		{Role: holtruntime.MessageRoleAssistant, Content: "a1"},
		{Role: holtruntime.MessageRoleUser, Content: "u2"},
		{Role: holtruntime.MessageRoleAssistant, Content: "a2"},
		{Role: holtruntime.MessageRoleUser, Content: "u3-latest"},
	}
	...
	for i := 1; i < len(out); i++ {
		if out[i].Role == holtruntime.MessageRoleUser && out[i-1].Role == holtruntime.MessageRoleUser {
...
func TestRecoverNoopDoesNotConsumeReactiveBudget(t *testing.T) {
	...
	provider := &mockProvider{turns: [][]holtruntime.StreamEvent{{
		{Type: holtruntime.StreamEventText, Content: "SUMMARY"}, {Type: holtruntime.StreamEventDone},
	}}}
	...
	big := []holtruntime.Message{
		{Role: holtruntime.MessageRoleSystem, Content: "sys"},
		{Role: holtruntime.MessageRoleUser, Content: strings.Repeat("u", 4000)},
		{Role: holtruntime.MessageRoleAssistant, Content: strings.Repeat("a", 4000)},
		{Role: holtruntime.MessageRoleUser, Content: "u2"},
		{Role: holtruntime.MessageRoleAssistant, Content: "a2"},
		{Role: holtruntime.MessageRoleUser, Content: "u3"},
	}
...
func TestRecoverDisabledIsNoop(t *testing.T) {
	...
	msgs := []holtruntime.Message{{Role: holtruntime.MessageRoleUser, Content: "x"}}
	called := false
	// recover must not invoke the provider/summarize when disabled, even on a
	// context-limit error string.
	got, retried, err := st.recover(context.Background(), &mockProvider{turns: [][]holtruntime.StreamEvent{{
		{Type: holtruntime.StreamEventText, Content: "should not be called"}, {Type: holtruntime.StreamEventDone},
