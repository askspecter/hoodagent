...
func extractLatestPlan(messages []holtruntime.Message) string {
...
func loadedSkills(messages []holtruntime.Message) []skillEntry {
	...
	for _, message := range messages {
		if message.Role != holtruntime.MessageRoleTool || message.ToolCallID == "" {
			continue
		}
...
// result text. ToolResult.Meta is not part of holtruntime.Message history, so the
// rendered "Loaded N tools" output is the durable transcript format.
func loadedToolSchemas(messages []holtruntime.Message) []skillEntry {
	...
	for _, message := range messages {
		if message.Role != holtruntime.MessageRoleTool || !toolSearchIDs[message.ToolCallID] {
			continue
		}
...
func projectInstructionEntries(messages []holtruntime.Message) []skillEntry {
	bodyBySource := map[string]string{}
	sourceOrder := make([]string, 0)
	for _, message := range messages {
		if message.Role != holtruntime.MessageRoleUser {
			continue
		}
...
func appendPreservedState(summary string, middle []holtruntime.Message) string {
...
func latestSummaryContent(messages []holtruntime.Message) string {
	for i := len(messages) - 1; i >= 0; i-- {
		m := messages[i]
		if m.Role == holtruntime.MessageRoleUser && strings.HasPrefix(strings.TrimSpace(m.Content), summaryLabel) {
			return m.Content
		}
	}
	return ""
