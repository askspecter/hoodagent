...
func EventUsagePayload(u holtruntime.Usage) map[string]any {
...
// Report is the aggregated usage view rendered by `holt usage report`. Cost is a
...
func BuildReport(events []sessions.Event, meta []sessions.Metadata, registry *modelregistry.Registry, netLOC int) (Report, error) {
	...
	for _, event := range events {
		...
		cost, err := modelregistry.CalculateCost(model, holtruntime.Usage{
