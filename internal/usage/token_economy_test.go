...
func TestEventUsageRoundTripPreservesCacheAndReasoningCost(t *testing.T) {
	...
	lossy, err := modelregistry.CalculateCost(model, holtruntime.Usage{InputTokens: 500_000, OutputTokens: 20_000})
	if err != nil {
...
// Holt cache/reasoning fields are omitted so non-cache turns stay compact and
// decode identically to the pre-feature payload.
func TestEventUsagePayloadOmitsZeroFields(t *testing.T) {
	p := EventUsagePayload(holtruntime.Usage{InputTokens: 1000, OutputTokens: 200})
