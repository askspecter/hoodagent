...
func LoadFile(path string, workspaceRoot string) (holtruntime.ImageBlock, error) {
	resolved := path
	...
	if err != nil {
		return holtruntime.ImageBlock{}, fmt.Errorf("image file not found: %s", path)
	}
	...
	if !info.Mode().IsRegular() {
		return holtruntime.ImageBlock{}, fmt.Errorf("image file must be a regular file: %s", path)
	}
	if info.Size() > MaxImageBytes {
		return holtruntime.ImageBlock{}, fmt.Errorf("image %s is larger than the 10 MiB limit", path)
	}
	...
	if err != nil {
		...
		return holtruntime.ImageBlock{}, fmt.Errorf("cannot open image %s: %w", path, err)
	}
	defer file.Close()

	data, err := io.ReadAll(io.LimitReader(file, MaxImageBytes+1))
	if err != nil {
		return holtruntime.ImageBlock{}, fmt.Errorf("cannot read image %s: %w", path, err)
	}
	...
	if mediaType == "" {
		return holtruntime.ImageBlock{}, fmt.Errorf("unsupported image type for %s (allowed: png, jpeg, gif, webp)", path)
	}
