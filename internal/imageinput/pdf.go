...
// transitive deps), so HOLT stays a single static cross-compilable binary with
...
type DocumentOptions struct {
	...
	Vision bool
	// MaxPages bounds how many pages the vision path renders. Holt means
...
// IsProbablyDocumentPath reports whether a path looks like a document HOLT can
...
func rasterizeWithPoppler(data []byte, maxPages int) ([]holtruntime.ImageBlock, error) {
	...
	dir, err := os.MkdirTemp("", "holt-pdf-raster-")
	if err != nil {
	...
	images := make([]holtruntime.ImageBlock, 0, len(entries))
...
func loadRenderedPage(name string) (holtruntime.ImageBlock, error) {
	info, err := os.Stat(name)
	if err != nil {
		return holtruntime.ImageBlock{}, err
	}
	if info.Size() > MaxImageBytes {
		return holtruntime.ImageBlock{}, fmt.Errorf("rendered page %s exceeds the per-image limit", filepath.Base(name))
	}
	...
	mediaType := holtruntime.NormalizeImageMediaType(http.DetectContentType(data[:sniffLen]))
	if mediaType == "" {
		return holtruntime.ImageBlock{}, fmt.Errorf("rendered page %s has an unsupported image type", filepath.Base(name))
	}
	return holtruntime.ImageBlock{MediaType: mediaType, Data: data}, nil
}
