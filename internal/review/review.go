...
var defaultCheckSpecs = []checkSpec{
	{Env: "HOLT_REVIEW_DIFF_CHECK", Label: "Diff hygiene", Command: "git diff --check"},
	{Env: "HOLT_REVIEW_TEST", Label: "Tests", Command: "go test ./..."},
	{Env: "HOLT_REVIEW_BUILD", Label: "Build", Command: "go run ./cmd/holt-release build"},
	{Env: "HOLT_REVIEW_SMOKE", Label: "Smoke build", Command: "go run ./cmd/holt-release smoke"},
}
...
func BuildSummaryInputFromEnv(env map[string]string) SummaryInput {
	return SummaryInput{
		...
		HeadSHA:      firstNonEmpty(env["HOLT_REVIEW_HEAD_SHA"], env["GITHUB_SHA"]),
		Checks:       BuildChecksFromEnv(env),
		ChangedFiles: ParseChangedFiles(env["HOLT_CHANGED_FILES"]),
	}
}
...
func parsePRNumber(env map[string]string) int {
	value := firstNonEmpty(env["HOLT_PR_NUMBER"], strings.Split(env["GITHUB_REF_NAME"], "/")[0])
