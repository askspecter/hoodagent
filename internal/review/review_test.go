...
func TestBuildChecksFromEnvDetectsBlockers(t *testing.T) {
	checks := BuildChecksFromEnv(map[string]string{
		"HOLT_REVIEW_DIFF_CHECK": "success",
		"HOLT_REVIEW_TEST":       "failure",
		"HOLT_REVIEW_BUILD":      "success",
		"HOLT_REVIEW_SMOKE":      "skipped",
	})
...
func TestBuildMarkdownApprovingReview(t *testing.T) {
	checks := BuildChecksFromEnv(map[string]string{
		"HOLT_REVIEW_DIFF_CHECK": "success",
		"HOLT_REVIEW_TEST":       "success",
		"HOLT_REVIEW_BUILD":      "success",
		"HOLT_REVIEW_SMOKE":      "success",
	})
	markdown := BuildMarkdown(SummaryInput{
		Number:       30,
		HeadSHA:      "abcdef1234567890",
		Checks:       checks,
		ChangedFiles: []string{"cmd/holt-pr-review/main.go", "internal/review/review_test.go"},
	})
...
func TestBuildSummaryInputFromEnv(t *testing.T) {
	input := BuildSummaryInputFromEnv(map[string]string{
		"HOLT_PR_NUMBER":         "42",
		"HOLT_REVIEW_HEAD_SHA":   "0123456789abcdef",
		"HOLT_REVIEW_DIFF_CHECK": "success",
		"HOLT_CHANGED_FILES":     "b.go\na.go",
	})
