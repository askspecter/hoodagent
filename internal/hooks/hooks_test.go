...
func TestResolvePathsUsesXDGLocations(t *testing.T) {
	...
	if paths.UserConfigPath != filepath.Join(dir, "config", "holt", "hooks.json") {
		t.Fatalf("user path = %q", paths.UserConfigPath)
	}
	if paths.ProjectConfigPath != filepath.Join(dir, ".holt", "hooks.json") {
		t.Fatalf("project path = %q", paths.ProjectConfigPath)
	}
	if paths.AuditPath != filepath.Join(dir, "data", "holt", "hooks", "audit.jsonl") {
...
func TestLoadConfigLayersProjectOverridesAndDiagnostics(t *testing.T) {
	...
	if got := []string{result.Config.Hooks[0].ID, result.Config.Hooks[1].ID}; !reflect.DeepEqual(got, []string{"holt.audit", "holt.format"}) {
	...
	if !hasHookDiagnostic(result.Diagnostics, DiagnosticDuplicate, "holt.format", "") {
...
func TestConfigStorePersistsUpdates(t *testing.T) {
	...
	changed, err := store.SetEnabled("holt.preflight", false)
	if err != nil || !changed {
	...
	if findHook(config.Hooks, "holt.preflight") != nil {
...
func TestConfigStoreListReadsOnlyStorePath(t *testing.T) {
	...
	if got := hookIDs(config.Hooks); !reflect.DeepEqual(got, []string{"holt.real"}) {
...
func TestSelectMatchesEnabledHooksByEventAndWildcard(t *testing.T) {
	config := Config{Enabled: true, Hooks: []Definition{
		{ID: "holt.reads", Event: EventBeforeTool, Matcher: "read_*", Command: "node", Enabled: true},
		{ID: "holt.shell", Event: EventBeforeTool, Matcher: "bash", Command: "node", Enabled: false},
		{ID: "holt.done", Event: EventSessionEnd, Command: "node", Enabled: true},
		{ID: "holt.shell-edit", Event: EventBeforeTool, Matcher: "shell_*_edit", Command: "node", Enabled: true},
	}}

	if got := hookIDs(Select(config, SelectInput{Event: EventBeforeTool, ToolName: "read_file"})); !reflect.DeepEqual(got, []string{"holt.reads"}) {
		t.Fatalf("read selection = %#v", got)
	}
	if got := hookIDs(Select(config, SelectInput{Event: EventBeforeTool, ToolName: "shell_safe_edit"})); !reflect.DeepEqual(got, []string{"holt.shell-edit"}) {
...
func TestSpecialistHookEventsLoadAndSelect(t *testing.T) {
	...
	writeHookJSON(t, projectConfigPath, map[string]any{
		"hooks": []any{
			map[string]any{
				"id":      "holt.specialist-start",
				"event":   "specialistStart",
				"command": "node",
			},
			map[string]any{
				"id":      "holt.specialist-stop",
	...
	if got := hookIDs(result.Config.Hooks); !reflect.DeepEqual(got, []string{"holt.specialist-start", "holt.specialist-stop"}) {
		t.Fatalf("hook ids = %#v", got)
	}
	if got := hookIDs(Select(result.Config, SelectInput{Event: EventSpecialistStart})); !reflect.DeepEqual(got, []string{"holt.specialist-start"}) {
		t.Fatalf("specialistStart selection = %#v", got)
	}
	if got := hookIDs(Select(result.Config, SelectInput{Event: EventSpecialistStop})); !reflect.DeepEqual(got, []string{"holt.specialist-stop"}) {
...
func TestAuditStoreAppendsAndSkipsMalformedLines(t *testing.T) {
	...
	if err := os.WriteFile(auditPath, []byte(strings.Join([]string{
		`{"sequence":1,"createdAt":"2026-06-04T00:00:00Z","type":"hook_execution_started","hookId":"holt.seed","event":"sessionStart"}`,
		"{not-json",
		`{"sequence":2,"createdAt":"2026-06-04T00:00:01Z","type":"hook_execution_completed","hookId":"holt.seed","event":"sessionStart","status":"completed"}`,
		"",
	}, "\n")), 0o600); err != nil {
		t.Fatal(err)
	}
