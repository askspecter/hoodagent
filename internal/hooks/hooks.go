...
func ResolvePaths(options ResolvePathOptions) (Paths, error) {
	...
	return Paths{
		UserConfigPath:    filepath.Join(configHome, "holt", "hooks.json"),
		ProjectConfigPath: filepath.Join(cwd, ".holt", "hooks.json"),
		AuditPath:         filepath.Join(dataHome, "holt", "hooks", "audit.jsonl"),
	}, nil
}
...
func readSingleConfig(path string) (Config, error) {
	...
	if len(diagnostics) > 0 {
		...
		return Config{}, fmt.Errorf("invalid Holt hook config at %s: %s", diagnostic.Path, diagnostic.Message)
	}
	if !ok {
...
// KnownEvents returns the hook events Holt recognizes, in dispatch order.
...
// IsValidEvent reports whether event is one Holt recognizes.
