...
func TestMain(m *testing.M) {
	if len(os.Args) >= 3 && os.Args[1] == "--holt-bash-helper" {
	...
	case "http-server":
		...
		server := &http.Server{
			Handler: http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
				_, _ = response.Write([]byte("holt-server-ok"))
			}),
		}
...
func TestDetectShellCommandIssueFlagsWindowsBashisms(t *testing.T) {
	issue := detectShellCommandIssue(`cd /d/tmp/holt-pr-158 && ls -la`, "windows")
	if issue == nil {
...
func TestDetectShellCommandIssueAllowsWindowsCDSwitch(t *testing.T) {
	issue := detectShellCommandIssue(`cd /d D:\tmp\holt-pr-158 && dir`, "windows")
	if issue != nil {
...
func TestDetectShellOutputIssueAddsWindowsSyntaxHint(t *testing.T) {
	issue := detectShellOutputIssue(`cd /d/tmp/holt-pr-158 && ls -la`, "The syntax of the command is incorrect.", "windows")
	if issue == nil {
		t.Fatal("expected Windows syntax error to get shell guidance")
	}
	rendered := appendShellIssueHint("stderr:\nThe syntax of the command is incorrect.\nexit_code: 1", *issue)
	for _, want := range []string{"[holt] shell issue:", "Windows cmd.exe", "Suggestion:"} {
...
func helperCommand(name string) string {
	...
	return executable + " --holt-bash-helper " + name
}
