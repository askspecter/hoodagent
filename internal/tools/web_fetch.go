...
func (tool webFetchTool) RunWithSandbox(ctx context.Context, args map[string]any, _ *holtSandbox.Engine) Result {
	return tool.run(ctx, args)
}
...
func (tool webFetchTool) run(ctx context.Context, args map[string]any) Result {
	...
	request.Header.Set("User-Agent", "holt-web-fetch/0.1")
