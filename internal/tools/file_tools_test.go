...
func TestListDirectoryToolListsRecursivelyAndIgnoresJunk(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(root, "README.md"), "# Holt")
...
func TestGlobToolFindsMatchesWithLimit(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, filepath.Join(root, "a.go"), "package holt")
...
func TestGrepSkipsAlwaysExcludedDirectories(t *testing.T) {
	...
	mustWrite(".holt/state.json", "needle here")
	...
	for _, forbidden := range []string{".git", ".holt", "node_modules", "vendor", ".worktrees"} {
...
func TestGlobSkipsWorkspaceExcludedDirectoriesAndBinaryFiles(t *testing.T) {
	...
	writeTestFile(t, filepath.Join(root, ".holt", "state.json"), "{}")
	...
	for _, forbidden := range []string{".holt", "vendor", ".worktrees", "image.png"} {
		if strings.Contains(res.Output, forbidden) {
