...
func shellStub(name string, description string) string {
	return `#!/bin/sh
# ` + name + ` — Holt plugin tool entry point.
# ` + description + `
#
# Holt invokes this script with the tool-call arguments as a single JSON object
...
func nodeStub(name string, description string) string {
	return `// ` + name + ` — Holt plugin tool entry point.
// ` + description + `
//
// Holt invokes this script with the tool-call arguments as a single JSON object
// on stdin and expects the tool result on stdout.
...
Holt invokes this script with the tool-call arguments as a single JSON object on
