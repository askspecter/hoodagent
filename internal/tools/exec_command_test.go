...
func TestExecCommandForegroundServerReturnsSessionAndServesHTTP(t *testing.T) {
	...
	if string(bytes) != "holt-server-ok" {
...
func TestStartExecProcessFallsBackAfterPTYStartMutation(t *testing.T) {
	...
	command := exec.CommandContext(context.Background(), os.Args[0], "--holt-bash-helper", "success")
	output := newExecOutputBuffer()
...
// tens of gigabytes and got the whole holt process OOM-killed by the OS.
...
func resilientTempDir(t *testing.T) string {
	t.Helper()
	dir, err := os.MkdirTemp("", "holt-exec-interrupt-")
	if err != nil {
...
func TestTruncateExecOutputPreservesUTF8(t *testing.T) {
	...
	if !strings.Contains(truncated, "[holt] output truncated") {
