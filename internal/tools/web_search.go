...
type searchResult struct {
	...
	// range. Holt (the zero value) is treated as "absent" by the renderer.
	Score float64
}
...
func (tool webSearchTool) RunWithSandbox(ctx context.Context, args map[string]any, engine *holtSandbox.Engine) Result {
...
func (tool webSearchTool) Run(ctx context.Context, args map[string]any) Result {
	...
	if tool.backend == nil {
		return errorResult("Error: no search backend configured. Set HOLT_WEBSEARCH_BASE_URL (and HOLT_WEBSEARCH_API_KEY) to enable web_search.")
	}
...
// " — score 0.91" after the title. Absent (holt), negative, and sub-0.005 scores
...
// HOLT_WEBSEARCH_BASE_URL is unset (the tool then reports it as unconfigured).
func defaultSearchBackend() searchBackend {
	baseURL := strings.TrimSpace(os.Getenv("HOLT_WEBSEARCH_BASE_URL"))
	if baseURL == "" {
		return nil
	}
	return &httpSearchBackend{
		...
		baseURL:  baseURL,
		apiKey:   strings.TrimSpace(os.Getenv("HOLT_WEBSEARCH_API_KEY")),
		provider: strings.TrimSpace(os.Getenv("HOLT_WEBSEARCH_PROVIDER")),
	}
}
...
func (backend *httpSearchBackend) Search(ctx context.Context, query string, limit int) ([]searchResult, error) {
	...
	// query; without this the HOLT_WEBSEARCH_PROVIDER knob would be inert.
	if backend.provider != "" {
	...
	request.Header.Set("User-Agent", "holt-web-search/0.1")
	if backend.apiKey != "" {
