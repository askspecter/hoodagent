...
// session — i.e. it was modified outside Holt. The write tools surface it with
...
// content against this baseline to detect a modification made outside Holt.
//
...
func fileConflictMessage(relativePath string) string {
	return "Error writing " + relativePath + ": " + ErrFileChangedOnDisk.Error() +
		" (it may have been edited outside Holt). Re-read it with read_file, then re-apply your change so you do not overwrite the newer content."
}
