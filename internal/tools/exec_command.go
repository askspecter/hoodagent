...
const (
	...
	// tens of gigabytes over several hours and got the whole holt process
	// OOM-killed by the OS.
	maxExecOutputBufferBytes         = 2 * 1024 * 1024
	execSessionStopTimeout           = 3 * time.Second
	execSessionEvictedMessage        = "[holt] session evicted: too many background terminals\n"
	execOutputBufferTruncatedMessage = "[holt] output buffer truncated: undrained output exceeded 2MiB, oldest output dropped"
)
...
func (tool execCommandTool) RunWithSandbox(ctx context.Context, args map[string]any, engine *holtSandbox.Engine) Result {
...
func (tool execCommandTool) run(ctx context.Context, args map[string]any, engine *holtSandbox.Engine) Result {
	...
	if interactive := holtSandbox.DetectInteractiveCommand(commandText, runtimeGOOS()); interactive.Interactive {
...
func (tool execCommandTool) startSession(commandText string, absoluteCwd string, relativeCwd string, ttyRequested bool, engine *holtSandbox.Engine, sandboxPermissions SandboxPermissionOverride) (*execSession, error) {
