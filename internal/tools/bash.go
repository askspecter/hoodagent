...
import (
	...
	"github.com/askspecter/holt/internal/secrets"
)
...
func (tool bashTool) RunWithSandbox(ctx context.Context, args map[string]any, engine *holtSandbox.Engine) Result {
	return tool.run(ctx, args, engine)
}

func (tool bashTool) run(ctx context.Context, args map[string]any, engine *holtSandbox.Engine) Result {
	...
	if interactive := holtSandbox.DetectInteractiveCommand(commandText, runtime.GOOS); interactive.Interactive {
	...
	monitor := holtSandbox.StartDenialMonitor(context.Background(), plan.MonitorTag)
	err = command.Run()
...
func commandEngineForSandboxPermissions(engine *holtSandbox.Engine, sandboxPermissions SandboxPermissionOverride) *holtSandbox.Engine {
...
func buildBashCommand(ctx context.Context, commandText string, absoluteCwd string, engine *holtSandbox.Engine) (*exec.Cmd, holtSandbox.CommandPlan, error) {
	spec := holtSandbox.CommandSpec{
	...
	plan := holtSandbox.CommandPlan{
		Backend: holtSandbox.Backend{
			Name:    holtSandbox.BackendUnavailable,
...
func addSandboxMeta(meta map[string]string, plan holtSandbox.CommandPlan) {
...
func interactiveBlockResult(detection holtSandbox.InteractiveCommandResult) Result {
	message := fmt.Sprintf(
...
func formatBashOutput(stdout string, stderr string, exitCode int) string {
	...
	if n := len(outFindings) + len(errFindings); n > 0 {
		parts = append(parts, fmt.Sprintf("[holt] redacted %d likely secret(s) from this output before showing it.", n))
	}
	if len(parts) == 0 {
