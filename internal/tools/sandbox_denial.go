...
func markLikelySandboxDenial(meta map[string]string, plan holtSandbox.CommandPlan, exitCode int, sections ...string) {
...
func likelySandboxDenied(plan holtSandbox.CommandPlan, exitCode int, sections ...string) bool {
	kind, _ := sandboxDenialKind(plan, exitCode, sections...)
	return kind != ""
}

func sandboxDenialKind(plan holtSandbox.CommandPlan, exitCode int, sections ...string) (string, string) {
	if !plan.Wrapped {
		return "", ""
	}
	...
	if plan.TargetBackend == holtSandbox.BackendLinuxBwrap && exitCode == 128+31 {
		return SandboxDenialKindSandbox, "seccomp"
	}
	return "", ""
}

func networkDeniedBySandbox(plan holtSandbox.CommandPlan) bool {
	return plan.PermissionProfile.Network.Mode == holtSandbox.NetworkDeny || plan.Policy.Network == holtSandbox.NetworkDeny
}
