...
func detectShellCommandIssue(command string, goos string) *shellIssue {
	...
	if windowsBashStyleCDPattern.MatchString(trimmed) ||
		...
		return &shellIssue{
			...
			Message:    "Command looks like POSIX/Bash syntax, but Holt runs bash tool commands through Windows cmd.exe on this host.",
...
func detectShellOutputIssue(command string, output string, goos string) *shellIssue {
	...
	if strings.Contains(haystack, "the syntax of the command is incorrect") ||
		...
		return &shellIssue{
			...
			Suggestion: "Translate the command to Windows cmd.exe syntax, set the bash tool cwd argument instead of running cd, or prefer native Holt tools for file inspection.",
		}
	}
	return nil
}
...
func appendShellIssueHint(output string, issue shellIssue) string {
	...
	hint := "[holt] shell issue: " + issue.Message
