...
func TestCollectStreamWithOptionsEmitsTextReasoningAndUsageCallbacks(t *testing.T) {
	...
	go func() {
		...
		events <- StreamEvent{Type: StreamEventText, Content: "holt"}
		events <- StreamEvent{Type: StreamEventDone}
	}()
	...
	if collected.Text != "Hello holt" {
		t.Fatalf("text = %q, want Hello holt", collected.Text)
	}
	if len(textDeltas) != 2 || textDeltas[0] != "Hello " || textDeltas[1] != "holt" {
...
func TestCollectStreamFlushesOpenToolCallsWhenChannelCloses(t *testing.T) {
	...
	go func() {
		...
		events <- StreamEvent{Type: StreamEventToolCallDelta, ToolCallID: "call_closed", ArgumentsFragment: `holt"}`}
	}()
	...
	toolCall := collected.ToolCalls[0]
	if toolCall.ID != "call_closed" || toolCall.Name != "grep" || toolCall.Arguments != `{"query":"holt"}` {
