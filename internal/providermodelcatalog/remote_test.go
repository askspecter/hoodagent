...
func TestModelsDevProviderIDMapsZeroAliases(t *testing.T) {
	...
	for holtID, want := range tests {
		provider, ok := providercatalog.Get(holtID)
		if !ok {
			t.Fatalf("provider %q missing from catalog", holtID)
		}
		if got := ModelsDevProviderID(provider); got != want {
			t.Fatalf("ModelsDevProviderID(%q) = %q, want %q", holtID, got, want)
		}
	}
}
