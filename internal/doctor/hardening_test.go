...
func TestSandboxCheckReportsWindowsNativeSetupStates(t *testing.T) {
	...
	t.Setenv("HOLT_WINDOWS_SANDBOX_HOME", t.TempDir())
	...
	if setupStatus != "missing-or-out-of-date" || !strings.Contains(setupRemedy, "holt sandbox setup") {
	...
	// on PATH for the Windows backend to be usable — the running holt binary acts
	...
	// require a setup marker) the check passes. The actionable "run `holt sandbox
