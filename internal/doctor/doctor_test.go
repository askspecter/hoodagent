...
func TestRunReportFailsInvalidModelAndMissingProvider(t *testing.T) {
	...
	invalid := Run(Options{
		...
		Provider: config.ProviderProfile{
			...
			ProviderKind: config.ProviderKindOpenAI,
			BaseURL:      config.OpenAIBaseURL,
			Model:        "not-a-holt-model",
		},
	})
	if invalid.OK {
		t.Fatalf("invalid model should fail: %#v", invalid)
	}
	if check := invalid.Check("provider.model"); check == nil || check.Status != StatusFail || !strings.Contains(check.Message, "unknown Holt model") {
...
func TestProviderModelBuiltInUnknownModelFails(t *testing.T) {
	...
	check := report.Check("provider.model")
	if check == nil || check.Status != StatusFail || !strings.Contains(check.Message, "unknown Holt model") {
