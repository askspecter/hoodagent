...
func TestCheckReportsAvailableUpdate(t *testing.T) {
	...
	if !result.ReleaseAsset.Verified || result.ReleaseAsset.ArchiveName != "holt-v0.2.0-linux-x64.tar.gz" || result.ReleaseAsset.ChecksumName != "holt-v0.2.0-linux-x64.tar.gz.sha256" {
...
func TestCheckFallsBackReleaseURL(t *testing.T) {
	...
	wantURL := "https://github.com/askspecter/holt/releases/tag/v0.2.0"
...
func TestCheckResolvesEndpointPrecedence(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			options: Options{Endpoint: "Gitlawb/option-holt", Repository: "Gitlawb/repo-holt"},
			env:     "Gitlawb/env-holt",
			want:    Endpoint("Gitlawb/option-holt"),
		},
		{
			name:    "environment wins over repository",
			options: Options{Repository: "Gitlawb/repo-holt"},
			env:     "Gitlawb/env-holt",
			want:    Endpoint("Gitlawb/env-holt"),
		},
		{
			name:     "repository wins over default",
			options:  Options{Repository: "Gitlawb/repo-holt"},
			want:     Endpoint("Gitlawb/repo-holt"),
			clearEnv: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.clearEnv {
				t.Setenv("HOLT_UPDATE_RELEASE_URL", "")
			} else {
				t.Setenv("HOLT_UPDATE_RELEASE_URL", tt.env)
			}
			options := tt.options
...
func TestCheckRejectsMissingReleaseAssets(t *testing.T) {
	_, err := Check(context.Background(), Options{
		...
		Fetch: func(context.Context, string) (Release, error) {
			return Release{
				TagName: "v0.2.0",
				Assets: []Asset{
					{Name: "holt-v0.2.0-linux-arm64.tar.gz"},
				},
			}, nil
		},
	})
	...
	if !strings.Contains(err.Error(), "holt-v0.2.0-linux-x64.tar.gz") || !strings.Contains(err.Error(), "holt-v0.2.0-linux-x64.tar.gz.sha256") {
...
func TestExpectedAssetCheckUsesInstallerArchiveNames(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			goos:        "darwin",
			goarch:      "arm64",
			archiveName: "holt-v0.2.0-macos-arm64.tar.gz",
			platform:    "macos",
			arch:        "arm64",
		},
		{
			...
			goos:        "windows",
			goarch:      "amd64",
			archiveName: "holt-v0.2.0-windows-x64.zip",
			platform:    "windows",
			arch:        "x64",
		},
	}
...
func TestResolveEndpointAcceptsURLAndRepositorySlug(t *testing.T) {
	got, err := ResolveEndpoint("Gitlawb/alt-holt", DefaultRepository)
	if err != nil {
		t.Fatalf("ResolveEndpoint returned error: %v", err)
	}
	if got != Endpoint("Gitlawb/alt-holt") {
...
func TestFormatResult(t *testing.T) {
	...
	if !strings.Contains(output, "Release asset: holt-v0.2.0-linux-x64.tar.gz") || !strings.Contains(output, "Checksum asset: holt-v0.2.0-linux-x64.tar.gz.sha256") {
...
func releaseForTarget(t *testing.T, tag string, goos string, goarch string) Release {
	...
	return Release{
		TagName: tag,
		HTMLURL: "https://github.com/askspecter/holt/releases/tag/" + tag,
		Assets: []Asset{
