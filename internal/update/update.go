...
const (
	DefaultRepository = "askspecter/holt"
...
func Check(ctx context.Context, options Options) (Result, error) {
	...
	endpoint, err := resolveEndpoint(firstNonEmpty(options.Endpoint, os.Getenv("HOLT_UPDATE_RELEASE_URL")), repository)
	if err != nil {
		return Result{}, err
	}
...
func Format(result Result) string {
	if result.UpdateAvailable {
		lines := []string{
			fmt.Sprintf("[holt] Update available: %s -> %s", result.CurrentVersion, result.LatestVersion),
		...
		if target := releaseAssetTarget(result.ReleaseAsset); target != "" {
			lines = append(lines, "Download the verified "+target+" release asset, then replace the current holt binary.")
		} else {
			lines = append(lines, "Download the verified release asset, then replace the current holt binary.")
		}
		return strings.Join(lines, "\n")
	}
	lines := []string{
		fmt.Sprintf("[holt] up to date (%s)", result.CurrentVersion),
...
func fetchRelease(ctx context.Context, endpoint string) (release Release, err error) {
	...
	request.Header.Set("User-Agent", "holt/update")
...
func expectedAssetCheck(version string, goos string, goarch string) (AssetCheck, error) {
	...
	archiveName := fmt.Sprintf("holt-v%s-%s-%s.%s", version, platform, arch, extension)
	return AssetCheck{
		Platform:     platform,
		Arch:         arch,
