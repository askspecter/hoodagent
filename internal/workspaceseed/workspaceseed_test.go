...
func TestBuildOrdersAndClassifiesWorkspaceSeed(t *testing.T) {
	...
	if got.CWD != filepath.Clean(filepath.Join("tmp", "Holt")) {
	...
	if want := []string{"AGENTS.md", "HOLT.md", "README.md", "cmd/", "docs/", "go.mod", "internal/", "package.json"}; !reflect.DeepEqual(got.Layout, want) {
	...
	if want := []string{"AGENTS.md", "HOLT.md"}; !reflect.DeepEqual(got.MemoryFiles, want) {
		t.Fatalf("MemoryFiles=%v want %v", got.MemoryFiles, want)
	}
}

func TestBuildKeepsPathsRelativeToCWD(t *testing.T) {
	root := filepath.Join(t.TempDir(), "Holt")
	dirty := false
	got := Build(Input{
		CWD:       root,
		GitBranch: "main",
	...
	if !strings.Contains(rendered, "cwd: Holt") {
...
func TestBuildFromWorkspaceUsesWorkspaceIndexWithoutGit(t *testing.T) {
	root := t.TempDir()
	writeSeedFile(t, root, "go.mod", "module example.test/holt\n")
	writeSeedFile(t, root, "cmd/holt/main.go", "package main\n")
