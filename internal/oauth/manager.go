...
type ManagerOptions struct {
	...
	// operator exporting HOLT_OAUTH_ALLOW_PRESETS — used by the interactive wizard
	...
	// HOLT_OAUTH_<NAME>_* overrides. Leave false for hermetic tests.
	AllowPresets  bool
	Now           func() time.Time
...
func (m *Manager) resolveConfigForKey(ctx context.Context, key string) (Config, error) {
	...
	// provider configured with only HOLT_OAUTH_<NAME>_ISSUER_URL can still refresh
