...
type StoreOptions struct {
	...
	// keyring. When empty it falls back to HOLT_OAUTH_STORAGE.
	Storage string
...
// honoring HOLT_OAUTH_TOKENS_PATH, then XDG_CONFIG_HOME, then the home dir.
func ResolveStorePath(env map[string]string) (string, error) {
	if override := strings.TrimSpace(envValue(env, "HOLT_OAUTH_TOKENS_PATH")); override != "" {
	...
	return filepath.Join(configHome, "holt", "oauth-tokens.json"), nil
}

// NewStore builds a token store with the configured backend (file by default,
// or the OS keyring when Storage/HOLT_OAUTH_STORAGE selects it).
func NewStore(options StoreOptions) (*Store, error) {
	...
	if storage == "" {
		storage = strings.TrimSpace(envValue(options.Env, "HOLT_OAUTH_STORAGE"))
	}
...
// HOLT_OAUTH_* / HOME / XDG_CONFIG_HOME values. Only a nil map uses os.Getenv.
