...
// (as if HOLT_OAUTH_ALLOW_PRESETS=1 were exported) so an interactive wizard/CLI
...
// drop the operator's HOLT_OAUTH_<NAME>_* overrides. The flag is then forced on.
...
// provider. Every field is overridable per provider via HOLT_OAUTH_<NAME>_*
...
// with HOLT_OAUTH_ALLOW_PRESETS (see presetsAllowed). A preset carries a
...
// HOLT_OAUTH_<NAME>_* override is set.
//
...
// change without notice (override via HOLT_OAUTH_XAI_*), and using it requires a
...
var builtinOAuthPresets = map[string]providerPreset{
	...
	// pre-filled; the operator supplies HOLT_OAUTH_HUGGINGFACE_CLIENT_ID from
...
// are OFF unless the operator opts in with HOLT_OAUTH_ALLOW_PRESETS set to a
...
func presetsAllowed(env map[string]string) bool {
	switch strings.ToLower(strings.TrimSpace(envValue(env, "HOLT_OAUTH_ALLOW_PRESETS"))) {
	case "1", "true", "yes", "on":
		return true
	}
	return false
