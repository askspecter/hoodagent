...
func PrepareExec(options PrepareExecOptions) (PreparedExec, error) {
	...
	if forkID != "" {
		...
		if parent == nil {
			return PreparedExec{}, ExecError{"Holt session not found: " + forkID}
		}
	...
	if resumeID != "" || options.ResumeLatest {
		...
		if sessionID == "" && options.ResumeLatest {
			...
			if latest == nil {
				return PreparedExec{}, ExecError{"No Holt sessions available to resume."}
			}
		...
		if session == nil {
			return PreparedExec{}, ExecError{"Holt session not found: " + sessionID}
		}
	...
	if strings.TrimSpace(options.CallingSessionID) != "" {
		...
		if parent == nil {
			return PreparedExec{}, ExecError{"Holt parent session not found: " + parentSessionID}
		}
...
func readExecContextEvents(store *Store, sessionID string) ([]Event, error) {
	...
	log.Printf("holt sessions: failed to rehydrate compaction events for %s; falling back to raw events: %v", sessionID, err)
...
func FormatExecPrompt(prompt string, prepared PreparedExec) string {
	...
	return strings.Join([]string{
		fmt.Sprintf("%s Holt session %s.", label, sessionID),
