...
func (store *Store) PlanRewind(sessionID string, options RewindOptions) (RewindPlan, error) {
	if !ValidSessionID(sessionID) {
		return RewindPlan{}, fmt.Errorf("invalid holt session id %q", sessionID)
	}
	...
	if len(events) == 0 {
		return RewindPlan{}, fmt.Errorf("holt session %s has no events to rewind", sessionID)
	}
...
func (store *Store) PlanCompaction(sessionID string, options CompactionOptions) (CompactionPlan, error) {
	if !ValidSessionID(sessionID) {
		return CompactionPlan{}, fmt.Errorf("invalid holt session id %q", sessionID)
	}
	events, err := store.ReadEvents(sessionID)
	if err != nil {
...
func buildCompactionPrompt(events []Event, maxChars int) (string, bool) {
	if len(events) == 0 {
		return "No compactable Holt session events.", false
	}
	lines := []string{
		"Summarize these Holt session events for future context.",
