...
// disabled with HOLT_CHECKPOINTS=off).
func CheckpointsEnabled() bool {
	return os.Getenv("HOLT_CHECKPOINTS") != "off"
}

func maxCheckpointBytes() int {
	if raw := os.Getenv("HOLT_CHECKPOINT_MAX_BYTES"); raw != "" {
...
// caller. Returns the appended event (or a holt Event if there was nothing to do).
...
func (store *Store) pruneOrphanBlobs(sessionID string) (int, error) {
	if !ValidSessionID(sessionID) {
		return 0, fmt.Errorf("invalid holt session id %q", sessionID)
	}
	unlock, err := store.lockSession(sessionID)
	if err != nil {
		return 0, err
