...
func TestResolveLoopPromptPriority(t *testing.T) {
	...
	// home/.holt/loop.md is used when cwd has none
	mustWrite(t, filepath.Join(home, ".holt", "loop.md"), "HOME PROMPT")
	if got, _ = ResolveLoopPrompt(cwd, home); got != "HOME PROMPT" {
		t.Fatalf("home loop.md not used, got %q", got)
	}
	// cwd/.holt/loop.md wins over home
	mustWrite(t, filepath.Join(cwd, ".holt", "loop.md"), "CWD PROMPT")
...
func TestResolveLoopPromptRejectsSymlinkAndCap(t *testing.T) {
	...
	home := t.TempDir()
	// oversize file -> error
	big := filepath.Join(cwd, ".holt", "loop.md")
	...
	link := filepath.Join(cwd2, ".holt", "loop.md")
