# Holt Stream-JSON Protocol

Holt stream-json is a line-delimited protocol for headless clients such as
...
`holt exec --input-format stream-json` accepts these JSONL events from stdin or
`--file`.
...
Holt combines accepted input event content in order, separated by blank lines.
...
`holt exec --output-format stream-json` emits schema-versioned JSONL events.
...
is not pre-approved, Holt may emit `permission_request` followed by a denied
