...
Holt agent evals are maintainer fixtures for checking coding-agent behavior
...
`holt eval` defaults to `validate` mode. In validate mode, the command performs
...
```bash
go run ./cmd/holt eval --suite internal/agenteval/testdata/sample_suite.json
```

Use JSON output when another local tool needs the validation summary:

```bash
go run ./cmd/holt eval --suite internal/agenteval/testdata/sample_suite.json --json
```

### Run Mode

`holt eval run` scores one already-mutated Git workspace. It does not copy
...
go run ./cmd/holt eval run \
  ...
  --task document-stream-json-verify-events \
  --workspace /tmp/holt-eval-workspace \
  --report-dir /tmp/holt-eval-report \
  --json
```

### Bench Mode

`holt eval bench` runs the full benchmark harness for one task or a suite. Bench
...
```bash
go run ./cmd/holt eval bench \
  --suite internal/agenteval/testdata/sample_suite.json \
  --task document-stream-json-verify-events \
  --work-root /tmp/holt-evals \
  --agent-command holt exec --cwd {workspace} {prompt}
```
...
go run ./cmd/holt eval bench \
  ...
  --work-root /tmp/holt-evals \
  --model gpt-5 \
  --agent-command holt exec --model {model} --cwd {workspace} {prompt}
```
...
go run ./cmd/holt eval bench \
  --suite internal/agenteval/testdata/sample_suite.json \
  --work-root /tmp/holt-evals \
  --agent-command holt-agent-wrapper --task {task_id} --workspace {workspace} --prompt {prompt}
```
...
Scored reports use contract `holt.agenteval.report.v1`.
