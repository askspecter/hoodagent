# Holt Performance Benchmarks

The performance harness tracks three release-facing signals:

- Cold start: process startup time for `holt --version`.
- Binary first output: time from spawning the built `holt --version` command to
...
Cold start uses the built Go binary at `./holt` or `./holt.exe`. Run
`go run ./cmd/holt-release build` before the benchmark so it measures the
production runtime.
...
```bash
go run ./cmd/holt-perf-bench --output dist/perf/perf-bench.json
```
...
```bash
go run ./cmd/holt-perf-bench --cold-start-warn-ms=350 --first-output-warn-ms=600 --harness-end-rss-warn-mb=384
```

Or with environment variables:

```bash
HOLT_PERF_COLD_START_WARN_MS=350 go run ./cmd/holt-perf-bench
```

Supported environment variables:

- `HOLT_PERF_ITERATIONS`
- `HOLT_PERF_WARMUP_ITERATIONS`
- `HOLT_PERF_COLD_START_WARN_MS`
- `HOLT_PERF_FIRST_OUTPUT_WARN_MS`
- `HOLT_PERF_HARNESS_END_RSS_WARN_MB`

## CI Behavior

The `Performance Smoke` job builds the binary, runs
`go run ./cmd/holt-perf-bench --output dist/perf/perf-bench.json --ci`, and
