...
Specialists are named sub-agents that Holt can delegate focused work to through
...
| Scope | Path | Notes |
| --- | --- | --- |
| Built-in | compiled into Holt | `worker`, `explorer`, and `code-review` ship with the binary. |
| User | `~/.config/holt/specialists/*.md` | Available across local workspaces. |
| Project | `.holt/specialists/*.md` | Shared with the current repository when committed. |
...
holt specialist show worker
holt specialist path

holt specialist create api-review \
  --project \
  --description "Reviews API changes" \
  --tools read-only,plan \
  --prompt "Review API changes for compatibility and missing tests."

holt specialist edit api-review --project
holt specialist delete api-review --project
```
...
If the body is empty and `description` is set, Holt uses the description as the
system prompt and reports a warning in `holt specialist show`.
...
Holt registers these tools for top-level agent runs:
...
`.holt/specialists`, not the user specialist directory.
...
```text
${XDG_DATA_HOME:-~/.local/share}/holt/background/
```
...
If Holt is restarted while a background task is still marked `running`, the new
