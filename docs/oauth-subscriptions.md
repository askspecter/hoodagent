...
Holt supports two distinct things people mean by "log in with OAuth":

1. **OAuth login for a provider/gateway** that issues a *standard* bearer token —
   fully built in (`holt auth login …`). Holt attaches the token to model calls
   automatically.
2. **Using a ChatGPT or Claude *subscription*** (Plus / Pro / Max) instead of a
   ...
   reasons documented below. Holt ships a convenience preset and this recipe.
...
`Authorization: Bearer …` on its API, configure it with `HOLT_OAUTH_<NAME>_*`
env vars and log in:

```sh
export HOLT_OAUTH_ACME_CLIENT_ID=…
export HOLT_OAUTH_ACME_AUTHORIZE_URL=https://acme.example/oauth/authorize
export HOLT_OAUTH_ACME_TOKEN_URL=https://acme.example/oauth/token
export HOLT_OAUTH_ACME_SCOPES="openid profile"
holt auth login acme            # browser (loopback); --device for headless
holt auth status
```
...
`HOLT_OAUTH_STORAGE=keyring`) and never logged. See `holt auth --help`.
...
- **OpenRouter / xAI / ChatGPT / Hugging Face** are real OAuth: your browser
  ...
  `HOLT_OAUTH_ALLOW_PRESETS=1` or your own `HOLT_OAUTH_XAI_*`; see below.)
- **Device code (headless / SSH):** for a provider that supports it (xAI,
  ...
  (no `DISPLAY`) device code is used automatically; set `HOLT_OAUTH_DEVICE=1`
  to force it anywhere. The CLI equivalent is
  `holt auth login <name> --device`.
...
- **OpenRouter (no env needed)** — `holt auth openrouter` opens a browser, you
...
- **xAI (Grok) — opt-in preset** — xAI's flow needs an OAuth `client_id`. Holt
  ...
  Enable it with `export HOLT_OAUTH_ALLOW_PRESETS=1`, then `holt auth login xai`
  (browser, or `--device` for headless) works one-click; the token is used directly
  on `api.x.ai/v1`. Without the opt-in, set `HOLT_OAUTH_XAI_CLIENT_ID` (and
  endpoints, or an issuer) yourself via `HOLT_OAUTH_XAI_*`. Either way the preset is
  fully overridable by `HOLT_OAUTH_XAI_*` (env wins), and it requires a
  SuperGrok / X Premium+ subscription; the client_id is an undocumented public
  Grok-CLI client that may change without notice.
- **ChatGPT (Codex) — opt-in preset** — `holt auth chatgpt` opens a browser, you
  ...
  authorization server), the Codex backend will 401 and `holt auth status chatgpt`
  ...
  `HOLT_OAUTH_ALLOW_PRESETS=1`. As of mid-2026 the Codex backend is
...
- **Hugging Face — opt-in preset, BYO client_id** — `holt auth login huggingface`
  ...
  `client_id` via `HOLT_OAUTH_HUGGINGFACE_CLIENT_ID`. Enable the preset with
  `HOLT_OAUTH_ALLOW_PRESETS=1` (or omit it — the BYO client_id path uses
  `client_credentials = none` and doesn't need the opt-in). Free tier has strict
  rate limits; Pro removes them.

Any field of a preset is overridable via `HOLT_OAUTH_<NAME>_*`. For a fully custom
OAuth/OIDC provider, set those env vars (see `holt auth --help`) and
`holt auth login <name>`.
...
- **OpenAI (ChatGPT):** a "Sign in with ChatGPT" token only works against
  ...
  **First-class path (this version):** `holt auth chatgpt` does exactly that
...
So Holt does **not** call those backends directly or spoof those clients — that
...
`127.0.0.1`. The proxy absorbs the Cloudflare / client-spoofing surface; Holt
just points at it.
...
```jsonc
// ~/.config/holt/config.json (or ./.holt/config.json)
{
...
```sh
holt exec --prompt "say hi"   # routes through the proxy → your ChatGPT plan
```
