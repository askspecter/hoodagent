...
This is the **task** benchmark: how often HOLT completes a real coding task
...
HOLT actually contributes: the agent noticing its own broken edit and fixing it
...
| `commit`         | HOLT commit the run was built from                            |
...
## Integration point: headless `holt exec`

The harness drives HOLT through its headless surface — the same path CI uses:

```bash
holt exec --output-format stream-json --model <model> [--self-correct] "<task prompt>"
```
...
[`cmd/holt-perf-bench/testdata/terminal-bench-sample.json`](../cmd/holt-perf-bench/testdata/terminal-bench-sample.json):
...
SUITE=cmd/holt-perf-bench/testdata/terminal-bench-sample.json
MODEL=<your-model>

# baseline: self-correct OFF
go run ./cmd/holt-perf-bench tasks \
  --suite "$SUITE" --binary ./holt --model "$MODEL" \
  --version "$VERSION" --commit "$COMMIT" \
  --output dist/bench/tasks-baseline.json

# self-correct ON (auto-fix needs --auto medium or high; see note below)
go run ./cmd/holt-perf-bench tasks \
  --suite "$SUITE" --binary ./holt --model "$MODEL" --self-correct \
  --version "$VERSION" --commit "$COMMIT" \
  --output dist/bench/tasks-selfcorrect.json
```

`--version`/`--commit` also read from `HOLT_BENCH_VERSION` / `HOLT_BENCH_COMMIT`
