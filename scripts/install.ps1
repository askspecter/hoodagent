[CmdletBinding()]
param(
  [string]$Version = $env:HOLT_VERSION,
  [string]$Repository = $(if ($env:HOLT_REPO) { $env:HOLT_REPO } else { "askspecter/holt" }),
  [string]$InstallDir = $env:HOLT_INSTALL_DIR,
  [string]$GitHubApi = $(if ($env:HOLT_GITHUB_API) { $env:HOLT_GITHUB_API } else { "https://api.github.com" }),
  [string]$GitHubBaseUrl = $(if ($env:HOLT_GITHUB_BASE_URL) { $env:HOLT_GITHUB_BASE_URL } else { "https://github.com" })
)
...
if ([string]::IsNullOrWhiteSpace($InstallDir)) {
  $InstallDir = Join-Path $env:LOCALAPPDATA "holt\bin"
}
...
function Find-HoltExtractedFile {
  param(
    [string]$Root,
    [string]$FileName
  )
...
function Find-HoltOptionalExtractedDirectory {
  param(
    [string]$Root,
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("holt-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  Write-Host "Installing Holt $tag for windows-$arch"
  ...
  foreach ($fileName in $requiredFiles) {
    $sourcePath = Find-HoltExtractedFile -Root $extractDir -FileName $fileName
    Copy-Item -Path $sourcePath -Destination (Join-Path $InstallDir $fileName) -Force
  }
  $helpersPath = Find-HoltOptionalExtractedDirectory -Root $extractDir -DirectoryName "helpers"
  ...
  $targetPath = Join-Path $InstallDir "holt.exe"
  ...
  if (-not (Test-ZeroPathContainsDir -PathValue $userPath -Dir $InstallDir)) {
    try {
      ...
      Write-Host "Added $InstallDir to your user PATH. Restart your terminal to use 'holt'."
    } catch {
      Write-Warning "Could not update your user PATH automatically: $_"
      Write-Warning "Add $InstallDir to PATH manually to run holt from any directory."
    }
  }
