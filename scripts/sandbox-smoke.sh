...
export GOCACHE="${GOCACHE:-/tmp/holt-go-cache}"
...
for goos in linux darwin windows; do
  ...
  compile_pkg "$goos" ./cmd/holt
done
compile_pkg linux ./cmd/holt-linux-sandbox
compile_pkg linux ./cmd/holt-seccomp
build_cmd windows ./cmd/holt-windows-command-runner holt-windows-command-runner.exe
build_cmd windows ./cmd/holt-windows-sandbox-setup holt-windows-sandbox-setup.exe

case "$(go env GOOS)" in
  linux)
    HOLT_SANDBOX_REAL_SMOKE=1 go test ./internal/sandbox -run 'TestLinuxHelperRealSandboxSmoke|TestLinuxLandlockRealSandboxSmoke' -count=1
    ;;
  darwin)
    go test ./internal/sandbox -run TestSandboxExecProfileAllowsDevNullAndTemp -count=1
    ;;
  windows)
    HOLT_SANDBOX_REAL_SMOKE=1 \
      HOLT_WINDOWS_COMMAND_RUNNER_EXE="$tmpdir/windows-holt-windows-command-runner.exe" \
      HOLT_WINDOWS_SANDBOX_SETUP_EXE="$tmpdir/windows-holt-windows-sandbox-setup.exe" \
