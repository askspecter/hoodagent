#!/usr/bin/env node
// postinstall: fetch the prebuilt `holt` binary for this platform from the
// matching GitHub Release and place it next to bin/holt.js, which execs it.
//
...
//   holt-v{version}-{linux|macos|windows}-{x64|arm64}.{tar.gz|zip}  (+ .sha256)
//
...
//   HOLT_SKIP_DOWNLOAD=1      skip entirely, exit 0 (wrapper will guide if run)
//   HOLT_INSTALL_DRY_RUN=1    print the resolved plan as JSON, no network, exit 0
//   HOLT_INSTALL_PLATFORM=…   override process.platform (linux|darwin|win32|android)
//   HOLT_INSTALL_ARCH=…       override process.arch (x64|arm64)
//   HOLT_REPO=owner/name      override the GitHub repo (default askspecter/holt)
//   HOLT_GITHUB_BASE_URL=…    override the download host (default https://github.com)
...
const REPO = process.env.HOLT_REPO || 'askspecter/holt';
const BASE = (process.env.HOLT_GITHUB_BASE_URL || 'https://github.com').replace(/\/+$/, '');
...
const ALLOW_INSECURE = process.env.HOLT_ALLOW_INSECURE_DOWNLOAD === '1';
const MAX_DOWNLOAD_BYTES = 512 * 1024 * 1024;

function fail(message) {
  console.error(`[holt] ${message}`);
  process.exit(1);
}

function warnSkip(message) {
  // Exit 0 so an unsupported platform or an opt-out does not break `npm install`;
  // bin/holt.js reports a clear "no native binary" message if the user runs holt.
  console.error(`[holt] ${message}`);
  process.exit(0);
}
...
if (process.env.HOLT_SKIP_DOWNLOAD === '1') {
  warnSkip('HOLT_SKIP_DOWNLOAD=1 set — skipping native binary download.');
}

const rawPlatform = process.env.HOLT_INSTALL_PLATFORM || process.platform;
const rawArch = process.env.HOLT_INSTALL_ARCH || process.arch;
...
if (!platform || !arch) {
  warnSkip(
    `no prebuilt binary for ${rawPlatform}/${rawArch}. Build from source: ` +
      `https://github.com/${REPO} (go run ./cmd/holt).`,
  );
}
...
const binaryName = platform === 'windows' ? 'holt.exe' : 'holt';
const tag = `v${VERSION}`;
const assetName = `holt-v${VERSION}-${platform}-${arch}.${ext}`;
...
const optionalBinaries =
  platform === 'windows'
    ? ['holt-windows-command-runner.exe', 'holt-windows-sandbox-setup.exe']
    : platform === 'linux'
      ? ['holt-linux-sandbox', 'holt-seccomp']
      : [];

if (process.env.HOLT_INSTALL_DRY_RUN === '1') {
...
const markerPath = join(packageRoot, '.holt-binary-version');
  ...
  if (!ALLOW_INSECURE && !url.startsWith('https://')) {
    fail(
      `refusing insecure download origin for ${url}: only https:// is allowed ` +
        `(set HOLT_ALLOW_INSECURE_DOWNLOAD=1 to override for local testing).`,
    );
  }
  let response;
  try {
  ...
  if (!response.ok) {
    fail(
      `download failed (HTTP ${response.status}) for ${url} (release tag ${tag}). ` +
        ...
        `build from source: https://github.com/${REPO} (go run ./cmd/holt).`,
    );
  }
...
async function main() {
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'holt-install-'));
  try {
    ...
    for (const name of optionalBinaries) {
      ...
      if (!source) {
        console.error(
          `[holt] note: optional helper ${name} was not in ${assetName}; ` +
    ...
    console.error(`[holt] installed ${binaryName} ${VERSION} (${platform}-${arch}, ${sizeKb} KB).`);
  } finally {
